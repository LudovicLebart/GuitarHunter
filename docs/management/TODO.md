...
- [ ] **Piste : remplacer la boucle 22 villes Facebook par des points d'ancrage géographiques** *(Validé empiriquement le 2026-08-24, non implémenté — voir `JOURNAL.md`)*
    - *Détails :* `_run_facebook_scan()` lance une recherche Facebook complète par ville configurée (22 requêtes/cycle). Facebook applique déjà un rayon implicite non documenté (~40-80km) autour de la ville recherchée — validation lecture seule (`backend/scripts/audit_anchor_coverage.py`, aucune requête Facebook) : 2 ancrages (Longueuil, Saint-Bruno-de-Montarville) à 80km couvrent 99.7% des annonces déjà indexées, soit une réduction potentielle d'environ 91% du volume de requêtes.
    - *Reste à faire avant implémentation* : (1) fiabiliser l'estimation du rayon au-delà des 3 valeurs fixes testées — piste retenue : exploiter le signal déjà loggué `matched_other_city` (`bot.py:560-577`, une annonce trouvée en cherchant la ville X mais localisée dans la ville Y prouve directement que le rayon ≥ distance(X,Y)) comme borne inférieure convergente, combiné à un balayage de rayon élargi pour repérer le plateau de saturation (borne haute) ; (2) extraire l'algorithme de clustering (couverture d'ensembles gloutonne, pur/site-agnostique) dans un module partagé, réutilisable par Kijiji (`radius_km` déjà explicite et respecté par le site — couverture garantie par construction) et potentiellement LeBonCoin (à vérifier si `locations=` accepte un format géo, sinon même modèle "rayon implicite" que Facebook).
    - *Non commencé* : aucun changement de `bot.py`/`scan_marketplace()` à ce jour — discussion à l'étape explication/design, pas encore de "go" utilisateur sur le code.
