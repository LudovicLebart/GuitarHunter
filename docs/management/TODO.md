...
- [x] **Lot 2 — Requalification d'annonce par Gemini via le chat** *(codé 2026-08-27)* : function calling (`propose_deal_requalification`) + carte de proposition avant/après (`RequalificationProposalCard`) + Appliquer/Ignorer ; "Appliquer" réutilise le pipeline `ANALYZE_DEAL` existant (`retryDealAnalysis` avec la correction formatée en commentaire), aucun nouveau champ Firestore lu ailleurs que le message de chat lui-même. **Reste à faire : valider en conditions réelles** (pas d'accès Gemini/Firebase depuis l'environnement de dev) — proposition cohérente sur désaccord factuel, Annuler sans effet, Appliquer déclenche bien la ré-analyse.
