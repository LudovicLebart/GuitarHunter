...
- [ ] **Recalibrer le proxy `cropped_suspect`** (§8ter/§6 du plan) — se déclenche sur ~48% des photos bien détectées, probablement trop strict pour des guitares bien cadrées qui remplissent déjà 70-90% de la hauteur du cadre. Le chiffre `measurable` actuel (16,5%) est sous-estimé, à ne pas utiliser tel quel.
