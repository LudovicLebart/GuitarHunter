...
- [x] **Corriger + valider le filtre d'utilisabilité Phase 0 à l'échelle** — bugs techniques (buffering/écriture/timeout) et de documentation (audit du 2026-08-24) corrigés, run de validation réussi sur échantillon aléatoire (100 annonces/565 photos). Décision : pas besoin du run complet à 5974 photos pour valider la fiabilité des algos — reporté à quand un corpus de production sera réellement nécessaire.
- [x] **Corriger le proxy `cropped_suspect`** (§8quater du plan) — retiré de `measurable` après inspection visuelle (testait "instrument entier visible" au lieu de "zone sillet→chevalet visible"). `measurable` recalculé : 93,0% sur 1re photo, 50,4% sur l'ensemble du carrousel.
- [x] **Étendre les requêtes OWLv2 (sillet/touche/chevalet)** (§9 du plan) — toujours pas de localisation fiable. "the nut of a guitar neck" la plus prometteuse mais insuffisante seule ; "the saddle of a guitar bridge" activement trompeuse (verrouillée sur la tête).
- [x] **Tester SAM 2.1 en segmentation point-guidée** (§10 du plan) — nettement plus précis qu'OWLv2 (contour au pixel près) mais pas fiable en automatique : le masque idéal "manche+corps sans la tête" sort en 1re position sur les 2 photos acoustiques test, absent des 3 candidats sur les 2 photos électriques test.
- [ ] **Affiner le point de départ SAM 2.1 (localisation manche/sillet/chevalet, §10 du plan)** — piste non testée : point plus précis (pas dérivé d'une boîte OWLv2 imprécise) + un 2e point négatif sur la tête pour forcer son exclusion, avant de conclure que SAM 2.1 ne peut pas être automatisé pour ce besoin.
- [x] **Outillage Dataset B** (§5 du plan) — 2 fiches imprimables (protocole cale/photo + journal de session, `docs/assets/dataset_b_protocole/`) et base locale CSV→SQLite facile à mettre à jour (`backend/scripts/dataset_b/`). Revu 2 fois le même jour : d'abord après retour utilisateur (Fiche 2 en tableau de session), puis après consultation Opus+Fable (sens de la cale tranché par l'utilisateur, contrôles relief/répétabilité, vues rasantes, photos "type annonce" ramenées à 3-5/guitare/séance au lieu de systématisées par configuration).
- [ ] **Sprint d'annotation manuelle sur Dataset A** (§6 du plan, recommandé par Opus + Fable) — annoter à la main 100-200 photos tirées au sort (points sillet/12e frette/chevalet + tag exploitable oui/non), 1-2h estimées. Ferait passer les conclusions OWLv2/SAM 2.1 de n=4 à n=100+, vraie prochaine étape pour la localisation.
- [ ] **Mesurer + photographier les 2 guitares de l'utilisateur** (fiches prêtes, `docs/assets/dataset_b_protocole/`) — dont la guitare à manche vissé, en testant plusieurs épaisseurs de cale pour obtenir plusieurs configurations d'angle mesurées. Reporter les résultats dans `backend/scripts/dataset_b/*.csv`.
