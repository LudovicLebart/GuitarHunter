...
- [/] **Appliquer la détection de présence "guitare" (Phase 0) à l'échelle du Dataset A complet** (5974 photos), pas seulement l'échantillon de validation à 8 photos (§7 étape 4 du plan). *Run du 2026-08-20 (5974 photos, toutes images/annonce) tué par le timeout CI de 40 min — zéro résultat récupéré (sortie Python bufferisée, fichier écrit seulement en fin de script). Correctifs nécessaires avant de relancer : sortie non bufferisée (`python3 -u`), écriture incrémentale du résultat, timeout remonté (~100 min, débit mesuré ~1,27 image/s → ~78 min requises).*
