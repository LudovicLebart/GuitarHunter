...
- [/] **Fiabilité de la classification taxonomique** *(correctif majeur 2026-08-16, en attente de validation utilisateur)*
    ...
    - [/] **Réparation de l'existant** — le correctif agit à la lecture, il ne réécrit rien : une valeur ambiguë cesse d'être mal rangée mais bascule en « Autres », et une erreur réelle de l'IA reste une erreur.
        ...
        - [ ] **Rejouer en écriture** (`run_once.py`, `dry_run=False`) pour normaliser les 328 + ≈27, puis **repasser `ACTIVE = False` dans un commit séparé**.
        - [ ] **Reste ensuite ≈9 annonces réellement ambiguës** (étuis nommés « Guitare Electrique »/« Guitare Acoustique ») + ≈41 placeholders IA sur des non-instruments (aucune valeur, le Portier les a rejetés). Les 9 se corrigent d'un clic via `ClassificationEditor` — **option (c) « heuristique sur le titre » abandonnée** : trop peu de volume pour justifier une devinette.
