...
- [/] **Dette technique : base commune scraper LeBonCoin/Facebook/Kijiji (mesures anti-bot)** *(Ajouté 2026-07-22, remonté en priorité haute puis codé le 2026-08-24 sur `claude/scraper-stealth-mutualization`, non fusionné — voir `JOURNAL.md`)*
    - *Détails d'origine :* `backend/scraping_leboncoin/core.py` (`LeboncoinScraper`) dupliquait des éléments déjà présents dans `backend/scraping/core.py` (`FacebookScraper`) : listes UA/viewports, flags de lancement stealth, cycle de vie de session. `KijijiScraper` n'avait lui-même aucune rotation de proxy.
    - *Fait* : nouveau module `backend/scraping/stealth.py` — pools UA/viewport fusionnés, flags de lancement communs, sélection de proxy, `human_pause`/`simulate_scroll_and_mouse` (délais/scroll aléatoires, portés depuis LeBonCoin vers Facebook/Kijiji — jamais un délai fixe). Choix délibéré : fonctions/constantes partagées plutôt qu'une classe de base (les 3 scrapers ont des besoins de session réellement différents), documenté dans le module. 76/76 tests scrapers passent.
    - *Pas fait (hors périmètre, décision explicite)* : extension de la rotation de proxy à Kijiji — "pas la peine d'investir plus dans les proxies" (aucun proxy réel configuré à ce jour, voir point plus bas).
    - *Reste à faire* : validation en conditions réelles (aucun accès Facebook/Kijiji/LeBonCoin depuis l'environnement de dev), puis fusion dans `dev`.
...
- [/] **Cadence de scan calquée sur un rythme humain (pas d'activité nocturne)** *(Ajouté 2026-07-22, remonté en priorité haute le 2026-08-24)*
    - *Détails :* Un bot qui scanne à un rythme uniforme 24h/24 (y compris la nuit, ex: 3h du matin) est lui-même un signal comportemental détectable dans la durée — un humain ne consulte pas les petites annonces en pleine nuit.
    - [x] **Fréquence modulée ±20% (codé 2026-08-24, poussé sur `dev`)** : `backend/services.py::TaskScheduler` utilisait un intervalle strictement constant (`schedule.every(X).minutes`, jamais dévié) — remplacé par `schedule.every(a).to(b).minutes` (natif à la lib `schedule`, re-tiré au hasard après chaque exécution), ±20% autour de la fréquence configurée.
    - [ ] **Reste à faire : plage horaire nocturne réduite/coupée** — pas encore fait, décision produit à trancher (couperet ~00h-7h ou réduction progressive, variation aléatoire des bornes). Concerne potentiellement Facebook, Kijiji et une future intégration LeBonCoin.
