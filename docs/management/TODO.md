...
- [x] **Annonce Facebook active jamais capturée (id 4380960138835811, Saint-Bruno-de-Montarville, 295$)** *(Signalé 2026-08-25, cause racine confirmée et corrigée le même jour)*
    ...
    - *Remplissage international (annonces belges) toujours présent après ce correctif* : l'utilisateur a isolé le mode **headless** comme différence structurelle restante (son propre test en navigateur anonyme, sans compte, ne montre jamais ce remplissage). `ScraperConfig.headless` passé à `False` (2026-08-25) — LeBonCoin tourne déjà ainsi avec succès sur ce même service (Xvfb déjà installé). **Risque non vérifiable depuis le sandbox de dev** : si `DISPLAY` n'est pas réellement disponible pour l'ensemble du process, Facebook pourrait échouer au lancement — vérifier en priorité au prochain déploiement.
    - *Reste à faire* : valider en conditions réelles après fusion (1) que le scan Facebook démarre sans erreur en `headless=False`, (2) que le remplissage international disparaît, (3) que l'annonce (ou une similaire) remonte enfin dans un cycle automatique.
