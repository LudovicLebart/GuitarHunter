...
- [x] **Calibration d'une approche Playwright "douce" face à DataDome** *(Réussie 2026-07-21)*
    ...
    - [x] **Arrêt anticipé de la pagination via `known_ids` (2026-07-22)** : le tri `sort=time&order=desc` garantissant "plus récent d'abord", la pagination s'arrête dès qu'une page ne contient plus aucune annonce inconnue — inutile de vérifier les pages suivantes. **Non testé en conditions réelles.**
...
- [ ] **Cadence de scan calquée sur un rythme humain (pas d'activité nocturne)** *(Ajouté 2026-07-22)*
    ...
    - [x] **Première implémentation (2026-07-22, mode `--soak-cycles` uniquement)** : `is_night_time()`/`seconds_until_active()` dans `backend/scraping_leboncoin/core.py`, calculées sur `Europe/Paris` explicitement (pas l'heure système de la machine) — plage 00h-7h, reprise à une heure variable (pas un couperet fixe). Exposées par le package, prêtes à réutiliser pour `bot.py`.
