...
- [x] **Filtres de type en multi-sélection** *(Ajouté 2026-07-31, bug de sélection en chaîne corrigé 2026-08-01)*
    ...
    - [ ] **Résiduel non corrigé :** `guitare.basse` partage des noms de catégories intermédiaires (`solid_body`, `specialites`) avec `guitare.electrique`/`guitare.acoustique_acier` dans `leafPaths` (dernière branche traversée gagne) — une classification IA réduite à ce seul nom de catégorie (sans modèle précis) résoudrait vers la mauvaise branche. Pas encore observé en pratique.
...
- [x] **Feature : couleur, finition (application/brillance) et longueur de manche extraites par l'IA + filtres** *(2026-08-01)*
    ...
    - [ ] **Résiduel non corrigé (signalé par la même revue) :** les options de `FilterDrawer.jsx` (`FINISH_APPLICATION_OPTIONS`/`FINISH_TEXTURE_OPTIONS`) sont dupliquées à la main depuis la liste fermée du prompt, plutôt que dérivées dynamiquement comme `TAXONOMY_TREE` l'est depuis `taxonomy_master` — risque de dérive silencieuse si la liste change côté prompt sans mise à jour manuelle ici. Nécessiterait de restructurer `prompts.json` pour que le texte d'instruction envoyé à Gemini et les options du filtre partagent une seule source (actuellement le texte de l'instruction IA est une simple chaîne de prose, pas un template interpolé). Pas fait dans cette passe (refactor plus large que ce correctif).
