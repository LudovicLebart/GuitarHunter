...
- [ ] **Cadence de scan calquée sur un rythme humain (pas d'activité nocturne)** *(Ajouté 2026-07-22)*
    ...
    - [ ] **Reste à faire** : câbler ça dans la vraie cadence de scan (`bot.py`/scheduling), pas seulement le script de test. Facebook tourne déjà 24h/24 via `TaskScheduler` à cadence fixe en minutes (`schedule.every(X).minutes`), sans notion de plage horaire — à évaluer si ça vaut la peine de l'appliquer aussi côté Facebook, ou seulement pour LeBonCoin où le risque de détection est plus aigu — décision produit à trancher avec l'utilisateur.
