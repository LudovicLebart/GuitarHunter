...
- [/] **Fiabilité de la classification taxonomique** *(correctif majeur 2026-08-16, en attente de validation utilisateur)*
    ...
    - [/] **Réparation de l'existant** — le correctif agit à la lecture, il ne réécrit rien : une valeur ambiguë cesse d'être mal rangée mais bascule en « Autres », et une erreur réelle de l'IA reste une erreur.
        ...
        - [ ] **Lire le rapport dans les logs GitHub Actions** après le déploiement — 1er passage volontairement en **`dry_run=True`** (lecture seule, aucune écriture), puis **repasser `ACTIVE = False` dans un commit séparé**.
        - [ ] **Rejouer ensuite en écriture** (`dry_run=False`) pour la normalisation gratuite, si les chiffres du rapport ne révèlent rien d'inattendu.
        - [ ] **Décider ensuite, chiffres en main** : (a) correction manuelle des cas ambigus si le volume est faible ; (b) ré-analyse Gemini ciblée des seules annonces non résolvables (squelette : `reanalyze_sold_deals.py`), coût à chiffrer ; (c) heuristique gratuite « titre contenant étui/case/housse/gig bag → `etui_housse.*` » sur les seules annonces en échec de résolution — **heuristique, à ne déclencher qu'avec accord explicite**.
