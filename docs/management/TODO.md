...
- [ ] **Annonce Facebook active jamais capturée (id 4380960138835811, Saint-Bruno-de-Montarville, 295$)** *(Signalé 2026-08-25)*
    - *Confirmé* : jamais écrite dans `guitar_deals` pour aucun utilisateur (`find_deal_by_id.py`, lecture directe). Antérieur aux points d'ancrage — l'ancre couvrant cette ville la recherche directement.
    - *Audit racine (agent Opus)* : deux pertes silencieuses les plus probables dans `scan_marketplace()` — égalité stricte de ville (`is_city_allowed()`) vs sous-chaîne côté `bot.py`, et cartes abandonnées sans log exploitable (localisation/prix non extraits). Instrumentation poussée (voir entrée ci-dessus) pour trancher par preuve directe.
    - *Reste à faire* : lire les logs du prochain cycle Facebook pour l'ancrage Saint-Bruno-de-Montarville (nouveaux logs `warning` + compteurs `dropped_no_location`/`dropped_no_price`/`total_cards_seen`), et/ou tester `SCAN_URL` directement sur cette annonce (bypass connu de tous les filtres géographiques).
