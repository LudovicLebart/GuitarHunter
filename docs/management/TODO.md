...
- [x] **Outillage Dataset B** (§5 du plan) — 2 fiches imprimables (protocole cale/photo + journal de session, `docs/assets/dataset_b_protocole/`) et base locale CSV→SQLite facile à mettre à jour (`backend/scripts/dataset_b/`). Corrigé le jour même après retour utilisateur : Fiche 2 refaite en tableau de session (1 ligne/config, pas 1 page/config) + section "photos type annonce" (conditions non contrôlées) systématisée à chaque configuration.
