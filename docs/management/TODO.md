...
- [x] **Calibration d'une approche Playwright "douce" face à DataDome** *(Réussie 2026-07-21)*
    ...
    - [x] **Comportement anti-détection approfondi + mode soak test (2026-07-22)** : suite à l'observation que les actions étaient trop prévisibles/saccadées, ajout de trajectoires de souris interpolées, scroll par paliers (18-32, affinés après retour utilisateur), pauses non-uniformes, actions décoratives supplémentaires (ouverture aléatoire d'annonce, ajout aux favoris — accepté de laisser s'accumuler pour l'instant, retour furtif à une page précédente), et un temps de lecture minimum par page (~45-75s) qui termine par une descente jusqu'aux boutons de pagination avant de changer de page. Nouveau mode `--soak-cycles` (intervalle 45-90min par défaut, aligné/au-dessus du rythme Facebook) avec pause automatique la nuit (`is_night_time()`/`seconds_until_active()`, heure de Paris explicite — réutilisable tel quel pour `bot.py`). **Testé et validé en conditions réelles** (un crash Chromium rencontré en cours de route — scroll de descente trop massif — corrigé et revalidé).
    - [x] **Arrêt anticipé de la pagination via `known_ids` (2026-07-22)** : le tri `sort=time&order=desc` garantissant "plus récent d'abord", la pagination s'arrête dès qu'une page ne contient plus aucune annonce inconnue — inutile de vérifier les pages suivantes. **Validé via `leboncoin_soak_log.jsonl` (2026-07-24)** : 1er cycle sans historique → 21 pages/718 annonces (~26min) ; cycles suivants → 1-2 pages (~1.5-2.5min) dès que le contenu déjà vu est retrouvé — comportement exactement conforme à l'attendu.
    - [x] **Fix : crash Chromium + navigation interrompue (2026-07-23/24)** : deux erreurs Playwright transitoires rencontrées lors de campagnes soak réelles (`Page crashed` puis `Navigation ... is interrupted by another navigation` causée par le retour furtif à la page précédente). `search()` retente désormais une fois toute navigation échouée (recréation de l'onglet si crash, sinon pause + retry), sans jamais laisser d'exception sortir de la méthode (contrat "toujours (annonces, blocage)" renforcé lors de la revue de code du 2026-07-24).
    - [x] **Revue de code complète 8-angles (2026-07-24)** : au-delà des deux fixes ci-dessus, plusieurs faux positifs de blocage corrigés (403/429 de domaines tiers, réponses non re-nettoyées entre tentatives), fuite mémoire du listener de réponses (retenait l'objet complet de chaque requête), duplication mineure factorisée (`_move_mouse_to_element`, `_do_search`). Étendue au reste du chantier (`bot.py`/`notifications.py`) : distinction BAD_DEAL/REJECTED réparée dans les codes de sortie de `handle_deal_found()`, notification de scan manuel corrigée pour une mise à jour de prix (n'annonce plus "nouvelle annonce"), `total_cards_seen`/outcomes "unknown" enfin visibles dans le résumé de cycle. Détail complet dans JOURNAL.md.
    - [x] **Intégration réelle dans `bot.py` (2026-07-24)** : `run_leboncoin_scan()` réutilise `handle_deal_found()` tel quel (dédup, pré-filtres, IA, Firestore, notifications) via l'adaptateur `_map_leboncoin_ad()` — pas de pipeline dupliqué. Compte personnel unique (`leboncoinConfig.enabled: false` par défaut pour tout le monde). Cadence indépendante de Facebook, jitterisée +/-30% (voir point cadence ci-dessous). `known_ids` réel via `repository.py::get_known_deal_ids()` (20 chunks de `deals_index`). **Non testé en conditions réelles** (pas de session LeBonCoin/Firestore disponible depuis l'environnement de développement) — validation à la charge de l'utilisateur. Pour activer : `leboncoinConfig.enabled: true` + déposer `leboncoin_storage_state.json` sur le serveur (chemin : `LEBONCOIN_STORAGE_STATE_PATH`, config.py).

- [x] **Dette technique : base commune scraper (mesures anti-bot)** *(Ajouté 2026-07-22, revue de code — extrait le 2026-07-24)*
    - *Détails :* `backend/scraping_leboncoin/core.py` (`LeboncoinScraper`) dupliquait des éléments déjà présents dans `backend/scraping/core.py` (`FacebookScraper`) : listes UA/viewports, flags de lancement stealth, cycle de vie de session (`start_session`/`close_session`/`_ensure_session`).
    - [x] **Extraction (2026-07-24)** : nouveau package `backend/scraping_common/` — `human_behavior.py` (`is_night_time`, `seconds_until_active`, `HumanBehaviorMixin` : pauses/souris/scroll/survol d'élément) et `base_scraper.py` (`BaseMarketplaceScraper` : cycle de vie de session, UA/viewports par défaut, `storage_state_path` optionnel). `LeboncoinScraper` en hérite désormais ; pensé pour être réutilisé tel quel par de futurs modules (Kijiji, Reverb...).
    - [ ] **`FacebookScraper` n'a volontairement PAS été migré** — code en production, migration délibérément laissée pour un chantier séparé (proxy, géolocalisation, en-têtes spécifiques à gérer). Reste à faire si on veut appliquer les mêmes règles anti-détection (cadence, plages horaires) aux deux scrapers.

- [ ] **Cadence de scan calquée sur un rythme humain (pas d'activité nocturne)** *(Ajouté 2026-07-22)*
    - *Détails :* Un bot qui scanne à un rythme uniforme 24h/24 (y compris la nuit, ex: 3h du matin) est lui-même un signal comportemental détectable dans la durée — un humain ne consulte pas les petites annonces en pleine nuit. Point soulevé par l'utilisateur.
    - [x] **Première implémentation (2026-07-22, mode `--soak-cycles` uniquement)** : `is_night_time()`/`seconds_until_active()`, désormais dans `backend/scraping_common/human_behavior.py` (déplacées le 2026-07-24 lors de l'extraction de la base commune), calculées sur `Europe/Paris` explicitement (pas l'heure système de la machine) — plage 00h-7h, reprise à une heure variable (pas un couperet fixe). Exposées par le package, prêtes à réutiliser pour `bot.py`.
    - [x] **Câblé côté LeBonCoin dans `bot.py` (2026-07-24)** : `run_leboncoin_scan()` vérifie `is_night_time()` avant chaque cycle (skip si nuit) ; la cadence elle-même est jitterisée +/-30% autour de la fréquence de scan Facebook courante (`services.py::TaskScheduler`, jamais un intervalle fixe).
    - [ ] **Reste à faire côté Facebook** : `FacebookScraper`/son `TaskScheduler` tournent toujours 24h/24 à cadence fixe en minutes, sans notion de plage horaire ni de jitter — à évaluer si ça vaut la peine de l'appliquer aussi côté Facebook, ou si le risque de détection plus aigu de LeBonCoin (DataDome) justifie de le garder spécifique — décision produit à trancher avec l'utilisateur.
...
- [x] **Feature : Scraper Kijiji autonome, validé en conditions réelles** *(Module créé 2026-07-26, intégré au pipeline et validé de bout en bout le 2026-07-27)*
    ...
    - [ ] **Écarts assumés vs `FacebookScraper`** : pas de rotation de proxy (le filtre par ville est couvert par `locations.py`, le filtre géo/prix par `build_search_url()` — voir ci-dessus).
