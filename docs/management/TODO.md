...
## 🛒 Multi-plateforme — Scraper Kijiji (2026-07-26, validé en live le 2026-07-27)

- [x] **Feature : Scraper Kijiji autonome, validé en conditions réelles** *(Module créé 2026-07-26, validé live 2026-07-27, toujours non branché au pipeline)*
    ...
    - [x] **Mitigation de l'imprécision `location` : tri par GPS** *(2026-07-27)* — `locations.py::nearest_configured_city(latitude, longitude, city_coordinates, max_radius_km=None)` calcule la ville configurée la plus proche par distance Haversine (coordonnées GPS de l'annonce, toujours présentes/fiables, contrairement à `location.name`). **Utilitaire disponible mais pas encore appelé** par le pipeline de scan — reste à décider la forme du futur `scan_config` Kijiji (liste de villes + rayons, à l'image du modèle Facebook) avant de le brancher.
    ...
    - [ ] **Intégration `bot.py`** (hors scope de cette itération) : brancher `KijijiScraper` dans `run_scan()`, définir un `scan_config` Kijiji dédié côté Firestore/ConfigPanel (y compris la forme attendue par `nearest_configured_city` — villes + rayons), décider de la fusion ou de la séparation des annonces Facebook/Kijiji dans `guitar_deals`.
    - [ ] **Mapping de catégories** : seule la catégorie 613 (Guitars) est connue/utilisée pour l'instant ; ajouter les ID pertinents (amplis, étuis...) si besoin lors de l'intégration.
    ...
    - [ ] **Écarts assumés vs `FacebookScraper`** : pas de rotation de proxy, pas de géolocalisation forcée (le filtre par ville, lui, est désormais couvert par `locations.py`, contrairement à l'évaluation initiale du 2026-07-26).
