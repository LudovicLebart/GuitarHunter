...
- **Nouveau (§8ter) : proxy `cropped_suspect` mal calibré** — se déclenche sur ~48% des photos de première position pourtant bien détectées, probablement parce qu'une guitare bien cadrée en vertical occupe déjà 70-90% de la hauteur du cadre (§3quater) et touche donc naturellement le haut/bas de l'image sans être coupée. Le chiffre `measurable` qui en dépend (16,5% sur l'échantillon, 49% sur 1re photo) est sous-estimé — à ne pas citer comme mesure d'exploitabilité tant que le proxy n'est pas revu (élargir la marge, ou ne flaguer qu'un bord réellement inhabituel plutôt que haut/bas systématiquement).
