...
**Étape 2 — accès SSH : pas encore fait, décision à prendre.** MoneyBot dispose déjà d'un accès SSH fonctionnel au Dell (`moneybot/.github/workflows/deploy.yml`, secrets `SERVER_IP`/`SERVER_USER`/`SSH_PRIVATE_KEY` **du dépôt MoneyBot** — utilisateur `ludovic`, clé autorisée sur le Dell). Ces secrets sont scopés par dépôt : GuitarHunter ne peut pas les lire, même si le nom est identique à ses propres secrets `SERVER_IP`/`SERVER_USER`/`SSH_PRIVATE_KEY` (qui pointent vers le serveur de production GuitarHunter, une tout autre machine). Deux options pour donner à GuitarHunter un accès SSH au Dell, non tranchées :
  - (a) copier la valeur de la clé privée déjà autorisée (secret MoneyBot `SSH_PRIVATE_KEY`) dans un nouveau secret **GuitarHunter** (ex. `DELL_SSH_PRIVATE_KEY`) — zéro changement côté Dell, réutilise l'autorisation existante.
  - (b) générer une paire de clés dédiée à cet usage et ajouter la clé publique aux `authorized_keys` du Dell — plus propre en isolation (clé révocable indépendamment), mais nécessite une action manuelle sur le Dell.
  Décision utilisateur nécessaire — la clé n'étant accessible qu'à l'utilisateur (secrets GitHub, `authorized_keys` du Dell), aucune de ces deux actions n'est faisable directement depuis cette session.

**Étape 3 (bloquée par l'étape 2) :** une fois l'accès SSH en place, adapter `run_script_dell.yml` (actuellement un test de ping) en exécuteur de script Python sur le Dell (même patron que `run_script.yml`, cible différente) ; tester `nvidia-smi` avant tout script d'inférence.

**Étape 4 (bloquée par l'étape 3) :** valider en pratique que 8 Go de VRAM suffisent pour Florence-2/OWLv2/SAM 2.1 en inférence sur un petit échantillon du manifeste Dataset A (§3septies) avant tout traitement des 5974 photos.
