...
`backend/scripts/experiments/test_owlv2_parts_detection.py` : testé sur les 2 mêmes photos que le prototype Phase 1 ci-dessus (celle où le recadrage manuel fonctionnait, celle où il échouait), résultat **vérifié visuellement sur les images annotées**, pas seulement sur les scores :

- **"the soundhole of a guitar" se localise étonnamment bien dans les deux photos** — boîte petite et précise, directement sur la vraie rosace (scores 0.12-0.24, boîtes cohérentes). Résultat inattendu et potentiellement utile ailleurs (point d'ancrage géométrique, hors périmètre de cette investigation).
- **"the neck of a guitar" est incohérent d'une photo à l'autre** : sur la photo 1 (Fender, cadrage frontal), la boîte "neck" n'attrape en réalité que la zone tête/mécaniques (trop haute, ne descend pas jusqu'au corps) ; sur la photo 2 (Yamaha, angle de travers), elle couvre à peu près tout le manche visible, mais en incluant encore la tête — imprécise sans être totalement fausse.
...
**Conclusion honnête : cette piste ne résout pas le problème de localisation du manche.** OWLv2 fonctionne bien en présence/absence globale (§7) mais son vocabulaire visuel ne semble pas distinguer finement les parties anatomiques d'une guitare avec de simples requêtes textuelles — au moins pas "neck"/"headstock"/"bridge" formulées ainsi. Non testé : d'autres formulations de requête (ex. "the fretboard", "the fingerboard with frets"), un prompt combinant plusieurs synonymes, ou un modèle de segmentation dédié (SAM 2.1, jamais testé sur le Dell — Florence-2 a déjà échoué pour une raison différente, §7 étape 4). **Piste de la détection automatique du manche toujours non résolue** — ajouté à §6, points ouverts.
