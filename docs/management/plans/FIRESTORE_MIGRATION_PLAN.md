...
2. ✅ **Frontend → nouvelle API — construit et validé statiquement (2026-09-11)** : `src/services/apiService.js`, mêmes 44 fonctions/signatures que `firestoreService.js` (sauf `migrateOldDataToNewUser`, restée sur l'ancien fichier). 3 trous backend trouvés en traçant les 44 fonctions et comblés (config utilisateur/botStatus, corrections manuelles d'analyse IA, galerie photo du chat) — 93 tests au total. **Aucun hook/composant ne l'importe encore** — construit en isolation, le câblage réel (et l'adaptation de `useDealsManager.js`, plus de sharding `deals_index`) est un pas distinct, pas cette étape. Voir `JOURNAL.md` 2026-09-11.
