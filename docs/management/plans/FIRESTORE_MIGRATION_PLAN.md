...
1. ✅ **Bot → Postgres — construit et testé (2026-09-10)** : `backend/pg_repository.py::PostgresRepository`, miroir synchrone (`psycopg`, voir `backend/pg_db.py`) de `FirestoreRepository`, les ~20 méthodes réellement utilisées par `bot.py`/`services.py` (tracées une à une). 19 tests d'intégration contre un vrai Postgres local, voir `JOURNAL.md` 2026-09-10. **`bot.py` n'est PAS encore branché dessus** — construit en isolation, le branchement réel est la Phase B (fenêtre de bascule), pas cette étape.
   - ✅ **Revue de code (2026-09-11)** : 6 findings trouvés et corrigés (bug cross-tenant réel sur `create_new_deal`, piège logger reproduit, 2 opérateurs JSONB STRICT sans garde dans `chat_repo.py`, boucle infinie potentielle dans `purge_rejected_images`, perte de données silencieuse dans l'export, duplication de mapping ville). 81 tests au total. Voir `JOURNAL.md` 2026-09-11.
2. ✅ **Frontend → nouvelle API — construit ET câblé (2026-09-11/12)** : `src/services/apiService.js`, mêmes 44 fonctions/signatures que `firestoreService.js` (sauf `migrateOldDataToNewUser`, restée sur l'ancien fichier). 3 trous backend trouvés et comblés (config utilisateur/botStatus, corrections manuelles d'analyse IA, galerie photo du chat). Les 8 hooks/composants consommateurs importent désormais réellement `apiService.js` ; `useDealsManager.js` adapté à la forme complète des annonces (plus de sharding `deals_index`) ; `published_at_ts` trouvé manquant et ajouté au passage. 95 tests au total, `npm run build` passe. **`bot.py` toujours pas branché sur Postgres** (Phase B) — une base locale n'a donc aucune annonce sans données de test insérées à la main ; aucun test navigateur/Firebase Auth réel possible depuis cet environnement de dev. Voir `JOURNAL.md` 2026-09-11/12.
3. 🚧 **Déploiement infra, en cours (2026-09-12)** : `deploy.yml` synchronise `backend/api/*` depuis la branche de migration à chaque déploiement et tente de créer un service systemd `guitarhunter-api` (127.0.0.1:8000, pointé sur `guitarhunter_pg_staging`) à côté de `guitare-hunter`. **Bloqué par les droits sudo du serveur** (règle existante limitée à `systemctl restart guitare-hunter`) — action manuelle requise, voir `TODO.md`. Reverse proxy (nginx/Caddy) et Tailscale Funnel (accessibilité tranchée le 2026-09-09) : pas encore commencés, décision et étape séparées une fois le service confirmé opérationnel. Voir `JOURNAL.md` 2026-09-12.
