...
[2026-08-28] [PRO] Consultation externe (Opus + Fable) sur l'outillage Dataset B, corrections appliquées (projet neck-reset) → Résultat :
- **Demande utilisateur** : consulter Opus et/ou Fable sur les fiches/protocole Dataset B, et sur la pertinence de systématiser les photos "type annonce". Les deux ont été lancées par erreur au lieu d'une seule (utilisateur avait dit "OU") — reconnu, les deux ont été rapportées vu le travail déjà engagé.
- **Verdict convergent des deux modèles sur les photos "type annonce"** : ne pas systématiser à chaque configuration — à 20-60 photos sur 2 guitares, volume insuffisant pour "entraîner"/"calibrer" la localisation (confusion de ma part entre vérité terrain physique et annotations visuelles, relevée par les deux). Recommandation commune : 3-5 clichés **par guitare/séance** (pas par config), et surtout un **sprint d'annotation manuelle sur 100-200 photos du Dataset A** comme vraie prochaine étape pour la localisation — décision appliquée, point ajouté à `NECK_RESET_VISION_PLAN.md` §6.
- **Désaccord entre les deux modèles sur le sens de la cale** (Fable : côté talon baisse l'action ; Opus : une cale plane ne change que la hauteur, pas l'angle, sans trancher un sens). **Tranché par l'utilisateur avec la mécanique réelle** : cale côté tête → manche penche vers l'avant → action augmente → simule le besoin de neck reset. Cale simple confirmée suffisante (pas de biseau nécessaire). Fiche 1 corrigée avec une terminologie sans ambiguïté (tête/corps).
- **Autres corrections convergentes intégrées** : contrôle du relief (mesuré 1x, jamais retouché), retour à la config initiale en fin de séance (repétabilité), vue rasante pour les gros plans sillet/12e frette (pas perpendiculaire — la perspective écrase l'écart corde/frette sinon), lumière rasante en plus pour la vue décollement, photo avec réglet millimétré par configuration (teste enfin l'hypothèse "corde = étalon", jamais vérifiée), colonne "angle (degrés)" remplacée par une mesure concrète (règle sur les frettes, hauteur au-dessus du chevalet, en mm) — appliquée à la Fiche 2 et au schéma `mesures.csv`/`build_db.py` (`angle_manche_deg` → `projection_chevalet_mm`, ajout `relief_mm`).
- Détail complet dans `NECK_RESET_VISION_PLAN.md` §5, §6.
...
- **Chiffres recalculés sur l'échantillon déjà collecté** (565 photos, aucun nouveau run nécessaire) : measurable passe de 49%→**93,0%** sur la 1re photo, 16,5%→**50,4%** sur l'ensemble du carrousel — bien plus cohérent avec `usable` (97,0%/52,0%), confirme que le chiffre précédent était un artefact du proxy, pas une vraie mesure d'exploitabilité.
- Détail complet (les 3 cas inspectés) dans `NECK_RESET_VISION_PLAN.md` §8quater.

[2026-08-24] [PRO] Audit + correctifs + run de validation sur échantillon — projet neck-reset → Résultat :
- **Demande utilisateur : auditer le code de la tentative "Phase 0 à l'échelle" (commit `2caf531`, écrit hors session via un autre outil/modèle)**, remise en cause explicite. Revue de code dédiée (4 trouvailles, toutes confirmées manuellement) : docstring contredisant le code (dit "1 photo/annonce", le code boucle sur toutes), résumé final étiqueté "annonce(s)" alors que ce sont des comptages par photo, en-tête de workflow décrivant l'ancien script (`test_owlv2_parts_detection.py`), variable de boucle morte. Aucune de ces 4 trouvailles n'explique le crash (timeout), mais toutes auraient trompé un lecteur.
- **Question utilisateur clé, qui a changé le plan : "avons-nous besoin de tout le dataset pour tester la fiabilité des algos de détection ?" — non.** La fiabilité d'OWLv2 est déjà caractérisée statistiquement (8/8, puis 1034/1066) ; un run à pleine échelle sert à produire un corpus de production, pas à valider un algorithme. `--limit` passé d'un slice en tête de manifeste à un tirage aléatoire diversifié (`--seed`).
- **3 corrections techniques appliquées** (root cause du run raté du 2026-08-20, diagnostiqué séance précédente) : sortie non bufferisée (`python3 -u` + `flush()`), écriture incrémentale du JSONL (rien perdu sur interruption), `command_timeout` remonté à 100 min.
- **Run de validation (100 annonces, seed=42, 565 photos) : succès technique complet** — progression affichée en direct cette fois (confirmé, timestamps distincts), artefact récupéré sans problème.
- **Mais le résultat a révélé un vrai problème de calibration, pas juste un chiffre à accepter** : `cropped_suspect` (proxy de cadrage incomplet) se déclenche sur 48% des photos de 1re position pourtant bien détectées (97/100 usable, cohérent avec le run précédent) — cause probable : une guitare bien cadrée occupe déjà 70-90% de la hauteur du cadre (§3quater), donc touche naturellement les bords sans être coupée. Le chiffre `measurable` qui en dépend (16,5%/49%) est sous-estimé, documenté comme tel plutôt que présenté comme une vraie mesure — recalibration du proxy ajoutée aux points ouverts (§6).
- Détail complet dans `NECK_RESET_VISION_PLAN.md` §8ter.
...
- **`NECK_RESET_VISION_PLAN.md` §8bis et le point ouvert §6 corrigés** en conséquence — pas de nouvelle donnée, juste une lecture plus rigoureuse de celle déjà produite.
- **Leçon retenue** : vérifier une image annotée "à l'œil" une seule fois n'est pas suffisant quand plusieurs boîtes se chevauchent visuellement (couleurs proches, boîtes imbriquées) — une relecture demandée par un tiers a suffi à trouver l'erreur, ce qui aurait dû être fait plus soigneusement du premier coup.

[2026-08-19] [PRO] Action effectuée : OWLv2 en détection multi-parties testé, ne résout pas la localisation du manche (projet neck-reset) → Résultat :
- **Hypothèse testée à la demande de l'utilisateur** : réutiliser OWLv2 (déjà validé §7) avec des requêtes par pièce ("neck"/"headstock"/"bridge"/"soundhole") pour réparer le prototype Phase 1 (recadrage manuel actuel) et, plus largement, localiser les différentes parties de la guitare.
- **Résultat initial (première lecture des images trop rapide, corrigée juste après — voir l'entrée "Correction" ci-dessus)** : "neck" incohérent (n'attrape que la zone tête sur une photo, le vrai manche sur l'autre) ; "headstock" ne fonctionne pas comme détecteur distinct (boîte quasi identique à "toute la guitare") ; "bridge" jamais détecté ; "soundhole" cru fiable sur les 2 photos, en réalité fiable sur une seule.
- **Conclusion honnête : cette piste ne résout pas le problème.** OWLv2 fonctionne bien en présence/absence globale (§7, 97% sur 1066 annonces) mais ne distingue pas finement les parties anatomiques avec de simples requêtes textuelles. Pistes non testées pour la suite : autres formulations ("fretboard"/"fingerboard"), ou un modèle de segmentation dédié (SAM 2.1, jamais essayé — Florence-2 avait déjà échoué pour une raison différente).
- **Au passage** : `run_phase0_usability_filter.py` étendu avec des critères de "mesurabilité" (pas juste présence) — résolution minimale, cadrage suspect (bbox touchant un bord) — pas encore ré-exécuté à l'échelle avec ces nouveaux critères.
- Détail complet (images, scores par pièce) dans `NECK_RESET_VISION_PLAN.md` §8bis.

[2026-08-19] [PRO] Action effectuée : résultat de la Phase 0 à l'échelle sur le Dataset A (projet neck-reset) → Résultat :
- **1034/1066 annonces jugées utilisables (97,0%)** par OWLv2 (seuil de score 0.3), 5 erreurs de téléchargement/inférence, VRAM pic identique au test à 8 photos (808 Mo/7785 Mo, 10,4%) — pas de dérive mémoire avec le volume. Traitement en ~15 minutes (839s effectifs pour 1066 images).
- **Nuance explicite retenue** : ce chiffre mesure la *présence détectée* d'une guitare, pas l'exploitabilité complète (mesure d'action) — plus optimiste que le taux mesuré à la main sur l'échantillon Étape 0 (51%/70%, §3sexies), qui couvrait des critères que ce filtre ne couvre pas encore (cadrage, résolution, photos IA-générées). Les deux chiffres ne se contredisent pas, ils répondent à des questions différentes — documenté pour éviter toute confusion future entre les deux.
- **Bug trouvé et corrigé** : l'étape de récupération de l'artefact (`dataset_a_phase0.jsonl`) a échoué (`error in libcrypto` sur la clé SSH) — absence probable de saut de ligne final après interpolation du secret dans le script bash. Corrigé (`printf '%s\n'`), mais les chiffres ci-dessus viennent des logs stdout du job (déjà complets), pas du fichier récupéré — celui-ci reste sur le Dell pour l'instant, à rapatrier lors d'un prochain run.
- Détail complet dans `NECK_RESET_VISION_PLAN.md` §7, étape 5.

[2026-08-19] [PRO] Action effectuée : Phase 0 lancée à l'échelle + premier prototype Phase 1 (projet neck-reset) → Résultat :
- **Phase 0 à l'échelle** : `backend/scripts/run_phase0_usability_filter.py` (nouveau) applique la détection OWLv2 (validée §7) à la première photo des 1066 annonces du Dataset A. Manifeste `dataset_a_manifest.jsonl` désormais **versionné dans le dépôt** (`backend/scripts/data/`) plutôt que simple artefact CI éphémère, pour que le Dell y accède par `git fetch` sans dépendance Firestore. Lancé sur `ops/run-script` — résultat à consigner dans une prochaine entrée une fois le run terminé (~15-25 min attendues, 1066 images).
- **Prototype Phase 1 (détection des frettes, LSD+RANSAC), testé à la main sur 2 photos réelles** : premier essai sur la photo entière échoue nettement — la recherche de séquence géométrique matche du bruit de fond (mur, plancher, couverture) sur toute la hauteur de la photo, pas les vraies frettes. Avec un recadrage manuel + contraintes ajoutées (longueur/centrage/envergure bornés), la séquence se recale correctement sur la zone du manche pour une photo bien frontale, mais **échoue totalement** sur une 2e photo avec un angle légèrement de travers. **Conclusion honnête : prometteur mais pas robuste**, le recadrage manuel doit être remplacé par une vraie détection du manche avant d'aller plus loin — documenté sans sur-vendre le résultat, y compris l'échec, dans `NECK_RESET_VISION_PLAN.md` §8.
- **Clarification utilisateur retenue** : LSD+RANSAC est de la vision classique (CPU), pas un réseau de neurones — pas de risque de "devoir réécrire pour GPU" en ayant prototypé en local plutôt que sur le Dell.
