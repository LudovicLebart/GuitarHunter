...
[2026-08-24] [PRO] Mutualisation des mesures anti-détection Facebook/Kijiji/LeBonCoin (branche dédiée) → Résultat :
- **Demande utilisateur** : reprendre la dette technique "base commune scraper" (`TODO.md`, remontée en priorité haute le même jour), sur une branche séparée de `dev` — refactor plus risqué sur du code de scraping qui tourne en production. Explicitement écarté : pas d'investissement supplémentaire sur les proxies (extension à Kijiji non faite).
- **Nouveau module `backend/scraping/stealth.py`** : pools UA/viewport fusionnés (union des 3 listes auparavant dupliquées avec variations), flags de lancement Playwright communs, `build_proxy_config()` (sélection proxy, logique inchangée), `human_pause()` (délai aléatoire, jamais fixe), `simulate_scroll_and_mouse()` (scroll + mouvement de souris simulés).
- **Choix de conception explicite : fonctions/constantes partagées plutôt qu'une hiérarchie de classes** — les trois scrapers ont des besoins de session réellement différents (géolocalisation forcée + proxy pour Facebook, `storage_state` persistant + `headless=False` volontaire pour LeBonCoin, rien de spécial pour Kijiji) ; une classe de base avec héritage/composition sur `self.playwright/browser/context` aurait dû absorber ces différences via des points d'extension pour un gain marginal, sur du code qui tourne. Documenté dans le docstring du module pour qu'un futur lecteur ne le prenne pas pour un oubli.
- **`FacebookScraper`** (`backend/scraping/core.py`) : listes UA/viewport locales retirées au profit des pools partagés. **Techniques plus avancées portées depuis LeBonCoin** (qui les avait déjà, validées face à DataDome le 2026-07-21, mais jamais remontées vers Facebook) : la boucle de défilement dynamique scrollait d'une distance et à un délai **strictement identiques** à chaque itération (`page.mouse.wheel(0, 1000)` + `time.sleep(2)`, un signal robotique en soi) — remplacée par une distance et un délai aléatoires (`human_pause`). Ajout d'un geste "coup d'œil" (`simulate_scroll_and_mouse`) juste après l'application des filtres, avant le défilement systématique.
- **`KijijiScraper`** (`backend/scraping/kijiji/core.py`) : même traitement — pools UA/viewport partagés (gagne au passage `--disable-infobars`, absent de ses flags de lancement jusqu'ici), défilement DOM de repli (`_scrape_from_dom`, chemin secondaire — `__NEXT_DATA__` est la voie principale) avec distance/délai aléatoires au lieu de fixes.
- **`LeboncoinScraper`** (`backend/scraping_leboncoin/core.py`) : `_human_pause`/`_simulate_browsing` (méthodes locales, quasi identiques à ce qui vient d'être extrait — c'est en fait la source de portage vers Facebook/Kijiji) remplacées par les fonctions partagées. **UA/viewport volontairement NON alignés sur le pool partagé** : ce jeu précis (4 UA/3 viewports) est celui validé en conditions réelles face à DataDome, une extension silencieuse via un sous-ensemble positionnel du pool fusionné aurait introduit un UA différent (Edge au lieu du Chrome macOS d'origine) sans nouvelle validation — corrigé en gardant les valeurs explicites plutôt qu'un slice fragile.
- **Tests** : suite complète des 3 scrapers (`backend/scraping/test_core.py`, `backend/scraping/kijiji/test_kijiji_core.py`, `backend/scraping/kijiji/test_locations.py`, lancés depuis `backend/` — convention d'import de ces fichiers) — **76/76 passent**. `backend/test_bot.py` — 28/30, mêmes 2 échecs préexistants et sans rapport que la veille (normalisation de ville Kijiji). Fonctions de `stealth.py` testées isolément (imports, `human_pause`, `build_proxy_config` vide/non-vide).
- **Non testé en conditions réelles** (pas d'accès Facebook/Kijiji/LeBonCoin depuis cet environnement) — à valider par l'utilisateur avant fusion dans `dev`.
- **Commits** : à venir (branche `claude/scraper-stealth-mutualization`, pas fusionnée dans `dev` pour l'instant).
