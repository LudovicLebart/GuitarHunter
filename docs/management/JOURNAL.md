...
[2026-07-24] [PRO] Feature : intégration réelle LeBonCoin dans bot.py (scan + Firestore) → Résultat :
- **Contexte** : compte LeBonCoin personnel unique (pas multi-tenant) — `leboncoinConfig.enabled` à `False` par défaut pour tout le monde, à activer manuellement, plutôt qu'une vérification `user_id` codée en dur.
- **`config.py`** : `LEBONCOIN_STORAGE_STATE_PATH` — chemin fixe sur le serveur (comme `serviceAccountKey.json`), pas par utilisateur.
- **`bot.py::run_leboncoin_scan()`** : réutilise `handle_deal_found()` tel quel pour tout le pipeline (dédup, pré-filtres, analyse IA, Firestore, notifications) via un nouvel adaptateur `_map_leboncoin_ad()` qui convertit le schéma d'annonce LeBonCoin vers le même schéma que Facebook — pas de pipeline dupliqué. IDs préfixés `lbc_` pour éviter toute collision avec les ID Facebook dans `guitar_deals`. Le `LeboncoinScraper` est créé à la demande puis gardé vivant pour toute la durée du thread bot (pas de fermeture/réouverture entre deux scans, cohérent avec le travail anti-détection du chantier). Vérifie `is_night_time()` avant chaque cycle.
- **`handle_deal_found()`** accepte désormais `skip_price_prefilter` : la recherche LeBonCoin filtre déjà par prix côté site (`leboncoinConfig.min_price`/`max_price`, dans l'URL de recherche elle-même) — réappliquer `scanConfig.max_price` (potentiellement une valeur différente pour Facebook) aurait rejeté à tort des annonces LeBonCoin légitimes.
- **`repository.py::get_known_deal_ids(prefix)`** (nouveau) : lit les 20 chunks de `deals_index` (comme le frontend, pas un scan complet de `guitar_deals`) pour alimenter l'arrêt anticipé de pagination déjà implémenté côté scraper.
- **`services.py::TaskScheduler`** : le scan LeBonCoin a sa propre cadence, indépendante de Facebook, jitterisée **+/-30% autour de la fréquence de scan Facebook courante** (demandé explicitement) — jamais un intervalle fixe ; chaque exécution se replanifie elle-même avec un nouveau tirage aléatoire.
- **`main.py`** : câblage de `TaskScheduler` avec les nouvelles fonctions.
- **Vérifié** : import réel de `bot.py`/`services.py`/`repository.py`/`main.py` (stubs complets `firebase_admin`/`schedule`/`playwright`/`PIL`/`google.generativeai`) + tests en isolation de `_map_leboncoin_ad()`, `get_known_deal_ids()` et du mécanisme de replanification jitterisée (pas d'accumulation de jobs).
- **Non testé en conditions réelles** (pas de session LeBonCoin/Firestore disponible depuis cet environnement) — validation à la charge de l'utilisateur. Pour activer : `leboncoinConfig.enabled: true` dans Firestore + déposer `leboncoin_storage_state.json` sur le serveur.

[2026-07-24] [PRO] Fix : findings du /code-review complet (8-angles) sur le chantier LeBonCoin → Résultat :
- **`backend/scraping_leboncoin/core.py::search()`** : le retry sur `page.goto()` n'était pas lui-même protégé — une 2e erreur transitoire faisait remonter une exception hors de `search()`, violant son propre contrat ("jamais d'exception, toujours (annonces, blocage)") et plantant tout un soak test. Boucle bornée à 2 tentatives ; abandon propre (retourné, pas levé) si la 2e échoue aussi.
- **`_looks_blocked()`** : ne vérifiait pas le domaine des réponses 403/429 — une ressource tierce (pub/tracker) répondant 403 déclenchait un faux blocage. Restreint aux réponses de leboncoin.fr. Le nettoyage de `self._responses` ne se refaisait pas non plus entre la tentative échouée et la retentative — déplacé à l'intérieur de la boucle de tentatives.
- **Page crashée** : fermée (best-effort) avant d'être abandonnée, pour éviter d'accumuler des onglets zombies sur un soak test de plusieurs heures.
- **`base_scraper.py::_get_page()`** : le listener de réponses retenait l'objet `Response` complet de CHAQUE requête (images/JS/CSS...) pendant toute la durée de vie de l'onglet ; ne garde plus que `(status, url)` et seulement pour les 403/429.
- **`bot.py`** : BAD_DEAL (hors budget) et REJECTED (mot-clé) partageaient le même code de sortie `rejected_prefilter`, contredisant la distinction documentée dans CLAUDE.md — séparés en `rejected_prefilter`/`over_budget_prefilter`, avec messages de notification distincts. `processed` ne distinguait pas une vraie nouvelle annonce d'une réanalyse suite à baisse de prix — nouveau code `processed_update`. `total_cards_seen` (déjà renvoyé par `scan_marketplace`) n'était jamais consommé ; ajouté au résumé de cycle, de même que le compteur `unknown` (outcome non reconnu), invisible même si non nul.
- **`notifications.py`** : suppression de la clé morte `marked_sold` (jamais retournée par `handle_deal_found`) ; extraction de `_build_deal_link()` (déduplique la construction d'URL entre `notify_deal`/`notify_scan_url_finished`).
- **`StatsView.jsx::dailyVolumeData`** : le `useMemo` ne dépendait que d'`enrichedDeals` — si aucune nouvelle annonce n'arrive pendant >1 jour (le cas même visé par ce graphique), la fenêtre de 14 jours restait figée. Ajout d'une dépendance qui change une fois par jour, filtre numérique bon marché avant le scan complet de l'historique, clés de jour calculées une seule fois.
- **Dédoublonnage mineur** : `_move_mouse_to_element()` (scraping_common, factorise 4 occurrences), `_do_search()` (leboncoin_probe.py).
- **Vérifié mais non corrigé** : une revue soupçonnait un `mark_deal_as_sold()` perdu dans `handle_deal_found()` — vérifié via `git log`/`git show` : ce commit (5347222) n'est pas un ancêtre de cette branche (divergence de branches, pas une régression introduite ici) — à réconcilier au merge avec `dev`. `rebuild_index.py` (script séparé) et `FacebookScraper` (code en production) non touchés, hors scope.

[2026-07-24] [PRO] Refactor : base commune backend/scraping_common/ (session + comportement humain) → Résultat :
- **Contexte** : suite à la dette technique notée le 2026-07-22 (duplication `LeboncoinScraper`/`FacebookScraper`), extraction d'une base commune — en insistant sur la prudence pour ne pas toucher au code en production, et en pensant la conception pour de futurs modules (Kijiji, Reverb...).
- **`backend/scraping_common/human_behavior.py`** (nouveau) : `is_night_time()`, `seconds_until_active()`, `HumanBehaviorMixin` (`_human_pause`, `_human_mouse_move`, `_human_scroll`) — logique 100% générique, aucune dépendance à un site précis.
- **`backend/scraping_common/base_scraper.py`** (nouveau) : `BaseMarketplaceScraper` — cycle de vie de session (`start_session`/`close_session`/`_ensure_session`/`_get_page`), UA/viewports par défaut, `storage_state_path` optionnel (session authentifiée ou anonyme selon le futur module).
- **`LeboncoinScraper` hérite désormais de `BaseMarketplaceScraper`** — `core.py` passe de 491 à 234 lignes, ne garde que ce qui lui est propre (build_url, extract_ads, _looks_blocked, actions décoratives, search()). Signatures et comportement publics inchangés.
- **`FacebookScraper` : aucune modification** (vérifié via `git diff` — zéro changement). Sa migration reste un chantier séparé et délibéré, noté en TODO.
- **Vérifié** : import réel de tous les modules touchés (stub Playwright, pas seulement `py_compile`) + retest en isolation de la logique scroll/pause/nuit après déplacement — comportement identique confirmé.
