...
- **Résultat vérifié visuellement sur les images annotées** (pas seulement les scores) : "soundhole" se localise étonnamment bien dans les 2 photos test (boîte petite et précise, sur la vraie rosace) ; "neck" est incohérent (n'attrape que la zone tête sur une photo, tout le manche+tête de façon imprécise sur l'autre) ; "headstock" ne fonctionne pas comme détecteur distinct (boîte quasi identique à "toute la guitare") ; "bridge" jamais détecté.
