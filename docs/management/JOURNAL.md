...
[2026-09-09] [PRO] Chantier A, tranche 3 (deal_chat + WebSocket temps réel) — codée et validée, deux bugs latents (pas nouveaux à cette tranche) trouvés grâce aux tests.
- **`backend/api/chat_repo.py`** : accès SQL à `deal_chat` (CRUD messages, marquage galerie, statut proposition de restauration/requalification). Pas de `user_id` propre sur cette table (seulement une FK vers `guitar_deals`) — `get_deal_owner()` sert à vérifier la propriété du deal parent avant d'exposer/modifier son chat, réutilisé identiquement par le REST et le WebSocket.
- **`main.py`** : endpoints REST `/deals/{id}/chat...` (mêmes contrats que `firestoreService.js::addDealChatMessage`/`replaceDealChatMessage`/`markChatMessage*`) + WebSocket `/ws/deals/{id}/chat`, remplace `onDealChatUpdate`. Même pattern anti-course que la tranche 2 (accusé `{"type": "ready"}` après `add_listener()`), avec une différence : la vérification de propriété se fait ici **avant** `accept()` (pas après), puisqu'elle ne dépend que du `deal_id` de l'URL et non d'un filtrage a posteriori par canal partagé.
- **`schema.sql`** : ajout de `deal_chat` et de son trigger (`notify_chat_change`, canal `chat_changes`, filtré côté serveur par `deal_id` — pas par `user_id` comme `deal_changes`, cohérent avec l'absence de colonne `user_id` sur la table).
- **Piège de migration réel trouvé (pas un artefact de cette tranche, mais découvert en l'écrivant)** : `CREATE TABLE IF NOT EXISTS deal_chat` ne modifie jamais une table déjà créée par une tranche précédente (les tests avaient déjà fait exister la table avec un schéma partiel) — les nouvelles colonnes (`attached_image_part_indices`, `restoration_proposals`, etc.) n'étaient silencieusement jamais appliquées. Corrigé en restructurant `schema.sql` avec des `ALTER TABLE ... ADD COLUMN IF NOT EXISTS` séparés pour toute colonne ajoutée après la création initiale d'une table — règle documentée en commentaire dans le fichier pour les tranches suivantes.
- **Bug latent plus ancien découvert par cette tranche (présent depuis la tranche 1, jamais détecté avant)** : asyncpg ne décode jamais nativement les colonnes JSON/JSONB — elles revenaient comme des chaînes brutes, pas des `dict`/`list` Python. Resté invisible sur les tranches 1/2 faute de test qui vérifie la structure interne d'un champ JSONB imbriqué ; révélé par `test_mark_restoration_proposal_status` (`TypeError: string indices must be integers, not 'str'`). Corrigé une fois pour toutes via `db.py::_register_json_codecs()` (`set_type_codec` sur `jsonb`/`json`, appliqué à chaque connexion du pool via `init=`) — et retrait des `json.dumps()` manuels devenus redondants (et nuisibles, double-encodage) dans `commands_repo.py` et `chat_repo.py`.
- **Validé en conditions réelles** (Postgres local + vrai serveur) : 17 tests d'intégration au total (6 nouveaux pour cette tranche : 5 REST + 1 WebSocket). Toujours rien de branché à Firestore/au bot/au frontend existants.
- **Reste à faire** : tranches `restoration_plan_items`, `cities`/`user_city_prefs`, `shared_deals`, avant toute décision de bascule réelle.

[2026-09-09] [PRO] Chantier A, tranche 2 (guitar_deals + WebSocket temps réel) — codée, un vrai bug de course trouvé et corrigé grâce aux tests en conditions réelles.
- **`backend/api/deals_repo.py`** : accès SQL à `guitar_deals` (list/get/by-ids/favori/achat/classification manuelle/reject/delete). Simplification notable vs `firestoreService.js` actuel : les mutations favori/achat/classification n'écrivent plus qu'une seule fois (colonnes indexées nativement par `schema.sql`), contre deux écritures aujourd'hui (le document + l'entrée `deals_index` correspondante, bricolage de sharding qui disparaît entièrement).
- **`schema.sql`** : trigger Postgres (`notify_deal_change`) sur `guitar_deals`, remplace `onDealsIndexUpdate`. Notifie sur TOUTE écriture quelle que soit son origine (bot en SQL direct comme cette API) — un seul point à maintenir plutôt qu'un `NOTIFY` manuel dupliqué dans chaque site d'écriture applicatif.
- **`main.py`** : endpoints REST (`GET`/`PATCH`/`DELETE /deals/...`) + WebSocket `/ws/deals` (canal `LISTEN` partagé entre utilisateurs, filtré côté serveur par `user_id` — le token Firebase passe en paramètre de requête, un navigateur ne pouvant pas poser d'en-tête personnalisé sur une connexion WS native).
- **Bug réel trouvé par les tests, pas un artefact** : condition de course entre l'ouverture de la connexion WebSocket (`accept()`) et l'enregistrement effectif du `LISTEN` Postgres juste après — une notification émise dans cette fenêtre (même très courte) se serait perdue silencieusement, Postgres ne rejouant jamais les `NOTIFY` manqués. Découvert car le premier test échouait de façon flaky selon un `sleep` present ou non. **Corrigé par un accusé de réception explicite** (`{"type": "ready"}`) envoyé une fois le `LISTEN` effectivement enregistré, que le client doit attendre avant de compter sur le canal — pertinent aussi côté frontend au moment de l'intégration réelle.
- **Difficulté d'outillage notée** : le `TestClient` Starlette (utilisé pour les routes REST) s'est avéré **ne pas délivrer fiablement** un message WebSocket envoyé depuis une tâche créée hors du flot requête/réponse direct (ici : le callback `add_listener` d'asyncpg, qui programme l'envoi via `loop.create_task` sur notification Postgres) — bloquait indéfiniment sans erreur. Un vrai serveur `uvicorn` (thread dédié) + un vrai client `websockets` ont été nécessaires pour tester le canal correctement ; classe de test séparée (`TestDealsWebSocket`) pour éviter un conflit de pool `asyncpg` entre la boucle asyncio du `TestClient` et celle d'`uvicorn`.
- **Validé en conditions réelles** (Postgres local + vrai serveur, pas des mocks) : 11 tests d'intégration au total (7 nouveaux pour cette tranche). Toujours rien de branché à Firestore/au bot/au frontend existants.
...
- **Résultat (20 776 documents scannés)** : `gemini-3.5-flash-lite` (T1) — 78 appels, 0 caché, écart médian 8,9s (le plus serré des 3 Tiers). `gemini-3.7-flash` (T2) — 40 appels, 7 cachés (17,5%), écart médian 13,2s. `gemini-3.1-pro-preview` (T3) — 11 appels, 0 caché, écart médian 53,8s. **Zéro avertissement JSON invalide** sur toute la fenêtre, tous modèles confondus.
- **Hypothèse "retry JSON" réfutée** : sans avertissement JSON invalide, les 7 hits du Tier 2 ne peuvent pas être des retries — ce sont des hits authentiques entre appels distincts.
- **Hypothèse "écart de temps/TTL trop court" également réfutée** : le Tier 1 a l'espacement le *plus serré* des 3 (8,9s médian) et pourtant 0% de cache, contre 17,5% pour le Tier 2 (espacement plus large) — si l'écart entre appels était la cause limitante, le Tier 1 devrait cacher au moins autant que le Tier 2, pas moins.
- **Recherche sur `ai.google.dev/gemini-api/docs/caching`** : le cache *implicite* (celui utilisé ici) a un seuil minimum de préfixe commun qui dépend du modèle — **4 096 tokens pour toute la famille Gemini 3/3.1** (Flash 3.5-3.8 et Pro 3.1 Preview inclus), contre 2 048 tokens sur l'ancienne génération 2.5. Notre estimation interne du bloc statique partagé (`GEMINI_PROMPT_CACHING_PLAN.md §7.1`, ~3 205 tokens) est **ancienne et sous ce seuil actuel** — probablement dépassée puisque la taxonomie a été enrichie plusieurs fois depuis, mais jamais remesurée. Le support du cache implicite sur Flash-Lite spécifiquement reste flou dans les sources trouvées — aucun seuil précis identifié pour ce palier.
- **Rappel important (déjà dans le plan mais à insister)** : le cache *implicite* est un mécanisme **opportuniste, best-effort, côté serveur Google** — aucun objet de cache persistant n'existe, rien n'est "permanent" ni garanti même au-dessus du seuil minimum. Seul le cache *explicite* (`client.caches.create()`, déjà écarté pour ROI insuffisant — `GEMINI_PROMPT_CACHING_PLAN.md §7.6`, ratio 0.13) crée un objet réellement persistant garantissant la réduction tarifaire.
- **Conclusion révisée** : le fix du 2026-09-07 fonctionne probablement comme prévu pour Tier 2 (cache authentique, pas un artefact) — le taux de 17,5% plutôt que 100% s'explique par la nature best-effort du cache implicite Google, pas par un bug côté projet. Tier 3 (11 appels) est cohérent avec une simple variance d'échantillon au même taux que Tier 2 (≈2 hits attendus, observer 0 n'est pas surprenant) — **aucune preuve que Tier 3 soit structurellement cassé**, contrairement à la conclusion précédente. **Tier 1 (Flash-Lite) reste la vraie anomalie** : statistiquement quasi impossible que 0/78 soit du hasard au taux de Tier 2 — cause probable non confirmée (comportement Flash-Lite différent/plus restrictif), à creuser si le sujet redevient prioritaire.
- **`run_once.py`** repassé à `ACTIVE = False` (commit `d3bb896`) après lecture du résultat.

[2026-09-08] [PRO] Archive de logs locale sur le serveur, en parallèle de Firestore → codé et déployé (run GitHub Actions #429, succès), non encore vérifié en conditions réelles.
- **Contexte** : le TTL Firestore de 3 jours sur `artifacts/{APP_ID}/users/{USER_ID}/logs` vient de faire perdre toute donnée "avant" lors de la mesure du fix caching (entrée suivante) — les logs `[tokens]` émis avant le déploiement du correctif (2026-09-07 03:49 UTC) avaient déjà expiré au moment de la mesure (2026-09-08 21h). Proposition initiale (réarchiver depuis Firestore vers Firebase Storage) écartée par l'utilisateur au profit d'une écriture directe sur disque — le process qui génère les logs tourne déjà sur le serveur.
- **`backend/logging_config.py::setup_logging()`** : ajout d'un `TimedRotatingFileHandler` (rotation quotidienne UTC, un fichier par utilisateur sous `logs/bot_{user_id[:8]}.log`) sur le logger `bot.{user_id[:8]}`, **en plus** du `FirestoreHandler` existant (inchangé, LogViewer non affecté). Actif même en mode offline (justement le cas où Firestore n'est pas joignable).
- **`backend/log_retention.py`** (nouveau) : job de maintenance quotidien — compresse en `.gz` les fichiers de plus de 30 jours, supprime ceux de plus d'un an. Rétention gérée entièrement par ce module (`TimedRotatingFileHandler(backupCount=0)`, pas de suppression automatique côté handler — évite la dépendance à son comportement interne peu fiable pour distinguer fichiers compressés/non compressés).
- **`main.py`** : job branché en singleton sur la boucle watchdog globale (`schedule.every().day.at("03:15")`, même pattern que `admin_stats`), indépendant de Firestore/`db_service`.
- **`.gitignore`** : `logs/` ajouté — non suivi par git, donc jamais touché par le `git reset --hard` du déploiement.
- **Estimation de volume** (à partir des ~21 516 logs Firestore trouvés sur la fenêtre TTL de 3 jours, tous utilisateurs) : ~1 Mo/jour en clair, ~70-80 Mo en régime de croisière avec la politique 30j clair / 1 an compressé. Négligeable.
- **Testé en local** (stub `firebase_admin`, pas d'accès réel depuis cet environnement de dev) : écriture du fichier confirmée ; compression/suppression/idempotence de `log_retention.py` vérifiées sur des fichiers datés synthétiques (5j intact, 40j compressé, 400j supprimé, déjà-`.gz` ignoré).
- **Non vérifié en conditions réelles** : impossible de confirmer depuis cet environnement que le fichier apparaît bien sur le serveur une fois le bot relancé — le job SSH du déploiement ne capture que la sortie du script, pas les logs runtime du service après le `systemctl restart`. À confirmer par l'utilisateur (`ls ~/GuitareHunter/logs/`) ou lors d'une prochaine vérification via `run_once.py`.

[2026-09-08] [PRO] Mesure de l'effet réel du fix caching implicite Gemini (`run_once.py`) → résultat mitigé, comparaison "avant" perdue (TTL Firestore).
- **Contexte** : validation du correctif `ce2e7fb`/`4e77b38` (2026-09-07, voir entrée dédiée) — jamais testé en conditions réelles au moment du déploiement. `run_once.py` armé pour parcourir `artifacts/{APP_ID}/users/*/logs` (collection_group, lecture seule) à la recherche des lignes `[tokens] ... cached=...`, résumé avant/après le déploiement (cutoff 2026-09-07T03:49:02Z, confirmé via l'historique GitHub Actions).
- **Résultat (run GitHub Actions #427, 2026-09-08)** : 21 516 documents scannés, 133 lignes `[tokens]` trouvées — **toutes postérieures au déploiement**. Aucune donnée "avant" : le TTL de 3 jours a déjà effacé cette fenêtre (l'instrumentation `[tokens]` elle-même n'existait que depuis le 2026-09-06 ~02h, la fenêtre "avant" possible était donc déjà étroite).
- **`gemini-3.7-flash` (Tier 2, Analyste)** : 41 appels, **7 avec cache actif (17,1 %)**, 48 689 tokens cachés sur 422 011 tokens d'entrée (**11,5 %**) — le fix fonctionne pour ce Tier, confirmé par des appels réels répartis sur plusieurs heures (pas un artefact isolé).
- **`gemini-3.1-pro-preview` (Tier 3, Expert Pro)** : 11 appels, **0 % caché** — alors que c'est précisément le Tier ciblé par le réordonnancement du prompt (`analyzer.py:406`). Échantillon faible (11 appels) ou fix insuffisant pour ce Tier : à surveiller avec plus de volume, voir `TODO.md`.
- **`gemini-3.5-flash-lite` (Tier 1, Portier)** : 81 appels, **0 % caché**, malgré 820 663 tokens d'entrée — de loin le plus gros volume, toujours aucun cache après le fix. Confirme par des données réelles (pas juste la facturation) l'hypothèse déjà soulevée : voir `TODO.md`.
- **`run_once.py`** repassé à `ACTIVE = False` (commit `43ea5ec`) après lecture du résultat dans les logs GitHub Actions.

[2026-09-08] [PRO] Fix : coordonnées GPS fautives de "Saint-lambert" (jamais réellement corrigées) + `needsReview` généralisé au repli Nominatim aveugle → codé, déploiement à vérifier.
- **Contexte** : l'utilisateur signale qu'une recherche Kijiji manuelle (`address=Saint-lambert&ll=48.9382022,-0.547455`, ancrage région Gaspésie) retourne 0 résultat. Diagnostic : `ll`/`radius` sont le filtre géographique réellement appliqué par Kijiji par-dessus l'ancrage régional du chemin d'URL (`locations.py`, docstring `nearest_resolvable_hub`) — un rayon de 5km centré en France (Normandie) sur un site 100% canadien ne peut structurellement rien retourner, d'où l'incohérence apparente avec le libellé "Gaspé" affiché (qui vient de l'ancrage, pas du filtre GPS). Ces coordonnées fautives étaient déjà identifiées dans le TODO (`audit_city_coordinates.py`, 2026-08-25) mais **`run_once.py` n'avait exécuté que l'audit en lecture seule** — jamais la correction elle-même, malgré l'entrée TODO l'annonçant comme piste retenue.
- **Confirmation utilisateur** : le "Saint-Lambert" visé est celui de l'agglomération de Longueuil (Montérégie), pas l'homonyme d'Abitibi-Ouest présent dans `city_coordinates.json` — coordonnées confirmées via Nominatim (45.5016203, -73.5102981).
- **Décision explicite de l'utilisateur, écartant ma proposition initiale** : ne PAS restreindre `_geocode_nominatim()` au Canada (`countrycodes=ca`) — discussion déjà eue par le passé, l'app doit pouvoir chercher des villes hors Québec/Canada. Le vrai problème n'est pas l'ordre des requêtes par pays, c'est que le repli Nominatim en aveugle (Priorité 3, `add_city_auto()`) faisait confiance sans réserve à son 1er résultat, sans aucun signal pour détecter un homonyme improbable.
- **`bot.py::add_city_auto()`** : `needs_review` couvre désormais deux cas indépendants (`coords_from_blind_geocode` nouveau) — (1) suggestion Facebook ne correspondant pas à `region_hint` (mécanisme existant, 2026-08-26) ; (2) coordonnées obtenues via le repli Nominatim aveugle, quel que soit le pays retourné. Ne bloque jamais l'ajout (le city_id reste indispensable au scraper), signal de vérification manuelle uniquement — cohérent avec le refus de restreindre géographiquement la recherche.
- **`backend/scripts/run_once.py`** : armé (`ACTIVE = True`) pour la correction réelle en production — réécrit lat/lng de la ville "Saint-lambert" du catalogue partagé (`artifacts/{APP_ID}/cities`) de 48.9382/-0.5474 vers 45.5016203/-73.5102981 et retire `needsReview` ; ne touche que les entrées encore sur les coordonnées fautives (idempotent, sûr sur push répété `dev`+`master`). **À repasser à `False` après vérification du déploiement (logs GitHub Actions).**
- **Tests** : `TestAddCityAuto` (`test_bot.py`) mis à jour — le test du repli Nominatim aveugle attend désormais `needsReview: True` (comportement change volontairement). Suite complète `test_bot.py` : 2 échecs pré-existants sans rapport (bug de casse `Longueuil`/`longueuil`, confirmé antérieur à ce correctif via `git stash`).
- **Non fait** : le `city_id` Facebook de "Saint-Lambert" (`110358832318694`) n'a pas été revérifié — reste un homonyme potentiel non confirmé, voir `TODO.md`.

[2026-09-07] [PRO] Correctif du caching implicite Gemini cassé (taxonomie non triée + ordre du prompt Tier 3) → codé, non testé en conditions réelles.
- **Contexte** : suite à une consultation Claude Opus sur l'enquête coût Gemini de la période de facturation (le calcul bottom-up qui colle à 3,4% de la facture réelle suppose le plein tarif sur toute l'entrée — signe que le cache implicite ne mord probablement pas), deux causes concrètes identifiées dans le code réel (`docs/management/plans/COST_OPTIMIZATION_CHANTIERS.md`, Chantier 0.b).
- **`backend/analyzer.py:97`** : `json.dumps(taxonomy_data, ...)` n'utilisait pas `sort_keys=True` — l'ordre des clés d'un dict Python n'étant pas garanti stable d'un process à l'autre, le préfixe envoyé à Gemini n'était pas identique octet pour octet entre threads/redémarrages, cassant le match de préfixe du cache implicite. Demandé depuis `GEMINI_PROMPT_CACHING_PLAN.md` §2.3, jamais appliqué jusqu'ici. Corrigé.
- **`backend/analyzer.py:406`** : le prompt du Tier 3 (`full_prompt_t3`) plaçait le contexte T2 (dynamique, différent à chaque annonce) **avant** le bloc statique taxonomie/prompt de base, détruisant tout préfixe cacheable pour ce Tier — alors que T1/T2 font déjà l'inverse correctement (bloc statique en tête, addendum spécifique au Tier après). Réordonné pour aligner T3 sur le même pattern.
- **`_call_gemini_json`** : ajout de `cached_content_token_count` au log `[tokens]` déjà en place, pour mesurer objectivement l'effet une fois déployé plutôt que de le déduire de la facture globale.
- **Non testé en conditions réelles** (pas d'accès Gemini depuis cet environnement de dev) — à valider par l'utilisateur via les logs `[tokens]` après déploiement (un `cached=` non nul confirmerait que le cache mord désormais).

[2026-09-06] [PRO] Instrumentation `usage_metadata` sur les appels Gemini backend (Tier 1/2/3) → codé, non testé en conditions réelles.
- **Contexte** : suite à la correction du diagnostic "photos à 2048px" (entrée plus bas), plus aucune mesure fiable du coût réel des tokens-image côté backend — seule la facture GCP globale existe, inexploitable au niveau d'un appel précis. Même principe déjà en place côté chat frontend (`geminiChatService.js`, `TOKEN_OPTIMIZATION_PLAN.md`), jamais fait côté backend.
- **`backend/analyzer.py::_call_gemini_json`** : après chaque `model.generate_content()` réussi, log `[tokens] model=... images=... in=... out=... total=...` sur `self.logger` (logger per-utilisateur, visible LogViewer — piège du logger de module déjà documenté dans `CLAUDE.md`). `images` compté directement dans `current_parts` (objets `PIL.Image`), pas besoin de changer la signature des 4 sites d'appel (T1/T2/T3/`analyze_deal_light`) qui passent tous par cette méthode unique. Champs lus via `getattr(..., 0)` (défensif si un champ diffère entre versions du SDK) plutôt que d'accéder directement aux attributs, pour ne jamais faire planter une analyse à cause du logging.
- **Limite assumée** : `usage_metadata` donne un total par appel (texte + photos mélangés), pas une répartition par photo — le nombre de photos est logué à côté pour permettre une estimation croisée (comparer des appels à volumes de photos différents) plutôt qu'une lecture directe.
- **`py_compile`** : succès. **Non testé en conditions réelles** (pas d'accès Gemini depuis cet environnement de dev) — à valider par l'utilisateur : les logs `[tokens]` apparaissent bien dans le LogViewer après un scan, puis laisser tourner quelques jours avant d'en tirer une vraie conclusion sur le coût des photos T2.

[2026-09-06] [FLASH] Audit du volume réel par utilisateur → le pool d'annonces partagé n'est plus "le plus gros levier", gain plafonné à ~5,8% des appels Gemini.
- **Contexte** : avant de s'engager sur le chantier "pool d'annonces partagé entre utilisateurs" (`TODO.md`), l'utilisateur doute être le seul utilisateur réellement actif — à vérifier avant d'investir dans un refactor architecture significatif pour un gain peut-être marginal.
- **Mécanisme** : `backend/scripts/analyze_funnel_by_user.py` (lecture seule, déjà vérifié sans écriture Firestore) armé dans `backend/scripts/run_once.py` (`ACTIVE=True`), exécuté en production via le job `deploy` (run GitHub Actions #408, après reconnexion Tailscale du serveur — 2 tentatives précédentes bloquées par le serveur injoignable). Résultat lu dans les logs de l'étape, `ACTIVE` repassé à `False` dans la foulée (même commit que cette entrée).
- **Résultat mesuré** : 7 utilisateurs enregistrés en Firestore. Volume total 95,03 annonces/jour (30 derniers jours) — **89,43/jour (94,1%) pour un seul utilisateur** (`wbPlgZgkW2VcAl0a2l44UMSDTaG2`), 5,53/jour (5,8%) pour un second, 0,07/jour pour un troisième, et 3 comptes sans aucune annonce (dont 2 UID visiblement placeholder, `uid_firebase1`/`uid_firebase2`).
- **Conséquence sur le pool partagé** : son gain ne dépend pas du volume total mais du **recoupement** entre utilisateurs actifs — avec un seul second utilisateur pesant 5,8% du volume, le gain maximum théorique (même en supposant 100% de chevauchement géographique avec le premier, non vérifié) est plafonné à ~5,8% des appels Gemini. `TODO.md` corrigé aux deux endroits où ce chantier était qualifié de "plus gros levier" — reste une piste valide mais mineure tant que la base d'utilisateurs actifs ne grandit pas.

[2026-09-06] [FLASH] Correction : le diagnostic "photos envoyées à 2048px" de l'entrée précédente n'est pas confirmé → retiré, instrumentation réelle proposée à la place.
- **Signalé par l'utilisateur** : inquiétude sur un possible agrandissement artificiel des photos Kijiji (plus petites nativement que Facebook) par `analyzer.py::_download_and_optimize_image`.
- **Vérifié dans le code** : `max_size=2048` est un plafond, pas une taille cible — le `if img.size[0] > max_size or img.size[1] > max_size` ne déclenche `img.thumbnail()` que si l'image DÉPASSE 2048px, et `PIL.Image.thumbnail()` ne fait par construction que rétrécir, jamais agrandir. Aucun grossissement possible dans cette fonction, Kijiji ou Facebook.
- **Découverte en creusant plus loin** : un script déjà existant, `backend/scripts/compare_image_resolution_by_source.py` (créé pour `NECK_RESET_VISION_PLAN.md`, déjà exécuté — voir entrée du 2026-08-19 plus bas, 857 annonces Facebook / 37 Kijiji), a mesuré une résolution réelle moyenne de ~650×960px sur les deux plateformes (Kijiji légèrement pire, 640×798px) — très en dessous du plafond de 2048px, qui ne se déclenche donc quasiment jamais en pratique.
- **Conséquence** : l'affirmation de l'entrée précédente ("photos envoyées à 2048px... tokens/photo mesurés ~9200 contre ~700 supposé") est invalidée sur le mécanisme de résolution, et le chiffre "~9200 tokens/photo" n'était de toute façon pas une vraie mesure — aucun log `usage_metadata` n'existe côté `analyzer.py` (contrairement au chat frontend, `TOKEN_OPTIMIZATION_PLAN.md`) — mais une division du total facturé par des hypothèses (nombre d'analyses T2, photos/annonce) elles-mêmes approximatives, et qui utilisaient en plus un plafond de 4 photos/annonce alors que le code en autorise jusqu'à 8 (`analyzer.py:201`). La vraie cause du coût élevé des tokens-image T2 reste à établir par une mesure directe, pas par déduction.
- **Corrigé** : `TODO.md` (entrée "Ciblage des photos") reformulée pour ne plus affirmer "2048px" comme cause confirmée. `ARCHITECTURE.md` n'était pas concerné (aucune mention de résolution photo). Recommandation ajoutée au TODO : instrumenter `_call_gemini_json` avec un log `usage_metadata` (même principe que le chat) avant de retenter un diagnostic chiffré.

[2026-09-06] [PRO] Tier 2 (Analyste) migré vers gemini-3.7-flash (tarif de lancement) + alerte email programmée pour la fin du tarif → codé, non testé en conditions réelles.
- **Contexte** : suite à l'analyse de coûts (entrée précédente), comparatif utilisateur des tarifs Gemini par tier (3.1 Pro, 3.7 Flash, 2.5 Flash, 2.5 Flash-Lite). Vérifié via recherche web (Artificial Analysis, sources citées à l'utilisateur) plutôt qu'accepté tel quel : confirmé que `gemini-3.7-flash` existe bien (sorti août 2026), score Intelligence Index 56 contre 52 pour `gemini-3.6-flash` actuel, plus rapide (340 vs 225 tokens/s), au tarif de lancement 0,75$/3,75$ par 1M tokens (in/out) jusqu'au 31/12/2026 puis 1,50$/7,50$ ensuite — identique au tarif standard déjà payé pour 3.6-flash, donc aucune régression de coût à terme, juste une remise temporaire. **Écarté explicitement** : les "Gemini 2.5 Flash"/"2.5 Flash-Lite" du comparatif utilisateur sont la génération que ce projet a quittée le 2026-07-31 précisément parce que Google la retire en octobre 2026 (`CLAUDE.md`) — les reprendre aurait été une régression déguisée en économie.
- **Retour utilisateur** : le changement de modèle est qualifié à raison de correctif temporaire ("un plaster sur le bobo") plutôt qu'un vrai plafond de coût — le vrai levier structurel reste le pool d'annonces partagé entre utilisateurs (`TODO.md`, toujours non implémenté, clarifié suite à une confusion de l'utilisateur qui le croyait déjà en place) et le ciblage des photos envoyées à Gemini (encore à planifier).
- **`config.py::GEMINI_MODELS`** : `default_analyst` → `gemini-3.7-flash`, ajouté à `available` (`gemini-3.6-flash` conservé dans la liste pour ne pas forcer les comptes déjà personnalisés en Firestore).
- **`backend/analyzer.py`** : les 2 fallbacks codés en dur (`analyze_deal`, `analyze_deal_light`) alignés sur le nouveau défaut.
- **`src/hooks/useBotConfig.js`** / **`src/components/ConfigPanel.jsx`** : état initial, `handleResetDefaults` et liste de secours mis à jour en miroir.
- **`backend/scripts/analyze_funnel_by_user.py`** : table de coûts T2 mise à jour au tarif de lancement réel (0,75$/3,75$) pour que les estimations de rentabilité du script restent justes jusqu'au 31/12/2026.
- **Alerte de fin de tarif** : Routine programmée (`trig_01M4kHt39BWECnmBKrf6W4a6`) pour le 2026-12-15 — ouvre une session dédiée qui revérifiera le pricing Gemini réel à cette date et enverra un email de recommandation, plutôt qu'une simple note documentaire qui dépendrait d'une relecture manuelle.
- **Non testé en conditions réelles** (pas d'accès Gemini depuis cet environnement de dev) — à valider par l'utilisateur : qualité des analyses Tier 2 inchangée ou meilleure, pas de régression de facturation.

[2026-09-06] [FLASH] Analyse des factures GeminiDev (août + septembre 2026) → cause racine identifiée, plan sommaire de migration Firestore documenté.
- **Contexte** : demande utilisateur — les coûts Firestore/Gemini restent élevés malgré l'optimisation tokens du 2026-08-23, alors qu'elle était censée régler le problème.
- **Diagnostic (facture août, 102,57$)** : trois causes cumulées, aucune isolément suffisante — (1) migration des modèles Flash/Flash-Lite du 2026-07-31 a multiplié le prix par token par ~3-4x indépendamment du volume ; (2) `analyzer.py::_download_and_optimize_image` envoie les photos à 2048px pour les Tiers 2/3 (jamais optimisé, contrairement au chat qui utilise 1024px) — tokens/photo mesurés ~9200 contre ~700 supposé dans `TOKEN_OPTIMIZATION_PLAN.md` ; (3) le bug de rejeu intégral des photos du chat (corrigé le 2026-08-23) n'a couvert que 8 jours sur les 31 facturés.
- **Facture septembre (partielle, comparaison période-à-période)** : confirme le correctif chat (tokens Pro -25%/-9%, transfert réseau Firestore -100%) ; révèle une nouvelle tendance à surveiller, Firestore Reads/Writes +41%/+74%, non expliquée par les causes déjà identifiées.
- **Firestore isolé** : ~16,16$/102,57$ (août) et ~2,36$/12,12$ (septembre partiel) — réel mais secondaire face à Gemini API (~80-85% de la facture).
- **Plan sommaire de remplacement de Firestore** documenté (`docs/management/plans/FIRESTORE_MIGRATION_PLAN.md`, `TODO.md`) suite à la confirmation utilisateur que le serveur du bot peut aussi héberger Postgres/API-WS/frontend (abandon de GitHub Pages envisagé) — Firebase Auth et Storage restent hors périmètre (coût négligeable). Bloqué sur un point réseau (accessibilité publique du serveur) avant toute implémentation — aucun code écrit sur ce chantier.

[2026-08-29] [PRO] Fix : commande `SCAN_URL` ("Scan d'URL Direct") sans retour ni log → poussé sur `dev`, à valider en conditions réelles.
- **Symptôme signalé** : l'utilisateur constate que les requêtes de scan d'URL perso ne fonctionnent plus — "rien ne revient et rien dans les logs".
- **Cause 1, confirmée** : `main.py::main_loop()` logue le dispatch/les erreurs de commandes (`SCAN_URL` inclus) via `logging.getLogger(__name__)`, jamais raccordé au `FirestoreHandler` — même piège logger déjà documenté (`CLAUDE.md`, `DATA_FLOW.md` §7), mais jusqu'ici jamais corrigé pour `main.py` lui-même. Si `scan_specific_url()` échoue avant d'avoir logué quoi que ce soit via `self.logger` (le logger per-utilisateur), l'erreur est totalement invisible pour l'utilisateur alors qu'elle existe bien côté serveur.
- **Cause 2, hypothèse la plus probable du blocage lui-même** : `bot.py::scan_specific_url()` appelait `self._browser_semaphore.acquire()` sans timeout — si un thread de scan automatique (Facebook/Kijiji) reste accroché sans jamais relâcher son permis (navigateur figé pendant un blocage anti-bot déjà documenté dans `TODO.md`, `headless=False` depuis le 2026-08-25, `close_session()` sans aucun timeout Playwright), la commande reste bloquée indéfiniment, sans exception donc sans aucun log.
- **`main.py`** : tous les logs de dispatch de commandes (`Commande reçue`, `Lancement de la commande...`, erreurs synchrones/asynchrones/post-pause, type inconnu) basculés de `logger` (module) vers `bot.logger` (per-utilisateur, visible dans le LogViewer).
- **`backend/bot.py::scan_specific_url()`** : `self._browser_semaphore.acquire()` → `acquire(timeout=300)` — si le délai est dépassé, log d'erreur explicite + reset du statut à `idle` au lieu d'un blocage silencieux sur `scanning_url`.
- **Tests** : `backend/test_bot.py` — 38/40 verts (2 échecs préexistants confirmés indépendants de ce correctif via `git stash`). Syntaxe validée (`py_compile`). **Non testable en conditions réelles depuis cet environnement de dev** (pas d'accès Playwright/réseau/production) — reste à confirmer par l'utilisateur : (a) une erreur de `SCAN_URL` produit désormais un log visible, (b) si la cause 2 est la bonne, un message d'erreur clair apparaît après 5 min au lieu d'un blocage silencieux.

---

[2026-08-27] [PRO] Requalification via le chat : remplacement de la ré-analyse Gemini par un patch direct (2e confrontation Opus) → validé en conditions réelles.
- **Contexte** : après un premier test réel de l'entrée précédente (Lot 2), retour utilisateur : "ça a pris du temps" et la correction n'a pas été suivie du premier coup (l'IA a reconverti vers l'ancienne analyse). Demande explicite : confronter Opus sur la nécessité de tout ré-analyser, chercher plus rapide sans rien perdre — suggestion de l'utilisateur : traiter la correction comme un "Tier 4" dédié.
- **2e revue Opus** (après lecture directe du code, pas que du résumé) : verdict tranché, la ré-analyse n'est pas juste lente, elle est aveugle et potentiellement destructrice — `bot.py::analyze_single_deal` passait `imageUrls` (Facebook, expirées) jamais `storageImageUrls`, la "ré-analyse" tournait donc texte-seul sur une annonce un peu ancienne (explique directement le symptôme : l'IA repart des mêmes infos que l'analyse d'origine) ; un rejet Tier 1 au 2e passage écrasait en plus `aiAnalysis` en entier (4 champs restants). Recommandation : patch déterministe direct sur `aiAnalysis` (les champs sont déjà validés/bornés côté client avant l'affichage du bouton Appliquer, aucune ré-inférence IA nécessaire), généralisant le mécanisme déjà en prod pour `manualClassification` (2026-08-16). **Nom "Tier 4" écarté** : les Tiers 1-3 sont une escalade d'un même acte d'inférence, un patch n'infère rien — documenté comme "Override utilisateur", dans la même famille que `manualClassification`.
- **`repository.py`** : nouveau `_get_manual_analysis_overrides()` (lit `manualAnalysisOverrides`, même principe que `_get_manual_classification`), re-superposé sur `analysis_data` dans `update_deal_analysis`/`update_deal_data_and_analysis` AVANT chaque écriture — la correction survit désormais à tout futur scan/"Ré-analyser" (`aiAnalysis` est toujours réécrit intégralement, jamais fusionné côté backend sans ce mécanisme).
- **`bot.py`** : fix indépendant signalé par la revue, inclus dans le même lot — `analyze_single_deal` utilise désormais `storageImageUrls or imageUrls` (comme `backfill_sold_scores.py`/`reanalyze_sold_deals.py`) ; corrige aussi le bouton "Ré-analyser" classique sur les annonces anciennes.
- **`firestoreService.js`** : nouvelle `applyManualAnalysisOverrides(dealId, chunkId, userId, fields, currentAnalysis)` — écrit directement `aiAnalysis.<champ>` + `manualAnalysisOverrides.<champ>` sur le doc, recalcule `interestScore` (`computeInterestScore`, déjà dupliqué côté JS) et patche le miroir `deals_index` (clés courtes existantes) — plus aucun appel Gemini. Suppression du code mort `formatRequalificationComment` (n'a plus d'appelant).
- **`useDealChat.js`** : `applyRequalificationProposal` appelle `applyManualAnalysisOverrides` au lieu de `retryDealAnalysis` — effet immédiat. `pendingRequalificationNoteRef` conservé (Gemini doit toujours être informé au tour suivant qu'une correction a été appliquée) mais le texte n'annonce plus une "analyse en cours".
- **Mise à jour optimiste** : nouveau `useDealsManager.js::handleAnalysisOverridesApplied`, threadé `Dashboard.jsx → DealCard → DealAnalysisModal → DealChatPanel` (même nécessité que `handleGalleryImageAdded`/`handleSetClassification` : `loadedDeals` n'est pas un listener temps réel) — la fiche ouverte reflète la correction sans reload.
- **`npx vite build`** + compilation Python : succès. **Validé en conditions réelles par l'utilisateur** ("Ça marche") — application instantanée confirmée.

[2026-08-27] [PRO] Requalification d'annonce via le chat Gemini (Lot 2 du plan CHAT_GALLERY_REQUALIFICATION_PLAN.md) → codé, non testé en conditions réelles.
- **Contexte** : l'utilisateur a demandé à Gemini, en conversation sur une annonce, de "requalifier l'annonce complètement (modification de la DB)" — Gemini a répondu par un texte décrivant les changements, sans jamais les appliquer. Vérifié dans le code : seules 3 fonctions étaient disponibles pour le chat (`propose_restoration_step`, `propose_restoration_reorder`, `request_photo_review`), aucune permettant de requalifier une annonce. Le Lot 2 du plan validé le 2026-08-21 couvrait exactement ce besoin mais n'avait jamais été implémenté ("Implémentation non démarrée") — pas un bug, une fonctionnalité conçue mais jamais codée.
- **`geminiChatService.js`** : nouvelle fonction `propose_deal_requalification` (function calling, indépendante d'`isPurchased` contrairement aux outils de restauration — un désaccord factuel peut survenir sur n'importe quelle annonce discutée) — verdict (enum `NEW_VERDICTS`), les 5 scores (0-10), marque/modèle/année/pays/couleur/finition/échelle de manche, tous optionnels sauf `justification` (obligatoire). `validateDealRequalificationProposal()` clamp/valide chaque champ (même principe que `validateRestorationStepProposal` — une valeur hallucinée hors plage/énumération est silencieusement écartée plutôt que rendue avec un bouton Appliquer qui écrirait n'importe quoi), retourne `null` si aucun champ valide ne subsiste. `formatRequalificationComment()` formatte la proposition en texte français, seule source utilisée à la fois par le commentaire envoyé au backend et par les libellés de la carte UI (`REQUALIFICATION_FIELD_LABELS`, exporté).
- **Application réelle** : "Appliquer" appelle `retryDealAnalysis(dealId, userId, commentaire)` — chemin `ANALYZE_DEAL` déjà existant et en production, décision reprise du plan (revue Opus 2026-08-21) plutôt qu'un champ Firestore dédié + mirroring d'index, qui avait plusieurs pièges bloquants identifiés à l'époque (StatsView écrasant la requalification, interestScore jamais recalculé, aucune validation avant application). Conséquence assumée : application non littérale (le backend peut interpréter différemment) et asynchrone (dépend du bot actif).
- **`useDealChat.js`** : `withRequalification`, 3e drapeau indépendant de function calling (comme `withPhotoRecall`, jamais gaté sur `isPurchased`) — `toolsSignature()` étendue à 3 caractères. `buildRequalificationProposalFromCalls()` extrait au plus UNE proposition par tour (contrairement aux propositions de restauration qui s'accumulent), threadée dans les 3 chemins existants de `executeTurn` (tour normal, tour rejoué après rappel de photo, rappel de photo invalide) au même titre que les propositions de restauration. Nouveau champ Firestore `requalificationProposal` (singulier, pas un tableau) sur le message modèle + `requalificationProposalState` pour l'état Appliquer/Ignorer (`markChatMessageRequalificationProposalStatus`, nouveau dans `firestoreService.js`) — jamais mélangé aux champs `restorationProposals`/`restorationProposalStates` existants, pour ne courir aucun risque sur le code de restauration déjà en production.
- **Piège corrigé avant même un premier test** (identifié en implémentant, pas en revue a posteriori) : `buildDealContextText()` (l'analyse de l'annonce) n'est injecté qu'au TOUT PREMIER message de la conversation — sans rien de plus, Gemini n'aurait jamais vu qu'une correction venait d'être appliquée sur les tours suivants, et aurait pu la re-proposer indéfiniment. Nouveau `pendingRequalificationNoteRef` : une note invisible posée par `applyRequalificationProposal()`, préfixée au PROCHAIN message utilisateur (jamais un tour orphelin — `sanitizeHistory` exige une alternance stricte user/model), puis effacée.
- **UI** : `RequalificationProposalCard` (`DealChatPanel.jsx`) — avant/après recalculé à CHAQUE rendu contre `deal.aiAnalysis` réel et courant (jamais un instantané capturé au tour de chat, même principe que `resolveRestorationReorderProposal` côté restauration), boutons Appliquer/Ignorer, état busy/erreur local réutilisant les maps déjà en place (clé `${messageId}:requalification`, jamais de collision avec les clés numériques des propositions de restauration).
- **`npx vite build`** : succès. Non testé en conditions réelles (pas d'accès Gemini/Firebase depuis cet environnement de dev) — à valider par l'utilisateur : proposition cohérente sur désaccord factuel argumenté, pas de proposition sur conversation banale, Ignorer sans effet sur l'annonce, Appliquer déclenche bien une commande `ANALYZE_DEAL` et le résultat final reflète la correction demandée.

[2026-08-26] [PRO] Fix : menu déroulant du sélecteur de ville invisible (z-index trop bas) → corrigé.
- **Contexte** : une fois le déploiement effectivement en place (voir entrée précédente), l'utilisateur confirme via DevTools — aucune erreur console, la requête vers Photon part bien — mais aucune suggestion visible. Diagnostic utilisateur confirmé : le menu déroulant se dessine **sous** la section suivante du panneau de configuration, pas au-dessus.
- **Cause** : `ConfigPanel.jsx::CityManagementSection` — les deux menus déroulants (catalogue existant ET suggestions Photon) utilisaient `z-10`. Ce bug préexistait déjà pour le menu du catalogue (code inchangé par la refonte du 2026-08-26) mais restait invisible en pratique, ce cas étant rarement déclenché ; le nouveau sélecteur Photon, lui, se déclenche dès 2 caractères et rend le bug systématique.
- **Corrigé** : les deux menus passent à `z-50`, la convention déjà utilisée ailleurs dans l'app pour ce type de popover flottant (`Dashboard.jsx`, `Navbar.jsx`, `RestorationPlanPanel.jsx`, `DealChatPanel.jsx`, `DealCardActions.jsx`, `ImageGallery.jsx`) — jamais `z-10`, réservé aux éléments empilés localement dans une carte.
- **`npx vite build`** : succès. Non revalidé visuellement dans un vrai navigateur (pas d'accès à l'app authentifiée depuis cet environnement) — à confirmer par l'utilisateur.
- **Ajustements UX demandés dans la foulée** : (1) flèche bas/haut du clavier pour naviguer dans les suggestions Photon — réutilise `createSuggestionKeyHandler` (`SearchSuggestions.jsx`, déjà utilisé pour l'autocomplétion de catégories, même convention ↓ ↑ Entrée Échap) ; (2) cliquer (ou valider au clavier) une suggestion Photon ne fait plus qu'insérer son libellé dans le champ et mémoriser le candidat choisi (`pickedCandidate`) — l'ajout réel reste une action explicite sur le bouton "+", jamais déclenché par la sélection elle-même. `pickedCandidate` est effacé dès que l'utilisateur retape du texte (le champ ne représente alors plus ce candidat précis), auquel cas "+" retombe sur l'ajout par chaîne simple (repli historique).

[2026-08-26] [PRO] Fix : menu déroulant toujours invisible malgré le z-index (clippé par un ancêtre `overflow-hidden`) → portail DOM.
- **Contexte** : le passage à `z-50` (entrée précédente) n'a pas suffi — l'utilisateur confirme via capture d'écran que le menu reste "derrière" la section suivante. Root cause réelle : `z-index` n'a d'effet qu'au sein d'un même contexte d'empilement CSS ; `CollapsibleSection.jsx` (parent direct de "Ajouter une zone") a `overflow-hidden` sur son conteneur (nécessaire à l'animation d'ouverture/fermeture) — un descendant en position absolue ne peut PAS dépasser visuellement cette boîte, quel que soit son z-index. Retirer `overflow-hidden` globalement était jugé trop risqué (utilisé par toutes les sections du panneau, probablement nécessaire au rendu de l'animation).
- **`ConfigPanel.jsx::CityManagementSection`** : le menu (catalogue ET Photon, unifiés en un seul bloc) est désormais rendu via `createPortal` (déjà importé/utilisé ailleurs dans ce même fichier pour le panneau lui-même) directement dans `document.body`, en `position: fixed`, avec ses coordonnées calculées depuis `getBoundingClientRect()` du champ de recherche — sort ainsi de tout ancêtre qui pourrait le clipper. Repositionné sur `scroll`/`resize` (écouteur en phase de capture sur `window`, seul moyen de capter le défilement d'un conteneur interne qui ne fait pas remonter l'événement en phase de bulles).
- **Ajustement demandé en parallèle** : après sélection d'un candidat (menu fermé, champ rempli), les flèches ↑/↓ rouvrent la liste au lieu de rester sans effet — l'utilisateur peut continuer à parcourir sans retaper de texte.
- **`npx vite build`** : succès. Non revalidé visuellement (pas d'accès à l'app authentifiée depuis cet environnement) — à confirmer par l'utilisateur, notamment le repositionnement pendant un défilement du panneau.

[2026-08-26] [PRO] Fix définitif : menu de suggestions rendu en flux normal (abandon du portail/position fixe) → capture mobile confirmant le problème persistant.
- **Contexte** : le portail (entrée précédente) était bien déployé et syntaxiquement correct (vérifié dans le bundle de prod), mais l'utilisateur, sur mobile, voit toujours le menu caché — capture d'écran à l'appui, seul un fin liseré apparaît sous le champ avant la section "Filtres" suivante. Cause probable : position fixe calculée par `getBoundingClientRect()` fiable au moment du calcul, mais le clavier virtuel mobile redimensionne le viewport visuel sans redimensionner le viewport de mise en page de façon prévisible d'un navigateur à l'autre — un classique des éléments `position: fixed` sur mobile. Proposition de l'utilisateur, retenue : faire grandir le bloc englobant plutôt que superposer un calque.
- **`ConfigPanel.jsx::CityManagementSection`** : le menu (catalogue + Photon) n'est plus en position absolue/fixe/portail — il est rendu dans le flux normal du document, juste après le champ, avec une simple marge (`mt-2`). Il pousse mécaniquement le contenu suivant (sections "Filtres"/"IA") vers le bas plutôt que de se superposer — aucun z-index, aucune dépendance à `overflow`/`getBoundingClientRect`/écouteurs de scroll, donc insensible par construction au clavier virtuel, à `CollapsibleSection` (`overflow-hidden`) et à l'ordre d'empilement d'autres portails (la fenêtre modale elle-même). `items-start` ajouté sur la ligne flex champ+bouton pour empêcher le bouton "+" de s'étirer à la hauteur du menu (comportement par défaut de `align-items: stretch`).
- **`npx vite build`** : succès. Toujours non revalidé visuellement (pas d'accès à l'app authentifiée depuis cet environnement) — à confirmer par l'utilisateur.

[2026-08-26] [PRO] Sélecteur de ville : tri par géolocalisation réelle du navigateur → validé en conditions réelles (menu enfin visible) puis étendu.
- **Confirmation utilisateur** : le menu de suggestions s'affiche enfin correctement (fix flux normal, entrée précédente). Demande de suite : trier les suggestions par proximité à la position GPS réelle de l'utilisateur plutôt que (seulement) par proximité aux villes déjà configurées — plus pertinent pour un utilisateur en déplacement, dont les villes configurées ne reflètent pas forcément où il se trouve maintenant.
- **`useCitySuggestions.js`** : `navigator.geolocation.getCurrentPosition()` appelée une seule fois par instance du sélecteur (pas à l'ouverture du panneau de config entier, seulement à la première recherche tapée) — jamais bloquant, un refus/erreur/navigateur sans support retombe silencieusement sur le tri existant (distance à la ville déjà configurée la plus proche, `minDistanceToCities`). Une fois la position obtenue, elle devient prioritaire pour tout tri ultérieur (recherches suivantes de la même session du sélecteur) — la toute première recherche peut encore utiliser le repli le temps que la géolocalisation réponde (best-effort, non attendue).
- **`npx vite build`** : succès. Non testé en conditions réelles (permission de géolocalisation, précision) — à valider par l'utilisateur.

[2026-08-26] [PRO] Sélecteur de ville : biais géographique envoyé à Photon + catalogue et Photon affichés simultanément.
- **Retour utilisateur (captures à l'appui)** : (1) taper "sain"/"saint" seul ne renvoyait qu'une correspondance catalogue ("Saints", résidu de test antérieur au correctif "clic = remplir seulement") — Photon était entièrement masqué dès qu'il y avait un match catalogue, quelle que soit sa pertinence ; (2) pour "Saint-marc" seul, aucun résultat québécois ne figurait parmi les 8 renvoyés par Photon — notre tri par distance ne peut reclasser QUE ce que Photon a déjà choisi de renvoyer, il ne peut pas en faire apparaître un absent. L'utilisateur a proposé de passer des paramètres à Photon plutôt que de retrier après coup, et demandé que le catalogue et Photon s'affichent tous les deux (la déduplication réelle se faisant à l'exécution, pas en masquant une liste par précaution).
- **Vérifié directement (curl)** : Photon accepte `lat`/`lon` en paramètres de requête pour biaiser SON PROPRE classement. Test : `q=sain` sans biais → résultats France/Russie ; avec `lat=45.53&lon=-73.35` (Montérégie) → **8/8 résultats québécois**, Saint-Bruno-de-Montarville en premier. Confirme l'hypothèse de l'utilisateur : le vrai problème était l'absence de biais serveur, pas la logique de tri côté client.
- **`useCitySuggestions.js`** : `lat`/`lon` transmis à l'appel Photon dès que `navigator.geolocation` a répondu (déjà demandée depuis l'entrée précédente) — remplace le tri client comme mécanisme principal de pertinence géographique ; le tri client (`minDistanceToCities`) reste un repli pour le cas sans géolocalisation.
- **`ConfigPanel.jsx::CityManagementSection`** : Photon est désormais interrogé et affiché dès 2 caractères **indépendamment** d'une correspondance catalogue — les deux listes s'affichent l'une sous l'autre (en-têtes "Déjà dans le catalogue" / "Nouvelles suggestions" quand les deux sont présentes) au lieu que le catalogue masque totalement Photon.
- **`npx vite build`** : succès. Non testé en conditions réelles dans l'app — à valider par l'utilisateur. La ville "Saints" (résidu de test) reste dans le catalogue, non traitée ici (donnée, pas du code).

[2026-08-26] [PRO] Retrait des suggestions "catalogue partagé" du sélecteur de ville — source de confusion confirmée en conditions réelles.
- **Constat utilisateur (capture à l'appui)** : taper "Sa" faisait apparaître "La saline" et "Saints" comme suggestions "déjà dans le catalogue" — des villes ajoutées par **d'autres utilisateurs de l'app**, sans aucun rapport avec sa zone (Montérégie), mélangées visuellement aux vraies suggestions Photon pertinentes. Cause : le catalogue de villes est **partagé entre tous les utilisateurs** (`artifacts/{APP_ID}/cities`) — une correspondance texte pure sur ce catalogue n'a aucune notion de pertinence géographique, contrairement à Photon qui est désormais biaisé par la position réelle de l'utilisateur.
- **Décision (proposée à l'utilisateur, choisie parmi 3 options)** : retirer entièrement cette liste de suggestions plutôt que de la filtrer par distance ou simplement mieux la labelliser — elle est devenue redondante une fois Photon fiable, et sa seule utilité (éviter un doublon exact) était déjà garantie indépendamment par `add_city_auto()` côté backend, jamais par cette liste d'affichage.
- **`ConfigPanel.jsx::CityManagementSection`** : suppression de `suggestions`/`addCityToWhitelist` (filtre catalogue par sous-chaîne) et de tout l'affichage associé — seules les suggestions Photon (géolocalisées) restent proposées à la saisie. La réactivation d'une ville déjà scannée par le passé pour CET utilisateur reste possible autrement (elle réapparaît directement dans "Villes Actives" si `isScannable` repasse à `true` — non affectée par ce retrait, qui ne concernait que les correspondances **texte** dans le catalogue partagé).
- **`npx vite build`** : succès. Non testé en conditions réelles — à valider par l'utilisateur.

[2026-08-26] [PRO] Diagnostic : le sélecteur de ville en direct ne s'affichait pas → cause trouvée, le déploiement n'avait jamais eu lieu.
- **Symptôme signalé** : après le push de l'entrée précédente (refonte du flux d'ajout de ville), l'utilisateur ne voit aucune suggestion à la saisie dans le ConfigPanel.
- **Vérifié** : `git fetch origin dev` confirme que la branche `dev` sur GitHub pointe bien vers le commit attendu (`3a4e29a`). Mais `mcp__github__actions_list` (workflow `deploy.yml`) montre que le dernier run déclenché reste celui du commit **précédent** (`3ceec48`, run #389) — aucun run, y compris `in_progress`/`queued`, n'existe pour `3a4e29a`. Le push a bien mis à jour la référence Git, mais n'a jamais émis l'événement `push` qui déclenche `deploy.yml` (`on: push: branches: [master, dev]`) — cause exacte non identifiée (comportement isolé à ce seul push ; tous les push précédents de cette session ont normalement déclenché un run). Le serveur de prod tournait donc toujours sur l'ancien code : le sélecteur n'a jamais pu être testé, indépendamment de la qualité du code lui-même.
- **Action** : nouveau commit (cette entrée) pour forcer un nouvel événement `push` sur `dev` et redéclencher le pipeline. À vérifier après ce push que le run apparaît bien et se termine en succès.

[2026-08-26] [PRO] Refonte du flux d'ajout de ville (picker en direct + `region_hint`) → Résultat : codé et testé unitairement (backend), non testé en conditions réelles.
- **Contexte** : suite au bug "Saint-lambert" géocodé en France (entrée du 2026-08-25 ci-dessous), l'utilisateur a demandé un script de correction de coordonnées ET une revue de la méthodologie d'ajout de ville, plutôt qu'une simple correction manuelle isolée jugée impraticable ("je ne sais même pas comment faire ni quelles sont les bonnes coordonnées"). Plusieurs pistes proposées puis rejetées par l'utilisateur avant la conception finale :
  - `countrycodes=ca` (suggestion Opus) rejetée par l'utilisateur : force le Canada (casse un catalogue légitimement multi-pays, ex: un autre utilisateur avec une ville "paris" en France) et ne résout pas les homonymes intra-Canada.
  - Biais de proximité vers un centroïde/une boîte englobante globale du catalogue existant, rejeté par contre-exemple de l'utilisateur : un utilisateur avec seulement "Paris, France" configuré qui veut ajouter "Montréal" (Québec) obtiendrait le mauvais homonyme, la proximité pure étant inopérante quand la nouvelle ville n'est proche de rien de déjà connu.
  - Design final retenu par l'utilisateur : un **sélecteur de suggestions en direct** (type autocomplétion d'adresse), plusieurs candidats géolocalisés affichés à la saisie, biaisés (jamais filtrés) par distance à la ville déjà configurée la plus proche, affinable en tapant un pays/une province/un code postal.
- **2 revues critiques par un agent Opus** avant implémentation : (1) l'ordre des chaînes de requête Nominatim n'était pas la vraie cause — CityFinder (`city_finder.py`) cliquait déjà la **première** suggestion Facebook à l'aveugle, une source d'erreur indépendante et potentiellement plus impactante que le géocodage Nominatim ; (2) Nominatim interdit explicitement l'autocomplete (recherche à chaque frappe) dans sa politique d'usage, et ce projet n'a pas de serveur HTTP pour en faire proxy (`main.py` est un worker de commandes Firestore, pas une API) — **Photon** (photon.komoot.io, OSM, CORS activé) retenu à la place pour la recherche en direct côté navigateur.
- **`backend/scraping/city_finder.py`** : `find_city_id_and_coords()` itère désormais TOUTES les suggestions Facebook proposées et sélectionne celle dont le texte contient l'indice de région (`region_hint`, ex: "Québec") au lieu de cliquer `.first()` aveuglément ; repli sur la première suggestion (avec avertissement) si aucune ne correspond. Retourne désormais un 4-uplet `(city_id, city_coords, matched_label, matched_confidently)`.
- **`backend/bot.py::add_city_auto()`** : accepte désormais soit une chaîne simple (comportement historique, repli), soit un dict structuré `{name, latitude, longitude, region_hint}` produit par le nouveau sélecteur frontend. Priorité absolue aux coordonnées confirmées visuellement par l'utilisateur sur toute coordonnée devinée (Facebook, puis Nominatim en dernier repli). Flag non-bloquant `needsReview: true` posé sur la ville si la suggestion Facebook cliquée ne correspond pas à l'indice de région fourni — signale sans jamais bloquer l'ajout (un `city_id` Facebook reste obligatoire pour le scraper quelle que soit la confiance).
- **`main.py`** : renommage cosmétique du paramètre `ADD_CITY` (`city_name` → `payload`), aucun changement de comportement.
- **Frontend** : `src/utils/geo.js` (Haversine + `minDistanceToCities`, distance à la ville la plus proche — jamais un centroïde global, pour respecter le contre-exemple Paris/Montréal), `src/hooks/useCitySuggestions.js` (recherche Photon débouncée, résultats reclassés par distance), `src/hooks/useCities.js::handleAddCity` (accepte le payload structuré, traduit `regionHint` → `region_hint`), `src/components/ConfigPanel.jsx::CityManagementSection` (affiche les suggestions Photon en direct quand aucune ville du catalogue partagé ne correspond déjà, cliquables pour ajouter via le nouveau chemin structuré ; dropdown de réactivation du catalogue existant et bouton "+" de secours inchangés).
- **Tests** : 5 nouveaux tests (`backend/test_bot.py::TestAddCityAuto`) — priorité des coordonnées, `needsReview` posé/non posé selon correspondance de région, repli Nominatim si payload simple. Suite complète toujours à 2 échecs préexistants sans rapport. `npx vite build` : succès.
- **Non testé en conditions réelles** — à valider par l'utilisateur après fusion : rendu du sélecteur dans l'app, ajout réel d'une ville via Photon, comportement de `needsReview` sur un vrai cas Facebook ambigu.
- **Reste à faire, non traité dans cette entrée** : la correction proprement dite de la fiche "Saint-Lambert" (id + coordonnées) reste bloquée — impossible à ce jour de vérifier si son `city_id` Facebook actuel (`110358832318694`) est réellement un homonyme étranger (recherche web infructueuse). À vérifier via un lookup `CityFinder` avec `region_hint="Québec"` une fois ce code déployé, avant toute écriture corrective.

[2026-08-25] [PRO] Diagnostic : coordonnées de "Saint-Lambert" en France dans un scan Kijiji → armé, résultat à venir.
- **Contexte** : après le passage de Facebook en `headless=False` (entrée précédente), le remplissage international (Belgique) persiste — hypothèse headless réfutée. En creusant un log Kijiji ("Scan Kijiji (URL directe)", pris à tort pour un scan manuel non autorisé — c'est en fait le log générique interne de `scan_search_url()`, utilisé aussi bien par le scan automatique que manuel), les coordonnées utilisées pour la ville configurée "Saint-Lambert" se sont révélées être `48.9382, -0.5474` — en France, pas au Québec. Ni le fichier statique `backend/resources/city_coordinates.json` (qui a bien un "saint-lambert" mais en Abitibi, 48.93/-79.17, un 3e homonyme distinct) ni Montréal-Saint-Lambert (~45.5/-73.5) ne correspondent — la valeur vient donc du catalogue Firestore de l'utilisateur, probablement géocodée par erreur vers un homonyme français à l'ajout de la ville (même piège que "Beloeil" Québec/Wallonie déjà documenté).
- **`backend/scripts/audit_city_coordinates.py`** (nouveau, lecture seule) : vérifie les coordonnées de toutes les villes configurées (tous utilisateurs) contre une boîte englobante généreuse pour l'est de l'Amérique du Nord (lat 40–55, lon -85 à -55), signale toute ville hors zone.
- **Résultat (exécuté en production)** : confirmé — **une seule ville sur les 22 configurées** par l'utilisateur principal a des coordonnées hors zone (`Saint-lambert -> lat=48.9382022, lon=-0.547455`, France). Aucune autre ville de son catalogue n'est concernée. Un autre utilisateur de l'app a une ville "paris" à Paris, France (`48.8589, 2.3200`) — probablement intentionnel (utilisateur différent), pas traité comme une anomalie. `run_once.py` repassé à `ACTIVE=False`.
- **Reste à faire** : corriger les coordonnées de "Saint-lambert" vers le vrai Saint-Lambert (Québec, Rive-Sud de Montréal, ~45.5/-73.5) — correction de donnée simple via le ConfigPanel de l'app (pas de code à changer), recommandée à l'utilisateur plutôt qu'un script d'écriture Firestore pour ce cas isolé.

[2026-08-25] [PRO] Facebook passé en `headless=False` (piste headless vs remplissage international) → Résultat : codé, non testé en conditions réelles.
- **Contexte** : après le correctif de l'ordre des filtres (entrée ci-dessous), le remplissage massif d'annonces belges persistait (78/120 cartes hors liste sur un cycle). L'utilisateur a testé lui-même en navigateur "anonyme" (donc sans compte, comme le scraper) et n'a JAMAIS observé ce remplissage international, même avec le tri par date — la seule différence structurelle restante identifiée : le mode **headless** du bot.
- **Vérification** : la géolocalisation n'est pas totalement absente comme suspecté plus tôt (H4 de l'audit Opus) — `ScraperConfig` (`backend/scraping/config.py`) a des défauts sensés (`locale=fr-CA`, `timezone=America/Montreal`, `geolocation≈Montréal`), seul le point précis de l'ancre n'est jamais substitué (code mort confirmé, mais imprécision modeste — Montréal reste à ~20km de Saint-Bruno). Peu probable à lui seul d'expliquer un déversement aussi massif de Belgique.
- **Indice trouvé en code** : `backend/scraping_leboncoin/core.py` tourne déjà en `headless=False` avec succès en production, sur ce **même** service/process (`xvfb` déjà installé pour lui dans `.github/workflows/deploy.yml`) — jamais appliqué à Facebook, qui restait sur le défaut `headless=True`.
- **`backend/scraping/config.py`** : `ScraperConfig.headless` par défaut passé à `False`. Aucun autre consommateur de cette classe (Kijiji a sa propre `KijijiScraperConfig` séparée, déjà `headless=True` par défaut, inchangée).
- **Risque assumé, non vérifiable depuis ce sandbox** : si `DISPLAY`/Xvfb n'était pas réellement exporté pour l'ensemble du process (hypothèse forte mais non confirmée — LeBonCoin tourne dans ce même process), Playwright échouerait au lancement pour Facebook plutôt que d'améliorer la qualité des résultats. **Vérifier en priorité au prochain déploiement que le premier cycle Facebook démarre sans erreur.**

[2026-08-25] [PRO] Cause racine confirmée par l'utilisateur : ordre d'application des filtres Facebook → Résultat : corrigé, non testé en conditions réelles.
- **Élément déclencheur** : l'utilisateur a testé manuellement l'URL de recherche Facebook (son propre navigateur, authentifié) — `query=guitare` retrouve l'annonce en premier, ce qui a d'abord fait suspecter la langue de la requête (hypothèse infirmée juste après : `query=guitar` en anglais retrouve aussi l'annonce depuis son navigateur). La vraie variable identifiée par l'utilisateur : **l'ordre d'application des filtres**. Si le filtre de tri par date (`creation_time_descend`) est la DERNIÈRE action effectuée (que ce soit via l'URL ou en cliquant dans l'UI), les résultats deviennent incohérents ("n'importe quoi") ; le filtre de PRIX doit impérativement être la dernière action.
- **`backend/scraping/core.py::scan_marketplace()`** : l'URL de navigation initiale contenait `minPrice`/`maxPrice` ET `sortBy=creation_time_descend` simultanément, puis `_apply_filters()` retapait le prix via l'UI par-dessus — une double application concurrente du prix, sans garantie sur l'ordre final effectif. Corrigé : l'URL initiale ne contient plus que `query`/`exact`/`sortBy` ; `_apply_filters()` (appelé après la navigation) reste la SEULE source de vérité pour le prix, appliquée sans ambiguïté en tout dernier avant le défilement.
- **Diagnostic complet de la journée** (résumé) : annonce 4380960138835811 confirmée introuvable en base (`find_deal_by_id.py`) → audit racine par agent Opus (2 pertes silencieuses corrigées côté instrumentation, non la cause finale) → `SCAN_URL` manuel confirme que l'annonce est bien active et que le pipeline de traitement fonctionne → tests manuels de l'utilisateur sur l'URL de recherche isolent l'ordre des filtres comme cause racine réelle, antérieure aux points d'ancrage.
- **Non testé en conditions réelles** — à valider par l'utilisateur après fusion : l'annonce (ou une annonce similaire actuellement mal classée) remonte-t-elle enfin dans un cycle automatique ?

[2026-08-25] [PRO] Instrumentation du scraper Facebook suite à l'audit racine (annonce 4380960138835811) → Résultat : codé, non testé en conditions réelles.
- **Suite du diagnostic ci-dessous** : l'utilisateur a demandé de chercher la cause racine, antérieure aux points d'ancrage, plutôt que de pousser le correctif de diagnostic tel quel — consultation d'un agent Opus pour un audit critique du pipeline `scan_marketplace()` (`backend/scraping/core.py`).
- **Conclusion de l'audit (résumé, non poussé comme JOURNAL séparé — directement suivi d'implémentation)** : deux pertes **silencieuses** de cartes identifiées avant même d'atteindre `bot.py` — (H1) `is_city_allowed()` (`core.py`) fait une égalité stricte de clé normalisée alors que le filtre `bot.py` en aval fait du sous-chaîne — une graphie courte sur la carte (ex: "Saint-Bruno" au lieu de "Saint-Bruno-de-Montarville") est rejetée AVANT d'atteindre la logique permissive de `bot.py` ; (H2) deux abandons complets sans aucune trace exploitable — localisation non extraite de la carte (`logger.debug`, invisible en production) et prix non extrait (aucun log du tout, `continue` implicite). Hypothèses secondaires : `_apply_filters()` rejoue le filtre prix via l'UI après coup et pourrait faire perdre `sortBy=creation_time_descend` en silence ; géolocalisation forcée inopérante en production (`FacebookScraper({}, {})` — dicts vides).
- **`backend/scraping/core.py`** : les deux abandons silencieux passent de `debug`/aucun log à `warning` avec ID Facebook + titre (grep-able), comptés séparément (`dropped_no_location`, `dropped_no_price`, nouveaux champs du dict retourné par `_scan_result()`). Alerte si `sortBy=creation_time_descend` disparaît de l'URL après `_apply_filters()`. Bloc `if price > 0 or "Gratuit"...` simplifié en garde explicite + retrait de l'imbrication devenue inutile (aucun changement de comportement).
- **`backend/bot.py`** : `total_cards_seen` (déjà calculé par le scraper, jusqu'ici jeté) et les deux nouveaux compteurs remontent dans `📊 Résumé du cycle Facebook`.
- **Aucun changement de décision** (mêmes annonces acceptées/rejetées qu'avant) — uniquement de la visibilité, pour trancher les hypothèses par preuve directe au prochain cycle plutôt que par déduction.
- **Non testé en conditions réelles** — à valider par l'utilisateur après fusion : que voit-on dans les logs du prochain cycle Facebook pour l'ancrage Saint-Bruno-de-Montarville ?

[2026-08-25] [PRO] Diagnostic : annonce 4380960138835811 introuvable en base → Résultat : jamais enregistrée, cause non identifiée avec certitude (pas de test réseau possible depuis le sandbox).
- **Contexte** : premier cycle Facebook post-fusion des points d'ancrage. L'ancre Saint-Bruno-de-Montarville a été scannée directement (recherche Facebook sur sa propre ville), mais le cycle n'a rien traité de nouveau (24 cartes vues, toutes rejetées "ville non autorisée" ou déjà connues). L'utilisateur signale que cette annonce précise (295$, Saint-Bruno-de-Montarville) reste introuvable dans l'app.
- **`find_deal_by_id.py` armé via `run_once.py`** : recherche directe par ID (lecture seule) à travers tous les utilisateurs — **introuvable**, ni sous l'ID brut ni préfixé `kijiji_`. Confirme que ce n'est pas un problème d'affichage frontend : l'annonce n'a jamais été écrite dans `guitar_deals`.
- **Hypothèse la plus probable (non confirmée, aucun accès réseau Facebook depuis ce sandbox)** : `scan_marketplace()` arrête son défilement dès stabilisation ("📜 Défilement stabilisé à 24 annonces après 3 itération(s)", observé dans les logs de ce cycle) — si cette annonce n'est plus assez récente pour figurer dans les ~24 premières cartes triées par `sortBy=creation_time_descend`, elle n'est simplement jamais vue par la recherche, indépendamment des points d'ancrage. Piste alternative écartée à ce stade faute de preuve : pas d'indice d'un bug introduit par le clustering (l'ancre couvrant Saint-Bruno a bien été recherchée directement).
- **Action immédiate recommandée à l'utilisateur** : utiliser `SCAN_URL` (scan manuel d'une URL précise, déjà existant) sur `https://www.facebook.com/marketplace/item/4380960138835811/` pour forcer l'ingestion de cette annonce précise, en contournant la découverte par recherche.
- **Non résolu / à suivre** : si `SCAN_URL` réussit à la récupérer, cela confirmerait l'hypothèse de profondeur de défilement (pas un bug d'ancrage) — resterait alors à décider si `max_ads`/la limite de défilement doit être augmentée pour ce cas.

[2026-08-25] [PRO] Implémentation des points d'ancrage (Facebook + Kijiji) et fiabilisation du rayon → Résultat : codé et testé unitairement, non testé en conditions réelles.
- **Suite de la validation empirique du 2026-08-24** : l'utilisateur a demandé, avant tout code, une explication de la généralisation (comment les ancrages sont déterminés, ce qui se passe pour un cluster distant type Québec/Paris), puis une fiabilisation du rayon (au lieu de 3 valeurs fixes devinées) et une réutilisation de la logique pour les autres scrapers — validé après présentation du plan (🚦 AIGUILLAGE), "go si c'est bien les 2 prérequis+ implémenter des anchor".
- **`backend/scraping/geo_clustering.py`** (nouveau, testé — `test_geo_clustering.py`, 6 tests) : `compute_anchor_clusters(cities, radius_km)`, couverture d'ensembles gloutonne pure et site-agnostique — sélectionne itérativement la ville non couverte qui couvre le plus d'autres villes non couvertes dans le rayon donné, jusqu'à couverture totale. Une ville sans coordonnées devient son propre cluster isolé plutôt que de faire échouer le calcul. `AnchorCluster.max_member_distance_km()` : distance entre l'ancre et son membre le plus éloigné — le strict nécessaire pour qu'un moteur de recherche à rayon explicite (Kijiji) couvre tout le cluster.
- **Prérequis 1 (fiabiliser le rayon Facebook)** : `_run_facebook_scan()` (`bot.py`) logue désormais `📏 Rayon observé pour 'X' : Ykm` à chaque scan — distance max réelle entre le point recherché et CHAQUE annonce retournée par Facebook, avant tout filtrage. Preuve directe (pas une hypothèse testée après coup) de l'étendue du rayon implicite Facebook ; alerte (`⚠️`) si une annonce dépasse `FACEBOOK_ANCHOR_RADIUS_KM` (80km, constante de classe) — signal à suivre dans le temps pour recalibrer si besoin.
- **Prérequis 2 (balayage de rayon)** : `audit_anchor_coverage.py` — `RADII_KM` passe de `[40, 65, 80]` à `range(20, 151, 5)`, avec calcul de la couverture COMBINÉE (union de tous les ancrages, pas juste par ancrage isolé) et détection automatique du plateau de saturation (premier rayon à partir duquel élargir encore ne trouve plus aucune annonce supplémentaire dans l'historique déjà indexé).
- **Intégration Facebook (`_run_facebook_scan`)** : boucle désormais sur les clusters (`compute_anchor_clusters(cities_to_scan, FACEBOOK_ANCHOR_RADIUS_KM)`) au lieu des villes individuelles — l'ordre de scan mélangé (2026-08-24) s'applique maintenant aux clusters. Le filtre STRICT multi-villes (mode `distance=0`) reste inchangé (s'appuie sur la liste complète des villes, pas sur le cluster). Le filtre par rayon (mode `distance>0`) a été corrigé pour mesurer la distance au **membre du cluster le plus proche** plutôt qu'à l'ancre seule — sinon la consolidation aurait artificiellement rétréci la zone couverte par rapport à une recherche dédiée par ville.
- **Intégration Kijiji (`_run_kijiji_scan`)** : même clustering, avec un garde-fou — une ville portant un `kijijiRadiusKm` explicite (réglage Firestore par ville) n'est **jamais** absorbée dans un cluster, toujours scannée seule avec ce rayon exact (intention explicite de l'utilisateur pour cette ville précise). Pour un cluster, le rayon envoyé à Kijiji = le plus grand entre `scanConfig.distance` (réglage global, à respecter tel quel) et `max_member_distance_km()` (strict nécessaire pour ce cluster) — jamais une constante fixe arbitraire, pour ne pas récupérer d'annonces hors de toute ville configurée sans besoin. `nearest_configured_city()` (post-filtre, inchangé) s'appuyant déjà sur toutes les villes configurées (pas seulement les ancres), une annonce d'un cluster reste rattachée à sa vraie ville d'origine.
- **Tests** : 6 nouveaux (`test_geo_clustering.py`) + 5 nouveaux dans `test_bot.py` (consolidation de villes proches, calcul du rayon minimal requis, respect d'un `scanConfig.distance` plus grand, non-clustering d'une ville avec `kijijiRadiusKm`, ré-attribution correcte de la ville d'une annonce issue d'un cluster). Suite complète : 82 tests (`backend/scraping/`) + 33 tests (`backend/test_bot.py`) — seuls 2 échecs préexistants et sans rapport avec ce changement (confirmés identiques avant/après via `git stash`, bug de casse dans `_build_city_display_names`/tests plus anciens, hors périmètre de cette tâche).
- **Non testé en conditions réelles** : aucun accès réseau Facebook/Kijiji depuis ce sandbox de dev (comme pour la branche `claude/scraper-stealth-mutualization`, toujours en attente) — à valider par l'utilisateur après fusion, en particulier le volume réel de requêtes Facebook par cycle et l'absence de régression sur les annonces trouvées.

[2026-08-24] [PRO] Validation empirique : couverture de 2 ancrages à rayon large vs boucle 22 villes → Résultat : 99.7% de couverture, piste confirmée mais pas encore implémentée.
- **Contexte** : discussion avec l'utilisateur sur le volume de requêtes Facebook (22 recherches par cycle, une par ville configurée) — confronté à Fable (recherche web incluse) sur la piste "rayon/géométrie". Conclusion Fable : pas de paramètre d'URL rayon/lat-lng fiable côté Facebook (vérifié sur des scrapers open-source réels), mais une recherche `/marketplace/{city_id}/search` applique déjà un rayon implicite non documenté (~40-65km) — donc 1-3 points d'ancrage suffiraient probablement à couvrir la zone actuelle des 22 villes, avec un filtre géométrique côté client (déjà existant, `scanConfig.distance`) pour exclure ce qui déborde (ex: le nord).
- **`backend/scripts/audit_anchor_coverage.py`** (nouveau, lecture seule) : AUCUNE requête Facebook — vérifie sur `deals_index` (latitude/longitude déjà indexées, `la`/`lo`) quelle proportion des annonces déjà trouvées par la boucle actuelle tombe dans un rayon donné (40/65/80km) autour de 2 ancrages candidats (Longueuil, Saint-Bruno-de-Montarville). Limite assumée et documentée dans le script : symétrique à un angle mort de la boucle actuelle elle-même (une zone jamais bien couverte resterait invisible ici aussi) — un point de départ, pas une preuve absolue.
- **Armé via `run_once.py`**, exécuté en production le 2026-08-24. **Résultat** : à 80km, les 2 ancrages combinés couvrent 99.7% des annonces Facebook déjà indexées ; 3 annonces au total restent hors des deux ancrages même à ce rayon maximum testé (dont au moins un cas plausible d'homonymie "Beloeil" Québec/Wallonie, déjà connu par ailleurs — voir `cities.py::regions_conflict()`). Conclusion : la piste des points d'ancrage (au lieu de 22 recherches séparées) est confirmée viable, soit une réduction potentielle d'environ 91% du volume de requêtes Facebook par cycle. `run_once.py` repassé à `ACTIVE=False` après lecture des logs.
- **Discussion de généralisation (avant toute implémentation, demandée explicitement par l'utilisateur)** : le nombre d'ancrages (2) est un résultat de la géométrie actuelle des 22 villes de Montérégie, pas une règle figée — à recalculer si la liste change. Algorithme proposé : couverture d'ensembles gloutonne (sélectionner itérativement la ville non couverte qui couvre le plus d'autres villes non couvertes dans le rayon, jusqu'à couverture totale) — le nombre d'ancrages émerge naturellement de la géométrie réelle. Un cluster distant (ex: Québec + banlieue, ou Paris) formerait automatiquement son propre ancrage séparé, sans toucher aux 2 ancrages Montérégie existants (Paris nécessiterait en plus un `city_id` Facebook différent, hors périmètre de l'algorithme).
- **Fiabilisation du rayon (au lieu de 3 valeurs fixes devinées)** : signal direct déjà produit en production et jamais exploité — `_run_facebook_scan()` (`bot.py:560-577`) logue déjà chaque annonce trouvée pendant la recherche d'une ville X mais localisée par l'annonce elle-même dans une autre ville Y de la liste (`matched_other_city`). Chaque occurrence prouve directement que le rayon implicite de Facebook autour de X est au moins égal à distance(X, Y) — contrairement au test de couverture (indirect, borné par ce que la boucle actuelle sait déjà trouver), c'est une mesure venant de Facebook lui-même. Piste retenue pour une future itération : consigner la distance de chaque match croisé (calcul quasi gratuit, coordonnées déjà en mémoire) et suivre le max observé dans le temps comme borne inférieure fiable et convergente, combiné à un balayage de rayon élargi (20→150km) sur `audit_anchor_coverage.py` pour repérer le plateau de saturation (borne haute).
- **Généralisation aux autres scrapers** : séparer l'algorithme de clustering (pur, site-agnostique — ville+coordonnées+rayon en entrée) de sa consommation par plateforme. Kijiji (`build_search_url()`) accepte déjà un `radius_km` explicite et réellement respecté par le site — pas d'estimation empirique nécessaire, la couverture y serait garantie par construction (plus fiable que côté Facebook). LeBonCoin (`scraping_leboncoin/core.py::build_url()`) utilise actuellement `locations=` en noms de villes sans paramètre géo confirmé dans le pipeline — à vérifier avant de décider s'il rejoint le modèle "rayon explicite" (Kijiji) ou "rayon implicite à estimer" (Facebook).
- **Non implémenté à ce stade** : aucun changement de `bot.py`/`scan_marketplace()` — l'utilisateur a demandé une explication avant tout code, pas encore de "go" sur l'implémentation.

[2026-08-24] [PRO] Ordre de scan Facebook mélangé + fréquence modulée (mesures anti-bot gratuites) → Résultat :
- **Demande utilisateur** : suite de la stratégie anti-bot — le backoff (pause de plusieurs heures après un blocage) a été explicitement refusé (une pépite peut disparaître en moins d'une heure, la réactivité prime). Deux questions factuelles à la place : l'ordre de scan des villes et la fréquence sont-ils déjà randomisés ? Réponse : non aux deux — corrigé.
- **`backend/bot.py::_run_facebook_scan()`** : les villes étaient scannées dans l'ordre fixe renvoyé par `get_cities()`, cycle après cycle — un signal comportemental robotique en soi. Mélangées via une **copie locale** (`random.shuffle` sur `list(cities_to_scan)`, jamais en place) — `cities_to_scan` est le même objet partagé avec `_run_kijiji_scan()`, exécuté en parallèle dans un autre thread (`_run_sources_in_parallel`), le muter en place aurait fait courir les deux threads sur une liste modifiée sous leurs pieds.
- **`backend/services.py::TaskScheduler`** : la fréquence de scan était un intervalle strictement constant (`schedule.every(N).minutes`, jamais dévié). `schedule` supporte nativement un intervalle aléatoire (`every(a).to(b).minutes`, re-tiré après chaque exécution) — nouvelle fonction `_jittered_bounds()`, ±20% autour de la fréquence configurée (`SCAN_FREQUENCY_JITTER_RATIO`), appliquée à `_setup_schedules()` et `update_scan_frequency()`.
- **Portée volontairement limitée à Facebook** (source du blocage constaté) — Kijiji non touché.
- **Tests** : `backend/test_bot.py` — 28/30 passent, 2 échecs préexistants et sans rapport (normalisation de ville Kijiji, `nearest_configured_city`), confirmés identiques avec et sans ce diff (`git stash`). Logique de `_jittered_bounds()` testée isolément. Non testable en conditions réelles (pas d'accès Facebook depuis cet environnement) — à valider par l'utilisateur (le blocage anti-bot est intermittent, un effet ne sera visible que sur plusieurs cycles).
- **Commits** : à venir (poussés sur `dev` avec cette entrée).

[2026-08-24] [PRO] `PROXIES` configurable via `.env` + remontée de la dette technique anti-bot → Résultat :
- **Demande utilisateur** : suite au diagnostic de blocage anti-bot (entrée précédente), stratégie en 2 temps — commencer par la rotation de proxy ("puisque c'est prêt"), remonter le reste (mutualisation scrapers, cadence humaine) en priorité plus haute dans le backlog plutôt que de le traiter maintenant.
- **`PROXIES` n'était en réalité pas prêt** : codé en dur dans `config.py` (fichier committé), contrairement à tous les autres secrets du projet (`GEMINI_API_KEY`, `FIREBASE_*`, SMTP...) qui passent par `.env`/le secret GitHub `DOT_ENV` — une vraie URL de proxy embarque souvent des identifiants (`user:password@host`), qui n'ont pas leur place dans l'historique git. Corrigé : `PROXIES` lu depuis `os.getenv("PROXIES")` (entrées séparées par des virgules), documenté dans `.env.example`. Logique de parsing testée isolément (`.env` absent → `[]`, comportement inchangé). Aucun proxy configuré à ce jour — reste à choisir un fournisseur (résidentiel recommandé) et renseigner la variable dans `DOT_ENV`.
- **`TODO.md`** : "Dette technique : base commune scraper LeBonCoin/Facebook (mesures anti-bot)" et "Cadence de scan calquée sur un rythme humain" remontées de la section Extension LeBonCoin vers `🚨 Priorité Haute`, avec référence au diagnostic du jour.
- **Commits** : à venir (poussés sur `dev` avec cette entrée).
...
- **Diagnostic (limité)** : "Failed to fetch (AI/error)" est le code générique du SDK Firebase AI pour un échec du `fetch()` lui-même, avant toute réponse HTTP (pas un 429/quota, qui aurait un code différent) — plus caractéristique d'un problème réseau transitoire (blip, bloqueur de pub/extension bloquant `googleapis.com`, DNS/TLS) que d'une instabilité du modèle Preview utilisé par le chat (`gemini-3.1-pro-preview`, sans fallback). Non tranché faute d'accès à une session réelle ou à des logs historisés depuis cet environnement (le chat tourne 100% côté client, rien n'était journalisé au-delà de `console.error`, perdu à la fermeture de l'onglet). Mitigation : le détail technique brut (`e.message`) est désormais inclus dans le texte de la bulle d'erreur — combiné au bouton Copier, une prochaine occurrence devient auto-diagnostiquable.
- **Réessayer (`useDealChat.js`)** : extraction du cœur d'un tour de chat (appel Gemini + cascade functionCalls/rappel de photo/propositions de restauration + persistance) dans `executeTurn()`, partagé entre l'envoi normal et le nouveau `retryMessage()` — sans ce partage, la cascade complète (dont le mécanisme de rappel de photo du 2026-08-23) aurait dû être dupliquée. `lastFailedTurnRef` garde les vraies `parts` (photos incluses) du dernier échec — non reconstructibles depuis Firestore pour le premier message d'une conversation (photos de l'annonce remplacées par un placeholder texte). Le succès/nouvel échec d'un retry REMPLACE le placeholder d'erreur en place (`firestoreService.js::replaceDealChatMessage`, nouveau) plutôt que d'ajouter un message, pour préserver l'alternance stricte user/model qu'exige l'historique rejoué. Le bouton n'est affiché que sur la toute dernière bulle si elle est en erreur (un ancien échec n'est plus rejouable).
- **Copier (`DealChatPanel.jsx`)** : bouton sur chaque bulle avec du texte, `navigator.clipboard.writeText()`, retour visuel "Copié ✓" 1.5s.
- **Build (`npm run build`) vérifié après chaque étape.** Non testé en conditions réelles (pas d'accès Gemini/Firebase depuis cet environnement) — à valider par l'utilisateur.
- **Commits** : à venir (poussés sur `dev` avec cette entrée).

[2026-08-24] [PRO] Recherche par ID d'annonce + correctif tri Facebook (annonce jamais scannée) → Résultat :
- **Recherche par ID (`src/hooks/useDealsManager.js::matchesTypeFilter`)** : demande initiale mal comprise ("ID de vendeur"), clarifiée par l'utilisateur en "numéro de l'annonce dans l'URL" — déjà l'identifiant primaire du document Firestore (`deal_id`). Coller l'ID seul ou l'URL complète dans la barre de recherche du Dashboard fait désormais remonter l'annonce (match exact sur `deal.id`, en plus — pas à la place — de la recherche texte existante) ; aucun résultat = absente de la base, sans UI supplémentaire.
- **Script de diagnostic (`backend/scripts/find_deal_by_id.py`, nouveau)** : sur le modèle des scripts d'audit existants (`audit_classifications.py`) — itère tous les utilisateurs, lookup direct `guitar_deals/{id}` (avec/sans préfixe `kijiji_`), armable via `run_once.py` depuis cet environnement de dev (aucun accès Firestore direct ici).
- **Bug réel trouvé en creusant le second usage** : l'utilisateur a signalé qu'une annonce qu'il venait de publier (Saint-Bruno-de-Montarville, 295$, ville et prix conformes à sa config) n'était pas détectée après 8h. Diagnostic par lecture de code (WebFetch et Playwright headless bloqués depuis cet environnement de dev — `net::ERR_CONNECTION_RESET`, probable incompatibilité HTTP/2 du proxy réseau du sandbox avec Facebook, sans rapport avec le bot en production) : `scan_marketplace()` (`backend/scraping/core.py`) ne passait aucun paramètre `sortBy` dans l'URL de recherche — Facebook trie par défaut par "Pertinence", pas par date, et seuls les `max_ads` premiers résultats (défaut 5) sont scrapés. Une annonce neuve peut donc ne jamais entrer dans la fenêtre scrapée, quel que soit le nombre de cycles écoulés. Kijiji n'a pas ce problème (tri par date déjà natif). Confirmé par l'utilisateur : oubli lors du portage de la logique Kijiji vers Facebook.
- **Correctif** : ajout de `&sortBy=creation_time_descend` à l'URL de recherche Facebook. Non testé en conditions réelles (pas d'accès Facebook depuis l'environnement de dev) — à valider par l'utilisateur au prochain cycle de scan en production.
- **Commits** : `8c30fa8` (recherche ID + script), `6e2c9f7` (correctif tri), poussés sur `claude/protocole-demarrage-a3euw5`.

[2026-08-23] [PRO] Plan 2 — Persona IA "luthier/vendeur référent" → Résultat :
- **Demande utilisateur** : suite du Plan 1 (tokens), exécuter le Plan 2 déjà cadré dans `docs/management/plans/TOKEN_OPTIMIZATION_PLAN.md`. Question de clarification avant validation : le persona s'applique-t-il uniquement via les boutons de prompts prédéfinis de la modale restauration, ou aussi en conversation libre ? Réponse : le persona est porté par la session Gemini elle-même (system instruction, gaté sur `deal.isPurchased`), donc actif pour TOUT message sur une annonce achetée, bouton ou saisie libre — confirmé et validé par l'utilisateur avant implémentation.
- **Persona** : `RESTORATION_SYSTEM_INSTRUCTION_ADDENDUM` (`geminiChatService.js`) étendu d'un nouveau bloc `RESTORATION_PERSONA_ADDENDUM` — identité double restaurateur ET vendeur référent, avis tranchés, attention à l'ordre de faisabilité des étapes (signaler quand une étape risque d'en abîmer une autre), marge de revente gardée en tête sans qu'on ait besoin de la redemander. Proactivité encadrée : une suggestion courte de prochaine étape en fin de réponse quand pertinent, jamais un tour dédié. Les instructions function-calling existantes (ajout/réordonnancement d'étape) sont conservées telles quelles, concaténées après le bloc persona.
- **Bouton "Conseil d'atelier sur l'étape en cours"** (`RestorationPlanPanel.jsx`), aux côtés de "Faire le point"/"Préparer l'annonce de revente" : cible l'item `in_progress` en priorité, sinon le prochain `pending`/`waiting` dans l'ordre affiché ; masqué si le plan est vide ou entièrement terminé/ignoré.
- **1 bug trouvé en revue de code et corrigé** : le bouton et le prompt affirmaient systématiquement "en cours" même pour l'étape de repli `pending`/`waiting` (jamais commencée ou bloquée), faussant la prémisse donnée au persona (instruit de trancher avec assurance, risquait de répondre "continue ce que tu fais" à une étape jamais commencée). Corrigé par un dictionnaire de formulations par statut (`ATELIER_STEP_PHRASING`), reflété à la fois dans le libellé du bouton et le texte du prompt envoyé.
- **Build (`npm run build`) vérifié après chaque étape.** Non testable en conditions réelles depuis cet environnement (pas d'accès Gemini/Firebase) — à valider par l'utilisateur.
- **Commits** : `0bb9781` (persona + bouton), `8560e43` (correctif de revue), poussés sur `dev` puis synchronisés sur `claude/gemini-startup-protocol-8fvqvw`.

[2026-08-23] [PRO] Plan 1 — Optimisation tokens du chat Gemini (concision, élision photos, rappel ciblé) → Résultat :
- **Demande utilisateur** : réduire la consommation de tokens du chat. Un premier correctif isolé (consigne de concision) a été mis en attente au profit d'une réflexion stratégique complète — l'utilisateur a demandé une revue avec Fable/Opus plutôt qu'un patch ponctuel, en incluant deux idées produit plus larges (persona IA "luthier/vendeur référent", pages narratives "histoire de la guitare"). Stratégie complète consignée dans `docs/management/plans/TOKEN_OPTIMIZATION_PLAN.md` (Plan 1 tokens exécuté maintenant ; Plan 2 persona et Plan 3 histoire en backlog, `TODO.md`).
- **Revue Fable (stratégie)** : poste de coût dominant identifié en lisant le code réel — les photos jointes sont rejouées intégralement à l'API à CHAQUE tour (`sanitizeHistory` rejoue les `parts` Firestore, `inlineData` compris).
- **Revue Opus (faisabilité technique du mécanisme d'élision/rappel)**, avant tout plan détaillé — verdict central tranché par lecture du SDK `@firebase/ai` installé : mélanger une part `functionResponse` et des parts `inlineData` dans un même `chat.sendMessage()` est **impossible** (le SDK lève `AIError(INVALID_CONTENT)` avant tout appel réseau), et ce rejet tombe dans le `catch` existant qui désactiverait silencieusement les tools pour toute la session — piège évité en écartant cette approche dès la conception. Alternative retenue : reconstruire une session fraîche depuis l'historique d'avant le tour et rejouer le tour utilisateur augmenté des photos demandées (réutilise le mécanisme déjà en prod du repli "modèle sans support function calling"). Cette revue a aussi trouvé un risque préexistant (limite Firestore 1 Mo/document probablement déjà frôlée par le premier message d'une annonce à beaucoup de photos) et un bug préexistant (`toolsUnsupportedRef` collant sur n'importe quelle erreur, pas seulement un rejet de function calling) — les deux corrigés dans ce lot plutôt que différés.
- **Lot A** : consigne de concision par défaut dans `SYSTEM_INSTRUCTION` (détails seulement sur demande explicite) + `generationConfig.maxOutputTokens` en filet de sécurité.
- **Lot B** : les photos de l'annonce ne sont plus jamais persistées en base64 dans Firestore (reconstructibles depuis `storageImageUrls`) — corrige le risque de dépassement de la limite 1 Mo/document ET réduit le coût. `toolsUnsupportedRef` ne se désactive plus durablement que sur une erreur diagnostiquée comme un vrai rejet de function calling (heuristique sur le texte d'erreur), pas sur une erreur réseau transitoire. Instrumentation temporaire (`console.log` de `usageMetadata`) pour mesurer le vrai gain plutôt que le supposer.
- **Lot C** : refs courts et stables par photo (`buildPhotoRefIndex`, `geminiChatService.js`) — `d-<hash>` pour une photo d'annonce (hash du pathname de l'URL Storage, jamais un index de position qui résoudrait silencieusement vers la mauvaise photo), `c-<messageId>-<partIndex>` pour une photo jointe en cours de conversation. Élision non-mutante des vieilles photos jointes dans l'historique rejoué (`elideOldChatPhotos`, budget 6 images, granularité tour entier, jamais le tour le plus récent) — active uniquement quand le rappel de photo est réellement disponible côté modèle.
- **Lot D** : fonction `request_photo_review` (indépendante de `isPurchased`, contrairement aux fonctions du plan de restauration) — Gemini peut redemander explicitement à revoir des photos précises, jamais toutes d'un coup. Rejeu sur session fraîche SANS le tool de rappel (empêche structurellement un 2ᵉ rappel dans le même tour). Plafond de 3 photos rappelées/tour, refs hallucinés jamais avalés silencieusement (distingués proprement des refs simplement plafonnés dans la note renvoyée au modèle). Badge "🔍 Photo réexaminée" sous la bulle concernée.
- **4 bugs trouvés et corrigés lors d'une passe `/code-review` locale** avant validation : follow-up d'un rappel de photo invalide qui ignorait silencieusement de nouveaux appels de fonction du modèle (proposition perdue) ; boucle de retry sur collision de ref photo inopérante au-delà de la première tentative (un hash de longueur fixe tronqué ne peut pas être "allongé" par slicing) — corrigée par génération de blocs de hash salés ; `maxOutputTokens` à 1024 tronquait une réponse détaillée légitime que le prompt système invite pourtant explicitement à donner sur demande — remonté à 4096 ; note de rappel de photo mélangeant refs plafonnées et refs introuvables dans une phrase ambiguë — séparées.
- **Build (`npm run build`) vérifié après chaque lot.** Testé et validé par l'utilisateur (non testable en conditions réelles depuis cet environnement — pas d'accès Gemini/Firebase).
- **Commits** : `d0accab` (stratégie + plan détaillé), `6f703ea` (Lots A-D), `1167b79` (correctifs de revue), poussés sur `dev` puis synchronisés sur `claude/gemini-startup-protocol-8fvqvw`.

[2026-08-23] [PRO] Plan de restauration : réordonnancement des étapes proposable par Gemini → Résultat :
- **Demande utilisateur** : après le glisser-déposer manuel (entrée du 2026-08-22), pouvoir laisser Gemini lui-même relire les étapes existantes (ordre/id) et proposer un réordonnancement, dans la continuité du function calling déjà en place pour l'ajout d'étapes.
- **Réévaluation du risque par Opus** (à la demande explicite de l'utilisateur, après un premier refus de principe) : reorder validé comme sûr avec garde-fous stricts, **mise à jour d'un item existant par id toujours exclue du périmètre** (id périmés jamais nettoyables une fois rejoués dans l'historique). Cette revue a aussi mis au jour un bug préexistant indépendant de la demande : `addRestorationItem` n'assignait jamais de champ `order` à la création, ce qui redéclenchait à chaque ajout le rattrapage silencieux (`backfillRestorationOrder`) sur TOUT le plan et effaçait un ordre de glisser-déposer déjà en place. Corrigé en premier, isolément (commit `afeca44`), à la demande explicite de l'utilisateur ("on va d'abord uniquement traiter le bug existant").
- **Conception retenue** : réordonnancement toujours proposé comme un ORDRE COMPLET (jamais un delta/déplacement relatif) — plus simple à valider entièrement contre l'état courant qu'une séquence de mouvements. Chaque étape porte un identifiant court et stable dérivé de son id Firestore (`buildRestorationItemRefs`, `geminiChatService.js`) injecté dans le contexte du plan — jamais l'id complet, jamais un numéro de position (qui change à chaque réordonnancement). La résolution d'une proposition (`resolveRestorationReorderProposal`) est **toujours recalculée contre l'état RÉEL et courant du plan**, à la fois pour l'aperçu affiché dans la carte de proposition ET au moment de l'application — jamais contre un instantané capturé au tour de chat. Un contrôle par horodatage (`updatedAt` d'un item) avait été envisagé puis écarté par la revue : cassé par le décalage horloge client/serveur, et un réordonnancement touche justement `updatedAt` de tous les items concernés.
- **Implémentation** : nouvelle fonction `propose_restoration_reorder` (function calling, aux côtés de `propose_restoration_step`), écriture batchée `reorderRestorationItems` (`firestoreService.js`, un seul `writeBatch`) réutilisée aussi par le glisser-déposer manuel à la place de N écritures individuelles. `useDealChat.js::sendMessage` sépare désormais les appels `propose_restoration_step`/`propose_restoration_reorder` (plafonds distincts : 5 ajouts, 1 réordonnancement par tour affiché), répond toujours à TOUS les appels reçus (contrat API préservé). Carte de proposition dédiée dans `DealChatPanel.jsx` : aperçu ordonné en direct si la proposition résout encore, avertissement "le plan a changé" sinon (bouton Appliquer désactivé) ; une erreur d'application (proposition devenue caduque entretemps) s'affiche désormais dans la carte au lieu d'un échec silencieux (`console.error` seul).
- **Build (`npm run build`) vérifié.** Testé et validé par l'utilisateur.
- **Commits** : `afeca44` (correctif `order` préalable), `780d27c` (réordonnancement par l'IA), poussés sur `dev` puis synchronisés sur `claude/gemini-startup-protocol-8fvqvw`.

[2026-08-22] [PRO] Plan de restauration : réorganisation par glisser-déposer + photos par étape + visionneuse partagée → Résultat :
- **Demande utilisateur** (après validation fonctionnelle du Lot A + Lot B) : pouvoir modifier l'ordre des étapes de restauration et ajouter des photos à chacune.
- **Décisions produit tranchées via AskUserQuestion** : glisser-déposer plutôt que boutons haut/bas (nouvelle dépendance acceptée) ; photos à la fois par upload direct (caméra/galerie du téléphone) ET par sélection dans la galerie déjà existante de l'annonce (les deux, pas l'un ou l'autre).
- **Réorganisation** : `@dnd-kit/core`+`@dnd-kit/sortable`+`@dnd-kit/utilities` (nouvelle dépendance, `react-beautiful-dnd` écarté car non maintenu). Nouveau champ `order` par item, **rattrapé silencieusement** (`backfillRestorationOrder`, un seul `writeBatch`) pour les plans créés avant cette fonctionnalité — jamais de mélange d'items ordonnés/non-ordonnés dans une même liste, jamais de script de migration à lancer manuellement. Poignée de glisser dédiée (pas la carte entière) pour ne pas interférer avec les boutons/champs déjà présents. Correctif appliqué avant push : le mode édition d'une étape retournait un DOM différent du conteneur suivi par `useSortable`, ce qui aurait détaché la ref de mesure pendant un glisser en cours sur un autre item — corrigé en gardant le même conteneur dans les deux modes.
- **Photos par étape** : nouveau champ `photoUrls` (tableau, `arrayUnion`/`arrayRemove`). Upload direct via `storageService.js::uploadRestorationPhotoToDealStorage` (miroir de la fonction équivalente du chat, préfixe `restoration_`) — `storage.rules` étendu (`chat_.*` → `(chat|restoration)_.*`), déployé automatiquement en CI comme depuis le Lot 1 galerie. Sélection depuis la galerie déjà existante de l'annonce via une petite modale de choix (clic direct pour attacher, pas de bouton "confirmer" séparé) — aucun nouvel upload dans ce cas.
- **Visionneuse plein écran partagée (retour utilisateur)** : les photos d'étape ouvraient d'abord un simple lien `target="_blank"` — demande de retrouver le même fonctionnement que la galerie principale de l'annonce (suivant/précédent, bouton fermer). `src/components/ImageGallery.jsx` : extraction de la partie plein écran en composant réutilisable exporté `ImageLightbox` (contrôlé par le parent — `currentIndex`/`onNext`/`onPrev`/`onClose` — plutôt que de gérer son propre index), réutilisé en interne par `ImageGallery` sans aucun changement de comportement pour ses appelants existants (`DealCardImage.jsx`, `SharedDealPage.jsx`). `RestorationPlanPanel.jsx` l'utilise pour les photos d'étape.
- **Swipe tactile (retour utilisateur)** : geste horizontal à un doigt ajouté à `ImageLightbox` (seuil 50px pour ne pas confondre avec un tap sur un bouton) — bénéficie automatiquement à la galerie principale ET aux photos d'étape, un seul endroit à maintenir.
- **Build (`npm run build`) vérifié après chaque étape.** Non testé en conditions réelles (pas d'accès Firebase depuis cet environnement, notamment le comportement tactile — drag-and-drop et swipe — sur un vrai appareil mobile).
- **Commits** : `61859b5` (réorganisation + photos), `955194e` (visionneuse partagée), `6d57ce6` (swipe tactile), poussés sur `dev`.

[2026-08-22] [PRO] Plan de restauration structuré (Lot A + Lot B) → Résultat :
- **Demande utilisateur** : une fois une guitare achetée pour la restaurer avant revente, pouvoir affiner les connaissances via le chat Gemini et bâtir un plan de restauration structuré et évolutif dans le temps.
- **Deux revues critiques dédiées avant tout code**, sur le même principe que le plan Galerie/Requalification : revue de fiabilité (Opus, 18 problèmes trouvés sur le brouillon initial — races d'ids périmés, tours Gemini à texte vide, écritures `undefined`, ambiguïté de la formule de marge) puis, à la demande explicite de l'utilisateur, une revue produit/workflow (Fable) sur l'utilité réelle du workflow pour quelqu'un debout devant sa guitare. 6 propositions Fable toutes retenues (statut `waiting`, coût réel `actualCost`/`completedAt`, bouton "Demander conseil" au Lot A ; contexte enrichi, "Faire le point", "Préparer l'annonce de revente" au Lot B) et une question ouverte tranchée avec l'utilisateur : marge ancrée sur `resale_potential` (valeur *après* restauration) plutôt que `estimated_value` (valeur *en l'état actuel*), qui aurait sous-estimé structurellement la marge. Plan détaillé : `docs/management/plans/RESTORATION_PLAN_PLAN.md`.
- **Lot A (commit `7b450cd`)** : nouvelle sous-collection `guitar_deals/{dealId}/restorationPlan` (jamais un champ sur le deal ni dans `aiAnalysis`, qui sont tous deux écrasés en entier par certains chemins backend). Hook `useRestorationPlan.js` (monté dans `DealAnalysisModal.jsx`, pas le panneau — la puce de résumé en a besoin même panneau fermé), `RestorationPlanPanel.jsx` (checklist, statuts `pending`/`waiting`/`in_progress`/`done`/`skipped`, coût estimé + réel, marge nette projetée). Point d'entrée : puce "Restauration X/Y" dans le bloc "Achetée" déjà existant plutôt qu'un 5ᵉ bouton dans le header (aurait recréé l'encombrement corrigé le même jour, voir entrée suivante). `showChat` généralisé en `activeView` (`'analysis'|'chat'|'restoration'`), vues gardées montées pour préserver le brouillon de chat.
- **Lot B (commit `5d3ee29`)** : function calling Gemini, `propose_restoration_step` (ajout d'étape uniquement — la mise à jour d'un item existant par id a été explicitement retirée du périmètre par la revue Opus, id périmés jamais nettoyables une fois rejoués dans l'historique). Carte Appliquer/Ignorer sous la bulle modèle, état persisté sur le message. `tools` gaté sur `deal.isPurchased`, jamais de tour function-call sans texte persisté, repli sans `tools` si le modèle configuré ne le supporte pas. Contexte du plan (statuts/coûts/totaux) injecté à chaque tour. Boutons "Faire le point"/"Préparer l'annonce de revente" (prompts prédéfinis, aucune infra supplémentaire).
- **2 passes `/code-review` locales après implémentation (commit `692f89d`)** — 9 bugs trouvés et corrigés :
  - `activeView` non réinitialisé si "Acheté" est décoché pendant que le panneau de restauration est ouvert → corps de modale vide (aucune des 3 vues ne correspondait plus).
  - `DealChatPanel` monté systématiquement à l'ouverture de la modale (régression vs. le montage à la demande d'avant cette fonctionnalité) → listener Firestore + session Gemini ouverts pour chaque annonce consultée, même sans jamais ouvrir le chat. Corrigé en montage paresseux (`hasOpenedChat`), gardé monté une fois ouvert pour préserver le brouillon en jonglant entre vues.
  - `functionCalls()` tronqué à 5 **avant** de construire les `functionResponse` → un tour à plus de 5 appels envoyait moins de réponses que d'appels, violant le contrat de l'API. Corrigé : répond à TOUS les appels, seul l'affichage des propositions reste plafonné à 5.
  - `completedAt` posé au passage à `done` mais jamais effacé en ressortant de ce statut — corrigé (`deleteField()` symétrique).
  - `remainingCost` calculé dans `useRestorationPlan.js` mais jamais affiché — ajouté une puce "Reste à payer" au lieu de le laisser mort.
  - `addRestorationItem` ne retournait pas l'id du document créé → `markChatMessageRestorationProposalStatus` était appelé sans `itemId`, le lien proposition→étape restait vide malgré le champ prévu pour. Corrigé, id retourné et transmis.
  - Session Gemini reconstruite à **chaque message** sur une annonce achetée au lieu de seulement au changement réel du statut Acheté (le listener Firestore réinitialisait un flag à chaque snapshot, forçant `sendMessage` à toujours détecter un "changement"). Corrigé via un `dealRef` toujours à jour lu par le listener, qui construit désormais la session avec le bon statut dès le départ.
  - Prompt rapide ("Faire le point"/"Préparer l'annonce") arrivant pendant un envoi déjà en cours était consommé et perdu silencieusement (le `sendMessage` interne no-op sur `sending`, mais `onDraftConsumed()` s'exécutait quand même) — corrigé, retenté dès la fin de l'envoi en cours.
  - Commentaire trompeur sur le retour de `addDealChatMessage` (prétendait être consommé par un appelant, ne l'était par aucun) — corrigé.
- **Build (`npm run build`) vérifié après chaque étape.** Non testé en conditions réelles (pas d'accès Gemini/Firebase depuis cet environnement) — à valider par l'utilisateur.
- **Commits** : `7b450cd` (Lot A), `5d3ee29` (Lot B), `692f89d` (correctifs de revue), poussés sur `dev`.

[2026-08-22] [PRO] Modale rapport IA : barre d'actions scindée en 2 groupes → Résultat :
- **Demande utilisateur** : dans la modale "Rapport d'Expertise IA", trop de boutons sur une seule ligne (`DealCardActions.jsx` complet : Favori, Acheté, Ré-analyser, Rejeter, Supprimer, Partager, Discuter avec Gemini, Voir l'annonce d'origine). Proposition validée : remonter Partager/Discuter avec Gemini/lien FB-Kijiji au niveau du titre (alignés à droite), garder le reste sur une deuxième ligne. Clôt un point resté ouvert dans `TODO.md` depuis 2026-07-31.
- **Implémentation** : `DealCardActions.jsx` scindé en deux composants exportés — `ManagementActions` (Favori, Acheté, Ré-analyser + ses 2 modales, Rejeter, Supprimer) et `ShareActions` (Partager, Discuter avec Gemini, lien FB/Kijiji). `DealAnalysisModal.jsx` affiche `ShareActions` sur la ligne du titre, `ManagementActions` sur une ligne dédiée en dessous. L'export par défaut `DealCardActions` (les deux groupes assemblés, comme avant) est conservé pour la carte liste (`DealCard/index.jsx`), inchangée visuellement.
- **Incident git en cours de route** : le stop-hook a demandé un commit/push alors que le travail était sur `dev` au lieu de la branche désignée `claude/gemini-startup-protocol-8fvqvw`. Cette dernière s'est révélée en retard sur `dev` (fonctionnalité "Acheté"/`isPurchased` absente) — un `git stash pop` sur cette branche plus ancienne a silencieusement fait disparaître le code lié à `isPurchased` (comportement standard d'un merge à 3 avec des branches divergentes, pas un bug d'outil). Détecté avant tout commit (fichier de sortie relu), corrigé en repartant du contenu `dev` (`git show dev:<fichier>`) puis en réappliquant le même découpage par-dessus, plutôt que de pousser un état incomplet.
- **Build (`npm run build`) vérifié après résolution.** Non testé en conditions réelles — à valider par l'utilisateur.
- **Commit** : `5372640`, poussé sur `claude/gemini-startup-protocol-8fvqvw`.

[2026-08-22] [PRO] Chat Gemini : photos multiples + envoi via bouton uniquement, 2 bugs corrigés en revue → Résultat :
- **Demande utilisateur** : dans le chat "Discuter avec Gemini", (1) pouvoir joindre plusieurs photos en un seul message au lieu d'une seule, (2) n'envoyer le message QUE via le bouton flèche, jamais avec la touche Entrée — pour supporter une saisie multilingue (IME) où Entrée valide un caractère composé.
- **Implémentation** (commit `34e0e33`) : `attachedImagePartIndex`/`addedToGalleryUrl` (champs singuliers) → `attachedImagePartIndices`/`addedToGalleryUrls` (tableau / map par index de photo, voir `DATA_FLOW.md` §5.1) ; sélecteur galerie en `multiple`, plafonné à 6 photos/message ; chaque photo d'un message a son propre bouton "Ajouter à la galerie". `handleKeyDown` (Entrée → envoi) supprimé du textarea. Rétrocompatibilité d'affichage pour les messages déjà en base (anciens champs singuliers).
- **Revue de code avant push (8 agents dédiés, angles complémentaires)** — 2 vrais bugs trouvés et corrigés (commit `eee66e6`) :
  - **Cleanup des URL de prévisualisation cassé par le passage au tableau** : l'effet `useEffect(() => { return () => imagePreviews.forEach(URL.revokeObjectURL); }, [imagePreviews])` révoquait tout le tableau précédent à chaque ajout/retrait d'une photo (pas seulement au démontage comme en mode single-image) — ajouter une 2e photo cassait la miniature de la 1ère. Corrigé en cleanup au démontage uniquement (`useRef` pour garder la dernière valeur sans redéclencher l'effet), les révocations ponctuelles restant explicites dans les handlers d'ajout/retrait/envoi.
  - **`filesToInlineParts` (`geminiChatService.js`) faisait échouer tout l'envoi sur une seule photo corrompue** (`Promise.all` sans garde par photo) — rendu résilient par photo, même principe que `buildDealImageParts` déjà en place.
  - **Repli vers l'ancien champ singulier oublié dans les gardes anti-double-ajout** (`handleAddToGallery` dans `DealChatPanel.jsx`, `addPhotoToGallery` dans `useDealChat.js`) : ces deux fonctions ne vérifiaient que le nouveau champ pluriel `addedToGalleryUrls`, pas l'ancien `addedToGalleryUrl` — risque de double upload/doublon en galerie sur un message pré-migration (masqué en pratique par le bouton visuellement désactivé, mais la garde elle-même était fausse). Centralisé dans deux helpers partagés (`getAttachedImagePartIndices`, `getAddedToGalleryUrl`, `useDealChat.js`), réutilisés par l'affichage ET les deux gardes.
  - **Le retrait total de l'envoi par Entrée a été signalé par plusieurs agents comme "régression pour les utilisateurs non-IME"** (suggérant un fix ciblé via `isComposing`) — **non retenu** : c'est exactement la demande explicite de l'utilisateur ("n'envoyer le message QUE si on clique sur la flèche... pas sur entrée"), pas un effet de bord à corriger. Un fix `isComposing` aurait réintroduit l'envoi par Entrée pour tout le monde sauf les utilisateurs IME, contredisant la demande.
- **Build (`npm run build`) vérifié après chaque étape** (implémentation, puis correctifs). **Non testé en conditions réelles** — à valider par l'utilisateur.
- **Commits** : `34e0e33` (fonctionnalité), `eee66e6` (correctifs de revue), poussés sur `claude/gemini-startup-protocol-8fvqvw`.

[2026-08-21] [PRO] Déploiement automatique des règles Firebase Storage en CI → Résultat :
- **Demande utilisateur** : après le rappel "déploiement manuel requis" laissé par le Lot 1 (chat Gemini → galerie), l'utilisateur a demandé si le Dell (machine GPU du projet neck-reset, accessible en SSH/Tailscale depuis la CI) pouvait exécuter le déploiement à sa place.
- **Le Dell écarté** : machine Python/PyTorch pour l'inférence vision, sans Node.js/Firebase CLI — aurait fallu tout installer pour rien. Piste plus directe trouvée en lisant `deploy.yml` : le job `deploy-frontend` a déjà Node 20 configuré pour `npm run build`, et le secret `FIREBASE_SERVICE_ACCOUNT_KEY` existe déjà (utilisé côté backend Admin SDK) — réutilisable comme `GOOGLE_APPLICATION_CREDENTIALS` pour authentifier `firebase-tools` sans rien installer de plus.
- **Ajout d'un step "Déployer les règles Firebase Storage"** (`npx firebase-tools@latest deploy --only storage --force`) après `Build`, `continue-on-error: true` pour ne jamais bloquer la publication du site (inconnue explicitement assumée avant le run : le SDK Admin contourne les règles et n'avait jamais eu besoin du rôle IAM "déployer des règles").
- **Premier run réel confirmé (run GitHub Actions #350)** : `✔ storage: released rules storage.rules to firebase.storage` — le compte de service a bien la permission nécessaire, aucune action manuelle supplémentaire à faire côté console GCP. Les règles du Lot 1 (`deals/{dealId}/{fileName}` : lecture publique, création authentifiée limitée au préfixe `chat_`/5 Mo/type image) sont donc **en production**.
- **2 commits bloqués par le classificateur de sécurité du mode auto** (modification de `.github/workflows/deploy.yml`, catégorie sensible même après validation explicite de l'utilisateur) — retentés avec succès après confirmation utilisateur, sans contournement.
- Reste à faire pour clore le Lot 1 : validation fonctionnelle réelle (upload photo → galerie) par l'utilisateur, seule étape non automatisable depuis cet environnement.

[2026-08-21] [PRO] Lot 1 implémenté : ajout d'une photo du chat Gemini à la galerie de l'annonce → Résultat :
- **Suite du plan validé ci-dessous** — premier des 2 lots séquencés, `npm run build` OK.
- **Premier upload Storage côté client du projet** (`src/services/storageService.js`) : tout l'upload existant était jusqu'ici backend Python (`backend/repository.py`). `base64ToBlob()` décode l'`inlineData` du message de chat, `uploadChatPhotoToDealStorage()` uploade vers `deals/{dealId}/chat_{timestamp}_{uuid}.jpg` et résout l'URL via `getDownloadURL()` (jamais une URL `storage.googleapis.com` construite à la main — un upload SDK Web ne pose pas l'ACL `publicRead` que `make_public()` pose côté backend, l'image serait en 403 partout).
- **`storage.rules`** (`allow read, write: if false` global jusqu'ici) : ouvert sur `deals/{dealId}/{fileName}` en lecture publique, **création** authentifiée uniquement (préfixe `chat_`, < 5 Mo, type image), `update`/`delete` refusés côté client. Cloisonnement par utilisateur non applicable (`dealId` = ID de l'annonce marketplace, pas propre à un utilisateur) — risque accepté, documenté dans le fichier de règles. **Déploiement manuel requis** (`firebase deploy --only storage`), confirmé non couvert par `deploy.yml` (déploie uniquement frontend + backend Python, jamais les règles Firebase).
- **UI** : bouton "Ajouter à la galerie" sur les bulles de chat portant une photo jointe par l'utilisateur (jamais sur les photos de contexte du 1er message). États idle → upload → "Ajoutée ✓" (persisté via `addedToGalleryUrl` sur le message, garde anti-double-clic). `useDealChat.js::addPhotoToGallery` orchestre upload + `arrayUnion` sur `storageImageUrls` + marquage du message. Mise à jour optimiste (`useDealsManager.js::handleGalleryImageAdded`, même pattern que `handleSetClassification`) car le doc complet de l'annonce n'est pas un listener temps réel.
- **Limite assumée, non corrigée dans ce lot** : une photo ajoutée via le chat n'est jamais vue par une ré-analyse ultérieure (`analyze_single_deal` construit son contexte image depuis les `imageUrls` Facebook d'origine, jamais `storageImageUrls`).
- **Non testé en conditions réelles** (pas de credentials Firebase dans cet environnement, règles Storage pas encore déployées) — à valider par l'utilisateur avant le Lot 2 (requalification via function calling Gemini).
- **Commits** : documentation du plan (`a10fce7`), implémentation Lot 1 (`9324f0b`), poussés sur `claude/gemini-startup-protocol-8fvqvw`.

[2026-08-21] [PRO] Plan validé (galerie via chat + requalification d'annonce), après confrontation Opus → Résultat :
- **Demande utilisateur** : permettre depuis le chat "Discuter avec Gemini" existant sur une annonce (1) d'ajouter une photo envoyée dans le chat à la galerie de l'annonce, (2) de laisser Gemini requalifier le contenu de l'annonce (verdict/scores/specs) suite à la conversation.
- **Plan initial** (champ dédié `chatRequalification` + mirroring dans l'index léger `deals_index/{chunk}.deals.{dealId}.cq.*` + helper `getEffectiveAnalysis(deal)` sur ~7 sites de lecture) soumis à une confrontation critique (agent Opus dédié) avant tout code, conformément au protocole projet.
- **3 problèmes bloquants trouvés par la revue** : `StatsView.jsx` faussement déclaré "sans changement nécessaire" (il fusionne en fait `loadedDeals`, aurait écrasé la requalification dès qu'une annonce est ouverte — bug intermittent dépendant du cache de session) ; `interestScore` jamais recalculé après requalification (deux scores différents affichés au même endroit) ; le correctif envisagé pour l'alias `analysis || reasoning` aurait en réalité écrasé le rapport d'expertise Tier 3 complet par une phrase de chat (`analysis` et `reasoning` sont deux champs distincts, pas un alias). Également signalé : aucune validation de plage/énumération avant "Appliquer" (un verdict halluciné aurait fait basculer l'annonce dans l'onglet "Erreurs" sans marche arrière) et des règles Storage proposées trop laxistes (`write: if auth != null` sur un chemin non cloisonné par utilisateur).
- **Architecture révisée avec l'utilisateur** : abandon du champ dédié au profit de la réutilisation du pipeline `ANALYZE_DEAL` existant — "Appliquer" une proposition de Gemini formate la correction en `userComment` et déclenche `retryDealAnalysis` (chemin déjà en production), au lieu d'une écriture Firestore littérale côté client. Élimine les 3 bugs ci-dessus par construction (zéro nouveau champ, zéro site de lecture à corriger).
- **Livraison scindée en 2 lots séquencés** (décision utilisateur, suivant la recommandation de la revue) : Lot 1 = galerie (upload Storage client, nouveau — aucun précédent dans `src/`), Lot 2 = requalification (function calling Gemini), avec validation entre les deux plutôt qu'un bloc de 13 fichiers.
- **Plan détaillé** : `docs/management/plans/CHAT_GALLERY_REQUALIFICATION_PLAN.md`. Implémentation non démarrée à ce stade (commit de documentation demandé par l'utilisateur avant de coder).
...
- **Bug auto-détecté et corrigé avant de communiquer un chiffre** : le filtre `is_guitar()` matchait par sous-chaîne (`"electrique" in classification`), incluant à tort 10 annonces `etui_housse.Etui_Rigide.Guitare Electrique` (des étuis, pas des guitares) — repéré en relisant la répartition par classification du premier run, corrigé en exigeant un préfixe complet `guitare.*`, re-exécuté.
- **Résultat final (chiffres corrigés, re-vérifiés) : 1066 annonces retenues → 5974 photos**, tout le périmètre guitare (acoustique, classique, électro-acoustique, électrique, basse). Exclues : 2423 rejetées, 143 sans photo, 1004 hors périmètre. Détail complet et répartition par classification dans `NECK_RESET_VISION_PLAN.md` §3septies.
- **Limite explicite** : ce chiffre est le corpus **brut** (Dataset A), pas le taux d'**exploitabilité** — celui mesuré en §3sexies (51%/70%) ne porte que sur un échantillon acoustique/classique de 47 annonces, pas encore vérifié sur le corpus électrique élargi.

[2026-08-19] [PRO] Investigation : résolution réelle des photos scrapées pour le projet neck-reset → Résultat :
- **Étape 0 du plan** (`docs/management/plans/NECK_RESET_VISION_PLAN.md`) exécutée via un nouveau workflow CI dédié (`.github/workflows/run_script.yml`, branche `ops/run-script`, séparé de `deploy.yml`/production — répertoire scratch, jamais de redémarrage du service) : `backend/scripts/export_neck_reset_sample.py` a mesuré **894 annonces acoustique/classique éligibles** (7 utilisateurs), échantillon de 50 exporté en HTML (artefact GitHub Actions).
- **Résolution effective calculée à ~1,36 mm/px** sur des photos 720×960px typiques : suffisante pour la mire de calibration par motif de frettes (~13px/case à la 12e frette) mais **insuffisante pour une mesure continue en mm de la hauteur d'action** (~0,7-1,1px de signal utile pour un écart réel de 1-1,5mm — sous le seuil fiable même en détection sub-pixel).
- **Cause tracée dans le code, pas supposée** : `backend/repository.py::upload_images_to_storage()` confirmé pass-through pur (aucun resize côté Guitar Hunter). Le plafond vient de Facebook (`backend/scraping/parser.py` lit `img.src` du DOM sans vérifier `srcset`) — confirmé par une URL réelle exposant le paramètre CDN `stp=dst-jpg_s960x960_tt6`. Contournement testé et fermé : modifier la taille demandée dans l'URL renvoie `HTTP 403 "URL signature mismatch"` (le token `oh=` signe l'URL entière).
- **Kijiji comparé et écarté comme alternative** (`backend/scripts/compare_image_resolution_by_source.py`, 857 Facebook vs 37 Kijiji annonces éligibles) : Kijiji légèrement *pire* (640×798px/41 Ko moyen) que Facebook (668×960px/93 Ko moyen), malgré une source de données plus structurée (`__NEXT_DATA__`) — hypothèse initiale invalidée par les faits. Aucun mode zoom accessible côté interface Facebook (desktop/mobile, confirmé par l'utilisateur).
- **Décision produit** : la sortie du score de Phase 2/3 du plan passe d'une estimation continue en mm à une classification grossière (basse/normale/haute/non mesurable) — détail complet dans `NECK_RESET_VISION_PLAN.md` §3quater.
- **2 bugs corrigés en cours de route** sur `run_script.yml` : ordre d'écriture des secrets vs `git clone` (dossier non-vide), puis `git clone` remplacé par `git init`+`fetch`+`reset --hard` (tolère les fichiers résiduels d'un run précédent interrompu) — les deux commits de fix sont dans l'historique de la branche.

[2026-08-19] [PRO] Réflexion : refonte du plan "neck reset" après revue externe (Fable) → Résultat :
- **Demande utilisateur** : faire réviser `NECK_RESET_VISION_PLAN.md` par un second modèle (Fable, via subagent) en cherchant des failles techniques et des alternatives open source/gratuites plus fiables.
- **Failles identifiées par la revue** : homographie mal posée (6 keypoints quasi colinéaires, guitare non plane), Fretboard Shadowing non calibrable sans contrôle de l'éclairage, Saddle Profiler non invariant à la couleur du sillet (casse sur Micarta/Graph Tech noirs), Bellying Scanner probablement corrélé à la compression JPEG plutôt qu'à une vraie déformation, absence de référence d'échelle et de mécanisme d'abstention, licence AGPL d'Ultralytics/YOLOv8 incompatible avec l'objectif open source. Pas de verdict de faisabilité binaire rendu — dépend d'une **Étape 0** non encore mesurée (taux réel de photos d'annonces exploitables sur les 700 déjà en base).
- **Alternatives retenues et intégrées au plan** : motif des frettes comme mire de calibration naturelle (LSD/RANSAC, zéro entraînement) pour Phase 1 ; mesure d'action en "diamètres de corde" (étalon métrique local, filtres de crêtes `scikit-image`) remplaçant Fretboard Shadowing et Saddle Profiler ; Florence-2 (MIT) + SAM 2.1 (Apache) remplaçant Grounding DINO/Gemini Flash pour la labellisation automatisée (§3bis), avec validation par résidu géométrique chiffré plutôt que comparaison visuelle ; MAPIE (conformal prediction) pour un score de sortie calibré avec option "non mesurable".
- **Deux clarifications apportées par l'utilisateur, intégrées** : (1) la vue de profil/en visée est rare dans les vraies annonces (~90% des cas) — découplage explicite entre la hauteur d'action (mesure locale, tolérante à l'angle, exploitable plus largement) et l'angle du manche par visée (nécessite une vue de profil, traité comme signal bonus) ; (2) le protocole déjà prévu (photos en conditions variées à chaque réglage de cale) protège contre la confusion "conditions de prise de vue" mais pas contre la confusion "apparence de cette guitare précise" (bois/chevalet constants sur toutes les photos d'une même guitare) — d'où l'ajout d'un critère formel de validation croisée leave-one-guitare-out au jalon go/no-go.
- **Plan restructuré** : nouvelle §2ter (résumé de la revue externe), Étape 0 ajoutée en tête du plan de match (§4), jalon go/no-go étendu à 3 critères formels (taux de photos exploitables, résidu géométrique des keypoints, corrélation leave-one-guitare-out). Aucun code écrit — session 100% réflexion/documentation.
...
- **Lancement confirmé** (run GitHub Actions #341, logs du step "Deploy via SSH") : `[run_once] Backfill léger des ventes corrompues lancé en arrière-plan (log: .../backfill_sold_scores.log).`, suivi du redémarrage réussi du service (`✅ Service guitare-hunter restarté avec succès`). Le contenu détaillé de `backfill_sold_scores.log` (nombre d'annonces effectivement backfillées, éventuels échecs) reste **non consultable depuis cet environnement de dev** (accès SSH requis) — à vérifier par l'utilisateur, ou lors d'une prochaine session avec accès au serveur.
- **`run_once.py` repassé à `ACTIVE = False`** dans ce commit, immédiatement après confirmation du lancement (protocole `CLAUDE.md`).
