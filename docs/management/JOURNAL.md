...
[2026-09-13] [PRO] `analyzer.py` : le Portier conserve aussi son propre verdict brut (`gatekeeperVerdict`), pas seulement marque/classification.
- **Contexte** : suite à une consultation Opus indépendante (branche `claude/guitarhunter-benchmark-setup-h9q9gr`) sur l'optimisation du Tier 1 — Opus a noté que le prompt du Portier lui demande déjà un verdict parmi `PEPITE, FAST_FLIP, LUTHIER_PROJ, CASE_WIN, COLLECTION, FAIR, BAD_DEAL` (pas seulement accept/reject), et a proposé un garde-fou pour un futur routage T1→T2/T3 basé directement sur ce verdict (`verdict_T1 ∈ {PEPITE, FAST_FLIP, ...}` = toujours promouvoir). Ce verdict brut n'était pas conservé (comme `gatekeeperBrand`/`gatekeeperClassification` avant l'entrée précédente) — impossible de vérifier, une fois assez de volume accumulé, si des annonces à `deal_score` élevé avaient un verdict T1 = FAIR (ce qui déciderait si ce garde-fou est sûr).
- **`backend/analyzer.py::_run_analysis_cascade`** : `gatekeeperVerdict` (= `gatekeeper_status`) ajouté sur les mêmes 4 chemins de retour déjà instrumentés pour `gatekeeperBrand`/`gatekeeperClassification`. Purement additif, aucun appel supplémentaire.
- **Reste à faire** : une fois quelques jours de volume accumulés (même délai que l'entrée précédente), un script croisera `gatekeeperVerdict` avec le `deal_score` final pour valider le garde-fou proposé par Opus.

[2026-09-12] [PRO] `analyzer.py` : le Portier (Tier 1) conserve désormais sa propre marque/classification devinée, même quand il accepte l'annonce.
- **Contexte** : travail exploratoire mené sur la branche séparée `claude/guitarhunter-benchmark-setup-h9q9gr` (harnais de benchmark, comparatifs de modèles) autour d'un chantier de routage T1→T2/T3 par recherche ciblée (piste de réduction de coût : ne promouvoir vers les Tiers chers que ce qui correspond à une recherche active de l'utilisateur, ou une pépite). Avant tout code de routage, il faut savoir si le Portier (`gemini-3.5-flash-lite`) identifie assez bien marque/type de corps pour qu'on puisse s'y fier. Or `_run_analysis_cascade` jetait jusqu'ici la sortie du Portier (`brand`/`classification`) dès qu'il acceptait l'annonce — seul le chemin de rejet la conservait (`gatekeeper_classification`) — rendant impossible toute comparaison entre la sortie T1 et le résultat final T2/T3 sur les annonces acceptées, qui sont pourtant la grande majorité des cas.
- **`backend/analyzer.py::_run_analysis_cascade`** : capture `gatekeeper_brand`/`gatekeeper_classification` dès la réponse du Portier, et les attache (`gatekeeperBrand`/`gatekeeperClassification`) sur TOUS les chemins de retour de la cascade (rejet, pas de déclenchement T3, T3 déclenché, erreur T2, erreur T3 avec repli T2) — pas seulement le rejet comme avant. Ces clés se retrouvent automatiquement dans `aiAnalysis` (les fonctions de `repository.py` y écrivent le dict retourné tel quel, aucun changement nécessaire côté persistance).
- **Purement additif** : aucun appel Gemini supplémentaire (le Portier calculait déjà ces champs, ils étaient juste jetés), aucun changement de prompt ni de comportement de la cascade, zéro risque de régression. Testé localement sur les 5 chemins de retour avec des doubles avant ce déploiement.
- **Objectif** : laisser s'accumuler quelques jours de données de production réelles, puis comparer `gatekeeperBrand`/`gatekeeperClassification` (T1) à ce que T2/T3 stockent finalement dans `aiAnalysis` — taux d'accord réel sur un vrai volume, sans avoir à rejouer artificiellement le Portier sur d'anciennes annonces. Script d'analyse pas encore écrit (à faire une fois assez de volume accumulé).
