...
- **Intégré** dans `CHANTIER_B_PERCEPTION_RAISONNEMENT_PLAN.md` (§1, §2, §8.1/§8.3 marqués tranchés/actés, verdict mis à jour). Reste bloquant avant code : écrire le contrat de perception complet (8.3, champ par champ du JSON de prod).

[2026-09-07] [PRO] Correction de cadrage Chantier B/F — deux motivations distinctes (coût vs fiabilité), consultation Opus précédente partiellement invalidée.
- **Erreur signalée par l'utilisateur** : le plan Chantier B avait été présenté (à moi puis à Opus) comme motivé par une "fiabilisation du Tier 3", ce qui a fait dévier toute l'analyse Opus vers une question hors sujet ("Gemini T3 a-t-il un vrai problème de perception ?", §8.6 de l'entrée précédente). Ce n'était pas la bonne question.
- **Cadrage corrigé, deux chantiers strictement séparés** : Chantier B = trouver un modèle de perception visuelle moins cher que Gemini pour le travail déjà fait par T1/T2 (Qwen3.8-Flash, GPT bon marché, ou autre), motivé par le **coût**, sans aucun jugement sur la fiabilité de Gemini. Chantier F = remplacer Gemini 3.1 Pro par Claude Sonnet 5 au Tier 3, motivé par la **fiabilité factuelle** ("il ne vérifie pas les données" — illustré par l'épisode du PDF fabriqué), selon deux variantes à trancher : (a) remplacement intégral (Sonnet 5 fait vision+raisonnement, indépendant de B) ou (b) remplacement du seul raisonnement (Sonnet 5 nourri par la perception bon marché de B, qui devient alors un prérequis de cette variante).
- **Conséquence sur l'objection 8.6 de la consultation Opus précédente** : ne s'applique plus telle quelle — "aucune preuve que T3 a un problème" n'est pas pertinent pour un chantier motivé par le coût, pas par un échec constaté. Reformulée en le vrai critère : le candidat bon marché égale-t-il la qualité de perception actuelle, à prix inférieur ? — c'est exactement ce que le benchmark (§5 du plan) doit mesurer, pas un prérequis séparé. Les autres objections Opus (8.1 où insérer la perception, 8.3 contrat de sortie à dériver du JSON de prod, 8.4 mesurabilité du harnais, 8.5 format des rectangles, 8.7 cohérence chat, 8.8 chiffrage) restent valides et bloquantes, indépendantes de la motivation.
- **Intégré** dans `CHANTIER_B_PERCEPTION_RAISONNEMENT_PLAN.md` (nouvelle §0 "Correction de cadrage", §1bis liant B et F) et `COST_OPTIMIZATION_CHANTIERS.md` (Chantier F, précision de motivation + variantes a/b). Aucun code écrit — les points 8.1/8.3 restent à trancher avant toute implémentation.

[2026-09-07] [PRO] Consultation Claude Opus sur le plan Chantier B → verdict "non prêt pour l'implémentation", décisions d'architecture manquantes.
- **Contexte** : suite à la formalisation du plan (entrée juste en dessous) et à sa soumission à Claude Opus pour second avis critique, même protocole que la consultation précédente sur `COST_OPTIMIZATION_CHANTIERS.md`.
- **Verdict** : la gouvernance du plan (motivation, ordre benchmark→code, refus de le vendre comme une économie) est jugée saine, mais **la décision d'architecture centrale n'est pas tranchée** — où s'insère la perception dans la cascade T1→T2→T3 (les 3 Tiers reçoivent aujourd'hui les mêmes photos, `analyzer.py:298-299/326/359/417`), avec deux chemins non couverts (`force_expert` qui saute T1, `analyze_deal_light` qui attaque T2 directement). Le §1 du plan était auto-contradictoire selon la réponse à cette question.
- **Angle mort jugé le plus important** : **aucune preuve dans le dépôt qu'un problème réel existe** — T3 (`gemini-3.1-pro-preview`) est le meilleur modèle vision du pipeline, tous les candidats de perception proposés sont plus faibles, le seul incident documenté (Qwen/Guerrilla) est une hallucination d'interprétation, pas un échec de perception de Gemini T3. Opus recommande de rassembler 3-5 échecs T3 réels de production imputables à la perception avant tout code — gisements déjà identifiés dans le projet (`initialVerdict`/`initialModelUsed`, `requalificationProposalState === 'applied'`).
- **Autres défauts identifiés** : le garde-fou anti-interprétation `hybrid` réutilisé "tel quel" fuit déjà (demande la "qualité apparente de fabrication", `candidates.py:203-204/209-211` — jugement, pas transcription) ; le contrat de sortie de perception doit être dérivé champ par champ du contrat JSON de production (`color`/`finish_application`/`finish_texture`/`visual_inspection`), pas copié du prompt de benchmark ; le critère de rollout §5 hérite silencieusement de 3 défauts du harnais déjà connus (juge qui ne voit jamais la sortie intermédiaire, absence de candidat "vraie cascade de prod" comme baseline, règle de notation punitive à réécrire) ; la motivation "cohérence avec le chat" n'est pas tenue par le plan tel qu'écrit (le chat n'injecte aujourd'hui ni `analysis` ni `visual_inspection`) ; correction de chiffrage sur le gain coût de la variante externe (Qwen).
- **Intégré** dans `CHANTIER_B_PERCEPTION_RAISONNEMENT_PLAN.md` (nouvelle section §8, statut du document mis à jour). Aucun code écrit — les décisions d'architecture (8.1, 8.3) et la collecte de preuve (8.6) restent à trancher avec l'utilisateur avant toute suite.

[2026-09-07] [PRO] Chantier B formalisé en plan d'implémentation dédié → `CHANTIER_B_PERCEPTION_RAISONNEMENT_PLAN.md`, consultation Opus lancée, aucun code de production touché.
- **Contexte** : suite à la synthèse des chantiers coûts (entrées plus bas) et à la discussion sur la fiabilité du Tier 3 comme source d'analyse "complète, factuelle et vérifiable" (identification Gibson/copie, année), l'utilisateur a demandé de formaliser le Chantier B (séparation perception/raisonnement) et de lancer la reconstruction du dataset de benchmark.
- **Deux corrections de portée explicites de l'utilisateur, appliquées avant de soumettre le plan** : (1) le garde-fou anti-interprétation reste scopé au logo pour cette itération, sa généralisation (bois, matériel, année) reportée à une discussion dédiée — point ouvert ajouté au `TODO.md` plutôt que tranché au fil de l'eau ; (2) la localisation des parties de guitare (rectangles) ne réutilise PAS le pipeline OWLv2/Dell déjà validé sur `NECK_RESET_VISION_PLAN.md` — l'utilisateur veut observer ce que les modèles candidats du benchmark produisent eux-mêmes pour cette tâche, exploratoire et sans notation automatique pour l'instant, plutôt que de les comparer à une référence externe précalculée.
- **Soumis à Claude Opus** pour un second avis critique (même protocole que la consultation précédente sur `COST_OPTIMIZATION_CHANTIERS.md`) avant tout code — résultat à intégrer dans une entrée séparée une fois rendu.

[2026-09-07] [FLASH] Vérification d'un rapport comparatif IA fourni par l'utilisateur → généré par Gemini 3.1 Pro Thinking, tarifs faux/périmés, un modèle inventé, méthodologie de scoring obsolète.
- **Contexte** : PDF ("Benchmark API IA : Scores, Tarifs et Évaluation — Septembre 2026 V6") comparant GPT-6 Astra, GPT-5.6 Sol, Gemini 3.1 Pro, Claude Sonnet 5, GPT-6 Mini, Qwen3.8 Max/Flash, concluant à un remplacement de Gemini 3.1 Pro par Sonnet 5 pour Guitar Hunter. Généré par Gemini 3.1 Pro Thinking lui-même, après 3 rondes de correction et un forçage à citer ses sources — confiance de l'utilisateur dans le résultat : 0%. Vérification demandée explicitement ("ne pas tenir compte des valeurs de ce document et vérifier les datas").
- **Méthode** : WebSearch pour chaque tarif/modèle cité + skill `claude-api` (cache 2026-06-24) pour les prix Claude Sonnet 5.
- **Résultat** : Gemini 3.1 Pro surestimé de 25% (2,50$/15,00$ annoncés contre 2,00$/12,00$ réels, alors que ce tarif exact est déjà câblé dans `run_once.py::PRICING["t3"]` — le document se contredit avec les propres données de facturation du projet). GPT-5.6 Sol au tarif d'avant la baisse du 24/08/2026. Qwen3.8-Flash surestimé ×3,5. "GPT-6 Mini" n'existe pas dans le catalogue OpenAI — modèle et tarif inventés. Tableau de scores MMLU/MMMU/HumanEval construit sur des benchmarks saturés/contaminés depuis longtemps pour tout modèle de pointe 2026 (plus publiés par aucun labo) — chiffres sans source vérifiable. Sur les benchmarks 2026 réels et pertinents pour l'usage (vision) : GPQA Diamond Sonnet 5 96,2% vs Gemini 3.1 Pro 94,3%, mais **MMMU-Pro (vision) Sonnet 5 76,3% vs Gemini 3.1 Pro 80,5% — Gemini devant**, contredisant la conclusion du document sur l'axe qui compte le plus pour le Tier 3 de production.
- **Conséquence** : intégré dans `COST_OPTIMIZATION_CHANTIERS.md` (Chantier F) — confirme plutôt que remplace la nécessité du harnais de benchmark maison (Chantier D) pour trancher sur des données propres au projet plutôt que sur des rapports génériques, potentiellement eux-mêmes générés par le modèle qu'ils évaluent.

[2026-09-07] [PRO] Enquête coût Gemini clôturée — calcul bottom-up sur la période de facturation réelle fournie par l'utilisateur → écart de 3,4% avec la facture, mystère résolu.
- **Contexte** : les entrées précédentes (2026-09-06, ci-dessous) avaient invalidé la piste "2048px" et proposé une instrumentation `usage_metadata` à laisser tourner plusieurs jours pour mesurer le vrai coût. L'utilisateur a fourni directement le CSV de facturation GeminiDev de la période 1-6 septembre 2026 ($9,76 côté Gemini API) et demandé une reconstruction bottom-up complète plutôt que d'attendre l'accumulation de logs.
- **Méthode** (`backend/scripts/run_once.py`, armé/exécuté/désarmé en une fois — run GitHub Actions #421, lecture seule) : pour chaque annonce dont `timestamp` tombe dans la fenêtre de facturation, reconstruction du coût de la cascade T1→T2→T3 (constantes mesurées sur `prompts.json` : prompt fixe par Tier, ~900 tokens/photo, sortie réelle du champ `aiAnalysis.analysis` quand disponible) ET, si une sous-collection `chat` existe pour cette annonce, du coût cumulé de toute la conversation (même modèle `gemini-3.1-pro-preview` que le Tier 3, chaque tour repayant l'historique complet — méthode déjà validée sur une conversation réelle isolée, entrée non journalisée séparément).
- **Résultat mesuré** : 1377 annonces dans la fenêtre. Photos/annonce : moyenne 3,5, médiane 3, max 10 (confirme que l'hypothèse "8 photos/annonce" utilisée dans une estimation antérieure était largement surestimée). Répartition par Tier atteint : T1 seul = 971 (70%), T2 = 342 (25%), T3 = 64 (5%). 19 annonces avec conversation de chat (46 appels modèle). Coût cascade ≈ 9,10$, coût chat ≈ 0,33$, **TOTAL bottom-up ≈ 9,4282$ contre facture réelle 9,76$ (écart 3,4%, dans la marge d'erreur du modèle)**.
- **Conclusion** : le modèle de coût par annonce était juste depuis le début — ce qui faisait dévier les estimations mensuelles précédentes (~12-14$/mois) de la réalité (~82-87$/mois observés en août) n'était pas une erreur de modélisation mais un volume mal calibré : le snapshot utilisé (`analyze_funnel_by_user.py`, run #408, 30 jours) mesurait 95,03 annonces/jour, alors que la période facturée réelle tourne à ~229 annonces/jour (1377/6j) — 2,4x plus. Cause du delta de volume non confirmée (backfill ponctuel, réanalyses en masse, ou simplement plus d'activité récente que le snapshot vieux de 30 jours) — à surveiller si le prochain rapprochement facture/modèle dévie à nouveau.
- **Chat confirmé comme poste de coût réel** (jusque-là non compté séparément dans aucune estimation antérieure) : ~3,5% du total sur cette période, mais utilise le même modèle que le Tier 3 ($12/M tokens de sortie) avec renvoi intégral de l'historique à chaque tour — un chantier de compression pourrait s'appliquer ici aussi si le volume de chat augmente.
- **`backend/scripts/run_once.py`** : ACTIVE repassé à `False` (commit séparé), résultats consignés dans le docstring de `run()`.

[2026-09-07] [PRO] Harnais de benchmark vision étendu — GPT-5-mini, Qwen3.8-flash (TokenRouter), candidat hybride et candidat de compression ajoutés → codé sur `claude/guitarhunter-benchmark-setup-h9q9gr`, pas encore exécuté avec les vraies clés API.
- **Contexte** : suite au comparatif de tarifs Gemini du 2026-09-06 (entrée plus bas), l'utilisateur a demandé un vrai test mesuré plutôt qu'une comparaison sur la seule base du prix (`TODO.md`, point "Test comparatif GPT-4o-mini / Qwen2.5-VL"). `backend/benchmark/` (candidates.py, judge.py, run_benchmark.py, dataset.json) déjà en place depuis une session précédente pour comparer Tier 2/Tier 3 Gemini ; étendu ici.
- **Corrections de modèles en cours de route** : `gpt-4o-mini` retiré de l'API OpenAI depuis février 2026 → remplacé par `gpt-5-mini` ($0,25/$2,00 par M tokens, vision confirmée). `qwen2.5-vl-72b-instruct` n'existe plus (lignée Qwen3 depuis 2026) → `qwen/qwen3.8-flash` retenu via TokenRouter (compte déjà créé par l'utilisateur, endpoint `https://api.tokenrouter.com/v1` compatible OpenAI). Un nom `qwen/qwen3-vl-32b-instruct` cité par une recherche web (classement OCRBench-V2) s'est avéré ne pas exister chez ce fournisseur — corrigé après que l'utilisateur l'a signalé via son propre tableau de bord OpenRouter ; commentaire de code ajouté pour ne pas réutiliser ce nom.
- **Candidat "hybrid" (`call_hybrid_qwen_gemini`)** : conçu et corrigé deux fois sur retour explicite de l'utilisateur avant la version retenue. V1 (rejetée) renvoyait les photos à Gemini en plus du rapport Qwen — l'utilisateur : "il ne faut pas renvoyer les photos à Gemini". V2 (rejetée) faisait deux appels Qwen séparés — l'utilisateur : "pourquoi deux appels vers Qwen, un seul pourrait suffire". Version retenue : **un seul appel Qwen** produit un rapport texte en deux parties (état physique + transcription OCR du logo/marque SANS jugement d'interprétation), puis Gemini Tier 3 (l'"oracle") ne voit JAMAIS les photos, seulement ce texte, et fait l'identification lui-même. Origine de la contrainte "sans interprétation" : Qwen avait hallucinné "marque budget OEM" sur une annonce Guerrilla Guitars (luthier artisanal québécois réel, confirmé par recherche web) — l'erreur venait du jugement de marque, pas de la lecture du logo elle-même.
- **Candidat "gemini_pro_compact" (`call_gemini_pro_compact`)** : teste si forcer le Tier 3 (prompt de prod exigeant un rapport "EXHAUSTIF") à répondre en puces strictes, puis faire réécrire cette sortie en prose par `gemini-3.5-flash-lite` (déjà utilisé comme Portier T1), dégrade le raisonnement par rapport au rapport exhaustif habituel. Objectif si le score reste équivalent : ~55% d'économie sur le poste de sortie du Tier 3 ($12/M tokens, le plus cher de la cascade).
- **`judge.py`** : deux correctifs requis par la montée de version du SDK Anthropic (>=1.0, 2026-08-20) — `temperature` retiré de la signature de `messages.create()` (plus de contrôle de reproductibilité possible avec cette version), et `claude-sonnet-5` ayant l'"adaptive thinking" activé par défaut, un `ThinkingBlock` pouvait consommer tout le `max_tokens` avant de produire le JSON attendu → `thinking={"type": "disabled"}` ajouté, recherche du premier bloc `type=="text"` plutôt que `content[0]`.
- **`CANDIDATES`** (`candidates.py`) : 6 candidats enregistrés — `gemini` (T2 prod), `gemini_pro` (T3, plafond qualité de référence), `gemini_pro_compact`, `gpt4o_mini` (clé `OPENAI_API_KEY` fournie par l'utilisateur), `qwen`, `hybrid`.
- **Non exécuté** : aucun run réel de `run_benchmark.py` avec les vraies clés API depuis cet environnement de dev dans cette session — reste à lancer par l'utilisateur (ou dans un contexte avec accès aux clés) pour obtenir les scores comparatifs réels.
