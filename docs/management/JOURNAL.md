...
- **Annonces "dans le bruit" sans label (frontend)** : cause identifiée en lisant `analyzer.py` — un double-échec IA (Tier 1 **et** Tier 2 en erreur) produit un verdict `ERROR_GATEKEEPER` avec un `reasoning` non vide. Ce verdict échappait à la fois au filtre `isError` de `useDealsManager.js` (qui ne détectait que `DEFAULT`/`ERROR` littéraux ou reasoning vide) et à `ARCHIVE_GROUP` (`constants.js`) → il atterrissait dans la vue "Toutes" avec le badge par défaut trompeur ("Analyse..."). `isError` élargi pour couvrir tout verdict absent de la taxonomie connue (`NEW_VERDICTS`/`LEGACY_VERDICTS`/`ARCHIVE_GROUP`), pas seulement les valeurs littérales `DEFAULT`/`ERROR`.
- **Favoris incluant du bruit (frontend)** : `matchesVerdictFilter` retournait `deal.isFavorite` sans filtrer le verdict pour le cas `FAVORITES`. Exclusion désormais des verdicts archivés (`REJECTED_ITEM`, `REJECTED_SERVICE`, `INCOMPLETE_DATA`, `REJECTED`) et des erreurs, en gardant `BAD_DEAL` ("trop cher" confirmé comme favori légitime par l'utilisateur). Le compteur `verdictCounts.FAVORITES` (2 emplacements) corrigé pour rester cohérent avec la liste affichée.
- **Annonces supprimées de Facebook encore visibles (backend, diagnostiqué, non corrigé)** : le bot détecte déjà les annonces supprimées par le vendeur (404, redirection vers l'accueil Marketplace dans `check_listing_availability`, `backend/scraping/core.py`) — fusionnées avec la détection de vente (`status: 'sold'`, pas de statut "supprimé" distinct). Le nettoyage (`cleanup_sold_listings`) tourne automatiquement toutes les 24h (`schedule.every(24).hours`, `backend/services.py`) → latence normale jusqu'à 24h, pas un bug. Le "ralentissement du chargement" perçu vient probablement plutôt du fait qu'`onDealsUpdate` charge **toute** la collection `guitar_deals` (y compris vendues/rejetées, jamais purgées) à chaque connexion. Ces deux constats ajoutés au `TODO.md`, non corrigés (décision produit à trancher : réduire l'intervalle ? filtrer/paginer côté serveur ?).
- **Persistance des filtres par utilisateur (Firestore)** : `useBotConfig.js` — nouvel état `uiFilters` (lu depuis le doc utilisateur au même titre que `scanConfig`/`analysisConfig`) + `saveUiFilters` (écriture debouncée 800ms). `DealsContext.jsx` relaie vers `useDealsManager`, qui hydrate une seule fois au premier chargement puis sauvegarde automatiquement à chaque changement (`filterType`, niveaux 1-4, condition, prix, `sortMode`) — la recherche texte libre n'est pas persistée.
- **Vérifié** : build/console propres (`npm run dev`), rendu de la page de login OK. Non testé en conditions réelles avec des annonces (nécessite authentification) — confirmé fonctionnel par l'utilisateur avant push.
...
[2026-07-13] [PRO] Fix : Menu "Ré-analyser" (options de rescan) de la carte annonce inaccessible au survol sur desktop → Résultat :
- **Symptôme signalé** : sur l'interface ordinateur, les options du menu Ré-analyser (Scan Standard, Luthier Expert, Avec commentaire...) disparaissent souvent avant qu'on puisse cliquer dessus ; quand on y arrive, rien ne se passe (ni rescan lancé, ni popup de commentaire). Meilleur fonctionnement sur mobile.
- **Cause confirmée** : même famille de bug que le fix Navbar du 2026-07-11 (ci-dessous), mais sur un gap **vertical** cette fois. Le conteneur `.relative` qui gère `onMouseEnter`/`onMouseLeave` (`DealCard.jsx`) n'englobe que le bouton RefreshCw (40×40px) ; le menu déroulant était positionné avec `bottom-full mb-2` (carte) / `top-full mt-2` (modale) — un `margin`, donc hors de la boîte du conteneur survolé. En traversant ce vide (~8px), le curseur sortait de `:hover` avant d'atteindre le menu, qui se démontait en plein trajet. Si ce démontage survenait pendant le clic (mousedown/mouseup), le clic ne touchait plus rien — d'où le second symptôme.
- **`src/components/DealCard.jsx`** : `margin` remplacé par `padding` sur un wrapper englobant (`pb-2`/`pt-2` au lieu de `mb-2`/`mt-2`), avec le style visuel du menu déplacé dans un `<div>` interne — le gap fait désormais partie de la zone survolée, comme pour la Navbar. Appliqué aux deux emplacements dupliqués : footer de carte (rendu direct) et `renderActionButtons` (utilisé par la modale d'analyse IA).
- **Fix vérifié avant/après** : reproduction isolée (HTML/Tailwind servi en local, hors app, sans dépendance Firebase) — la version "margin" se ferme bien quand le curseur traverse le gap, empêchant tout clic ; la version "padding" reste ouverte dans la même zone et le clic déclenche bien l'action. Non testé en conditions réelles dans l'app (nécessite authentification) — à confirmer par l'utilisateur.
...
- **`src/components/Navbar.jsx`** : ajout de `justify-center lg:min-w-[190px]` au conteneur `.group` (calibré sur la largeur max du menu à 4 boutons), pour que sa zone de survol couvre déjà toute la largeur du menu à toute hauteur. Limité à `lg:` (desktop) pour ne pas élargir inutilement la barre de navigation sur mobile.
- **Fix vérifié avant implémentation** : reproduction du bug puis du fix dans un fichier HTML/CSS isolé (classes équivalentes, sans dépendance Firebase), via Playwright — échec de clic confirmé sur les boutons excentrés avant le fix, succès sur les 4 boutons après.
