...
- **TTL de 3 jours** (`FirestoreHandler`, champ `expireAt`) : les logs Firestore sont volontairement éphémères. Concrètement découvert le 2026-09-08 en voulant mesurer l'effet d'un correctif déployé la veille : les logs `[tokens]` "avant" avaient déjà expiré, comparaison perdue.
- **Archive locale sur le serveur (2026-09-08)** : en plus du `FirestoreHandler`, le logger per-utilisateur reçoit désormais un `TimedRotatingFileHandler` (`backend/logging_config.py`, dossier `logs/` du serveur, un fichier par utilisateur, rotation quotidienne UTC) — non soumis au TTL de 3 jours, actif même en mode offline. `backend/log_retention.py` (job quotidien, `main.py`, singleton comme `admin_stats`) compresse `.gz` au-delà de 30 jours et supprime au-delà d'un an. Ce canal est pensé pour l'investigation a posteriori (scripts `run_once.py`, accès SSH) — le LogViewer continue de lire exclusivement Firestore, sans changement.
