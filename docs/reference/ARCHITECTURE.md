...
- **Rejet des annonces déjà vendues, filet de sécurité IA (2026-08-06)** : `gatekeeper_verbosity_instruction` instruit désormais explicitement le Portier (T1) de rejeter (`REJECTED_ITEM`) toute annonce dont le titre/description signale une vente déjà conclue ("VENDU", "SOLD", "deal closed"...), même si l'annonce est encore en ligne. Complète le filtre pré-IA basé sur `SOLD_MARKERS` (`bot.py::handle_deal_found()`, liste figée sur titre + 200 premiers caractères de description) sans le remplacer — ce filtre pré-IA est volontairement **contourné pour un scan manuel** (`is_manual_scan=True`), donc ce garde-fou côté Portier reste le seul filet pour ce chemin, et couvre aussi les formulations non présentes dans la liste figée. Même limite que ci-dessus : non rétroactif sur les prompts déjà personnalisés en Firestore.
...
- **Tableau de Bord de Statistiques (`StatsView.jsx`) :** Composant agrégeant les données de Firestore.
    ...
    - **Statistiques croisées (2026-08-06)** : 6 croisements, tous basés sur `analysisDeals` (index léger uniquement, aucune lecture Firestore supplémentaire) — voir `docs/explanation/STATS_REFLEXION.md` §2/§4/§5/§8 pour le détail produit. `priceScoreData` (score IA moyen par tranche de prix), `categoryData` (marge moyenne par catégorie), `aiAccuracyData` (% de score élevé parmi les annonces vendues vs l'ensemble), `sourceComparisonData` (Facebook vs Kijiji, source dérivée du préfixe `kijiji_` de l'ID — pas de `link` dans l'index léger), `geoOpportunityData` (volume/marge des verdicts `RADAR_GROUP` par ville), `liquiditySpeedData` (délai de vente réel moyen par tranche de `liquidity_score` prédit — valide la prédiction de liquidité de l'IA contre le comportement réel observé).
      - **Résolution de catégorie simplifiée** : `resolveCategoryLabel()` (module-level, `StatsView.jsx`) reconstruit une table de lookup exact+leaf depuis `taxonomy_master` (`prompts.json`), **sans** la recherche floue par sous-chaîne de `useDealsManager.js::findPathFuzzy` — dette technique acceptée (duplication partielle) pour limiter le risque à un composant d'affichage plutôt que de toucher la résolution de taxonomie partagée par le filtrage.
