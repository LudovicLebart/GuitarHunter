...
- **`src/components/DealCard/DealCardActions.jsx` — Bouton "Discuter sur Gemini" (2026-07-31)** : `buildGeminiChatPrompt(deal)` (`utils.js`) construit un texte (annonce + description + liens photo `storageImageUrls` + analyse IA existante) copié dans le presse-papier, puis ouvre `gemini.google.com/app` dans un nouvel onglet pour que l'utilisateur poursuive la conversation. **Testé en conditions réelles par l'utilisateur** : gemini.google.com n'a pas de mécanisme de préremplissage par URL (`?q=` initialement tenté, retiré après confirmation que ça ne fonctionne pas) — le presse-papier est le seul mécanisme de transfert, l'utilisateur doit coller manuellement. Les photos ne sont pas jointes (limite du presse-papier texte vers un site externe) : leurs URLs publiques pérennes (Firebase Storage) sont incluses comme liens cliquables dans le prompt à la place.
...
- **Bouton "Discuter sur Gemini" (`DealCardActions.jsx`, branche `feature/discuter-gemini`, 2026-07-31, non fusionné) :** Alternative volontairement plus simple qu'un chat IA intégré (pas de backend, pas de nouvelle collection Firestore) — l'utilisateur voulait pouvoir poursuivre la conversation sur une annonce déjà analysée sans reconstruire un chat complet. `DealCard/utils.js::buildGeminiChatPrompt(deal)` construit un prompt texte (titre, prix, localisation, description, lien, identification IA — marque/modèle/année/pays/couleur —, analyse financière, verdict, résumé/raisonnement) et une phrase invitant Gemini à poursuivre la discussion. Le bouton copie ce texte dans le presse-papier (`navigator.clipboard`, repli garanti) puis ouvre `gemini.google.com/app?q={prompt encodé}` dans un nouvel onglet, avec un plafond de longueur (`GEMINI_URL_PREFILL_LIMIT=6000`) au-delà duquel seule l'URL sans paramètre est ouverte pour éviter une URL disproportionnée. **Non validé en conditions réelles** : gemini.google.com n'expose aucun mécanisme documenté/officiel de préremplissage par URL, et Playwright n'a pas pu charger le site depuis cet environnement (proxy MITM du sandbox, `net::ERR_CONNECTION_RESET` — même limite déjà rencontrée sur les tests live Kijiji) pour vérifier si `?q=` préremplit réellement le champ de saisie. Le presse-papier reste le repli garanti indépendamment de ce résultat. À valider par l'utilisateur avant fusion dans `dev`.
