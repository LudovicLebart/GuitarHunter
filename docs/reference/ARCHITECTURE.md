...
### `backend/scraping/kijiji/` (Nouveau, 2026-07-26 — module autonome, pas encore branché)
- **`KijijiScraper`** (`core.py`) : Scraper Playwright pour Kijiji.ca, calqué sur `FacebookScraper` — même forme de `listing_data` en sortie (`title`/`price`/`description`/`imageUrl(s)`/`link`/`location`/`id`), plus `source: "kijiji"`. Fournit `scan_marketplace()`, `scan_specific_url()` (test isolé d'une annonce) et `check_listing_availability()` (miroir Facebook, pour le nettoyage périodique).
    - **Recherche via le champ de recherche du site** plutôt qu'une URL construite à la main : contrairement à Facebook (`city_mapping`/`get_city_id_and_coords`), il n'existe pas encore de mapping ville → ID de lieu numérique Kijiji. Le filtre de lieu tente de sélectionner une suggestion d'autocomplétion ; sans mapping, il peut ne pas s'appliquer si aucune suggestion n'apparaît.
    - **`KijijiListingParser`** (`parser.py`) : extraction de l'ID (dernier segment numérique du chemin `/v-<catégorie>/<lieu>/<titre>/<id>` — pas le premier, un slug intermédiaire peut aussi être numérique) et parsing de la fiche détail en 2 temps : JSON-LD (`<script type="application/ld+json">`, standard schema.org, plus stable qu'un sélecteur CSS) en priorité, repli sur des sélecteurs `data-testid`/DOM sinon. Réutilise `ListingParser.extract_price_from_text` (module Facebook) plutôt que de le dupliquer.
    - **Écarts assumés vs `FacebookScraper`** (pas des oublis) : pas de rotation de proxy, pas de géolocalisation forcée, pas de filtre par liste blanche de villes — dépendent tous du mapping ville→ID Kijiji qui n'existe pas encore côté produit.
    - **⚠️ Sélecteurs non validés en conditions réelles** : développé dans un environnement dont la politique réseau bloque `kijiji.ca` — les sélecteurs `data-testid` et le parcours de recherche sont basés sur la structure connue de Kijiji (avec replis génériques) mais nécessitent une validation manuelle sur le site réel avant intégration à `bot.py::run_scan()`.
    - **Non intégré au pipeline** : le module n'est référencé nulle part dans `bot.py`/`main.py` — c'est un scraper autonome et testable isolément (`backend/scraping/kijiji/test_kijiji_core.py`, 19 tests), en attendant une décision produit sur le branchement (config Firestore dédiée, fusion ou séparation des annonces Facebook/Kijiji dans `guitar_deals`).
