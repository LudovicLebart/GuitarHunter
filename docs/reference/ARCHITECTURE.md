...
- **`run_scan()`:** Déclenche le scraping des villes configurées. Régulé par le `scheduler`. Scraper instancié localement par city avec sémaphore protection.
  - **Filtre STRICT multi-villes (2026-07-21, mode `distance=0`)** : `scan_marketplace()` accepte déjà toute annonce dont la localisation correspond à **l'une des villes** de la liste autorisée (`is_city_allowed()`), pas seulement celle recherchée. `run_scan()` applique ensuite une logique à 2 voies sur le résultat : ville recherchée → traitée normalement ; **autre ville autorisée de la liste** → traitée immédiatement (au lieu d'être jetée après avoir payé le coût de la fiche détail complète — gaspillage confirmé, l'annonce était perdue ou refetchée intégralement une seconde fois au tour de cette autre ville). *(Simplifié de 3 à 2 voies le 2026-07-24 : la 3e branche "hors liste" était du code mort, `is_city_allowed()` garantissant déjà qu'aucune annonce ne peut arriver ici hors de la liste autorisée.)*
  - **Comptabilisation des échecs de cycle (2026-07-21, étendu 2026-07-24)** : `scan_marketplace()` retourne un dict (`deals`, `anti_bot_blocked`, `rejected_out_of_list`, `total_cards_seen`) au lieu d'une simple liste. `handle_deal_found()` retourne un code de statut par annonce (`scrape_failed`, `sold_marker`, `already_rejected`, `duplicate_unchanged`, `rejected_prefilter`, `over_budget_prefilter`, `processed`, `processed_update`). `run_scan()` agrège le tout dans un résumé de cycle (`📊 Résumé du cycle : ...`) loggé en fin de scan, incluant désormais `total_cards_seen` et le compteur `unknown` (visible seulement si non nul) — permet de distinguer un volume Facebook réellement bas d'annonces perdues côté scraper.
- **`handle_deal_found(listing_data, is_manual_scan=False, skip_price_prefilter=False)`:** Callback appelé par le scraper (`scan_marketplace()`, `scan_specific_url()`, et depuis 2026-07-24 `run_leboncoin_scan()`) pour chaque annonce trouvée. Orchestre : (1) upload des images vers Firebase Storage (`repo.upload_images_to_storage()`), (2) injection de `storageImageUrls` dans les données, (3) appel à l'analyseur IA, (4) sauvegarde dans Firestore. Retourne un code de statut (voir ci-dessus) plutôt que `None`. `skip_price_prefilter` (LeBonCoin) désactive le plafond `scanConfig.max_price` — sans objet quand la recherche elle-même filtre déjà par prix côté site.
...
- **`notify_scan_url_finished()` (2026-07-21, révisé 2026-07-24)** : Message adapté au code de statut retourné par `handle_deal_found()` (`processed`, `processed_update`, `duplicate_unchanged`, `already_rejected`, `rejected_prefilter`, `over_budget_prefilter`, `scrape_failed`, `sold_marker`), capturé par `bot.py::scan_specific_url()`. Inclut un lien direct vers l'annonce dans Guitar Hunter (`?dealId={id}`, construit via `_build_deal_link()`, factorisé avec `notify_deal()`) en plus du lien Facebook d'origine — omis pour `scrape_failed`/`sold_marker` (sortent de `handle_deal_found()` avant toute écriture Firestore, donc avant qu'un tel lien soit valide).
...
- **`FacebookScraper` n'hérite PAS de cette base** — code en production, migration délibérément laissée pour un chantier séparé (proxy, géolocalisation, en-têtes spécifiques à gérer). Voir `docs/management/TODO.md`.
...
- **`bot.py::run_leboncoin_scan()`** : réutilise `handle_deal_found()` tel quel via l'adaptateur `_map_leboncoin_ad()` (conversion du schéma d'annonce LeBonCoin vers le schéma Facebook) — pas de pipeline dupliqué. IDs préfixés `lbc_` (évite toute collision avec les ID Facebook dans `guitar_deals`). Le `LeboncoinScraper` est instancié une seule fois et gardé vivant pour toute la durée du thread bot. `known_ids` réel alimenté par `repository.py::get_known_deal_ids()` (lit les 20 chunks de `deals_index`, comme le frontend). Cadence planifiée indépendamment de Facebook, jitterisée +/-30% autour de la fréquence de scan Facebook courante (`services.py::TaskScheduler`), avec vérification `is_night_time()` avant chaque cycle. Contrôlé par `leboncoinConfig.enabled` (Firestore, `false` par défaut).
