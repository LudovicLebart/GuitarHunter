...
- **`run_scan()`:** Point d'entrée du cycle de scan planifié (régulé par le `scheduler`). Délègue tout le travail à **`_run_sources_in_parallel(scan_config, cities_to_scan)`** (2026-07-27) : lance `_run_facebook_scan()` si `scanConfig.facebook_enabled` (toggle "Source Facebook" dans `ConfigPanel`, **activé par défaut** — absent = `True`, pour ne pas désactiver silencieusement Facebook sur un compte existant créé avant ce réglage), et `_run_kijiji_scan()` si `scanConfig.kijiji_enabled` (toggle "Source Kijiji (bêta)") — chacun dans son propre thread (`threading.Thread`, daemon) si activé, rejoints (`join()`) avant que `run_scan()` ne considère le cycle terminé. Les deux sources sont désactivables indépendamment (2026-07-27, demande utilisateur — isoler un scan Kijiji seul, sans le bruit de Facebook dans les logs partagés, pour déboguer une absence de résultats Kijiji) ; si aucune des deux n'est activée, le cycle est ignoré avec un `warning` explicite plutôt qu'un scan silencieusement vide. Le cycle complet dure `max(durée Facebook, durée Kijiji)` au lieu de `Facebook + Kijiji` en séquentiel quand les deux tournent. LeBonCoin (voir plus bas) n'en fait PAS partie — cadence indépendante, planifiée séparément par `services.py::TaskScheduler`. Sûr à paralléliser : aucune des sources n'écrit le même document Firestore (IDs Kijiji préfixés `kijiji_`, LeBonCoin `lbc_`), `session_processed_ids` est isolé par thread (`threading.local()`), et `_browser_semaphore` (thread-safe) continue de plafonner le nombre réel de navigateurs Playwright ouverts simultanément, indépendamment du nombre de threads.
  - **`_run_facebook_scan(scan_config, cities_to_scan)`** : boucle par ville, scraper instancié localement par ville avec protection par sémaphore.
    - **Filtre STRICT multi-villes (mode `distance=0`)** : `scan_marketplace()` accepte déjà toute annonce dont la localisation correspond à **l'une des villes** de la liste autorisée (`is_city_allowed()`), pas seulement celle recherchée. `_run_facebook_scan()` applique ensuite une logique à 2 voies sur le résultat : ville recherchée → traitée normalement ; **autre ville autorisée de la liste** → traitée immédiatement (au lieu d'être jetée après avoir payé le coût de la fiche détail complète). *(Simplifiée de 3 à 2 voies : la 3e branche "hors liste" est du code mort, `is_city_allowed()` garantissant déjà qu'aucune annonce ne peut arriver ici hors de la liste autorisée.)*
    - **Comptabilisation des échecs de cycle** : `scan_marketplace()` retourne un dict (`deals`, `anti_bot_blocked`, `rejected_out_of_list`, `total_cards_seen`) au lieu d'une simple liste. `handle_deal_found()` retourne un code de statut par annonce (`scrape_failed`, `sold_marker`, `marked_sold`, `already_rejected`, `duplicate_unchanged`, `duplicate_cross_platform`, `rejected_prefilter`, `over_budget_prefilter`, `processed`, `processed_update`). `_run_facebook_scan()` agrège le tout dans un résumé de cycle (`📊 Résumé du cycle Facebook : ...`) loggé en fin de scan, incluant `total_cards_seen` et le compteur `unknown` (visible seulement si non nul).
  ...
  - **Logs tagués par source (2026-07-27)** : Facebook, Kijiji et LeBonCoin pouvant tourner en parallèle et écrire dans le même logger (`self.logger`), leurs lignes s'entremêlent dans le LogViewer — `handle_deal_found()` préfixe donc chacun de ses logs par `[Facebook]`/`[Kijiji]`/`[LeBonCoin]` (paramètre `source`, défaut `"Facebook"` pour ne pas casser les appelants historiques).
- **`handle_deal_found(listing_data, is_manual_scan=False, skip_price_prefilter=False, source="Facebook")`:** Callback appelé par le scraper (`scan_marketplace()`, `scan_city()`, `scan_specific_url()`, et `run_leboncoin_scan()`) pour chaque annonce trouvée. Orchestre : (1) upload des images vers Firebase Storage (`repo.upload_images_to_storage()`), (2) injection de `storageImageUrls` dans les données, (3) appel à l'analyseur IA, (4) sauvegarde dans Firestore. Retourne un code de statut (voir ci-dessus) plutôt que `None`. `skip_price_prefilter` (LeBonCoin) désactive le plafond `scanConfig.max_price` — sans objet quand la recherche elle-même filtre déjà par prix côté site.
  ...
  - **Pré-filtres avant analyse IA** : en plus de `_check_exclusion()` (mots-clés → `verdict: REJECTED`, stocké, code retour `rejected_prefilter`), un plafond de prix défensif compare le prix scrapé à `scanConfig.max_price` — filet de sécurité résiduel derrière le filtre de prix appliqué côté recherche (Facebook : `minPrice`/`maxPrice` dans l'URL + `_apply_filters()` ; Kijiji : `price=min__max` dans l'URL, voir plus bas ; LeBonCoin : `leboncoinConfig.min_price`/`max_price` dans l'URL de recherche, via `skip_price_prefilter=True`), qui peut échouer silencieusement (ex: timeout sur un champ de saisie Facebook). **Depuis le 2026-07-27**, une annonce hors budget est **ignorée sans écriture Firestore** (code retour `over_budget_prefilter`) plutôt que stockée avec un verdict `BAD_DEAL` — hors budget est traité comme hors périmètre de recherche, pas comme une "mauvaise annonce" à archiver ; comportement uniforme Facebook/Kijiji/LeBonCoin. `BAD_DEAL` reste un verdict IA légitime (voir `prompts.json`) quand l'*analyse* elle-même juge le prix excessif relativement à la valeur estimée — seul le raccourci de pré-filtre a été retiré (décision produit itérée suite à un signalement utilisateur, voir JOURNAL.md 2026-07-27).
...
- **`notify_scan_url_finished()` (2026-07-21, révisé 2026-07-24/27)** : Message adapté au code de statut retourné par `handle_deal_found()` (`processed`, `processed_update`, `duplicate_unchanged`, `duplicate_cross_platform`, `already_rejected`, `marked_sold`, `rejected_prefilter`, `over_budget_prefilter`, `scrape_failed`, `sold_marker`), capturé par `bot.py::scan_specific_url()`. Inclut un lien direct vers l'annonce dans Guitar Hunter (`?dealId={id}`, construit via `_build_deal_link()`, factorisé avec `notify_deal()`) en plus du lien d'origine — omis pour les issues qui sortent de `handle_deal_found()` avant toute écriture Firestore (`scrape_failed`, `sold_marker`, `over_budget_prefilter`, `duplicate_cross_platform`). **`source` (2026-07-27, défaut `"Facebook"`)** : étiquette correctement le message ("URL Facebook :"/"URL Kijiji :"/"URL LeBonCoin :") — auparavant toujours "URL Facebook" même pour un scan Kijiji, symptôme du même bug que `scan_specific_url()` ci-dessus (dispatch absent).
...
### `backend/scraping_common/` (Nouveau, 2026-07-24)
- **`BaseMarketplaceScraper`** (`base_scraper.py`) : cycle de vie de session Playwright (`start_session`/`close_session`/`_ensure_session`/`_get_page`), UA/viewports par défaut, `storage_state_path` optionnel (session authentifiée ou anonyme). À sous-classer par un scraper de site spécifique — `LeboncoinScraper` en hérite ; pensé pour de futurs modules (Reverb...).
- **`HumanBehaviorMixin` / `is_night_time()` / `seconds_until_active()`** (`human_behavior.py`) : comportement "humain" générique (pauses non-uniformes, trajectoires de souris interpolées, scroll par paliers, survol d'élément) et plage nocturne (`Europe/Paris` explicite, 00h-7h) — aucune dépendance à un site précis.
- **`FacebookScraper`/`KijijiScraper` n'héritent PAS de cette base** — code en production, migration délibérément laissée pour un chantier séparé (proxy, géolocalisation, en-têtes spécifiques à gérer). Voir `docs/management/TODO.md`.

### `backend/scraping_leboncoin/` (Nouveau, 2026-07-22 — intégré à `bot.py` le 2026-07-24)
- **`LeboncoinScraper(BaseMarketplaceScraper)`** : approche Playwright "douce" (mêmes mesures stealth que `FacebookScraper`, sans contournement actif de DataDome type SSL Pinning/TLS spoofing). Compte LeBonCoin personnel unique, pas multi-tenant.
- **Extraction** : parse le JSON structuré `__NEXT_DATA__` embarqué par LeBonCoin (Next.js) plutôt que des sélecteurs CSS fragiles — liste blanche stricte de champs, le bloc `owner` (données vendeur) n'est jamais extrait.
- **Pagination** : suit `max_pages` annoncé par le site lui-même ; arrêt anticipé via `known_ids` (le tri `sort=time&order=desc` garantissant "plus récent d'abord", dès qu'une page ne contient plus d'annonce inconnue, la suite est forcément plus ancienne).
- **Comportement anti-détection** : session/onglet réutilisés sur toute la durée de vie du scraper (jamais fermé/rouvert entre deux recherches), temps de lecture minimum par page (~45-75s) terminant par une descente jusqu'aux boutons de pagination, actions décoratives aléatoires (ouverture d'annonce, ajout aux favoris) — aucune ne sert à l'extraction, uniquement à la vraisemblance comportementale.
- **`bot.py::run_leboncoin_scan()`** : réutilise `handle_deal_found()` tel quel via l'adaptateur `_map_leboncoin_ad()` (conversion du schéma d'annonce LeBonCoin vers le schéma Facebook) — pas de pipeline dupliqué. IDs préfixés `lbc_` (évite toute collision avec les ID Facebook/Kijiji dans `guitar_deals`). Le `LeboncoinScraper` est instancié une seule fois et gardé vivant pour toute la durée du thread bot (sémaphore `_browser_semaphore` acquis une seule fois pour toute cette durée de vie, pas par cycle). `known_ids` réel alimenté par `repository.py::get_known_deal_ids(prefix="lbc_")` (lit les 20 chunks de `deals_index`, comme le frontend) — le préfixe `lbc_` est retiré avant l'appel à `search()`, qui compare contre des IDs bruts. Cadence planifiée indépendamment de Facebook/Kijiji (pas dans `_run_sources_in_parallel`), jitterisée +/-30% autour de la fréquence de scan Facebook courante (`services.py::TaskScheduler`, tag unique par instance), avec vérification `is_night_time()` avant chaque cycle et un verrou (`threading.Lock`) contre un double déclenchement (thread bot + watchdog global partageant le même scheduler `schedule`). Contrôlé par `leboncoinConfig.enabled` (Firestore, `false` par défaut).
