...
- **Tableau de Bord de Statistiques (`StatsView.jsx`) :** Composant agrégeant les données de Firestore.
    ...
    - **Statistiques croisées (2026-08-06)** : 5 croisements, tous basés sur `enrichedAllDeals`/`enrichedDeals` (index léger uniquement, aucune lecture Firestore supplémentaire) — voir `docs/explanation/STATS_REFLEXION.md` §2/§4/§5/§8 pour le détail produit. `priceScoreData` (score IA moyen par tranche de prix), `categoryData` (marge moyenne par catégorie), `aiAccuracyData` (% de score élevé parmi les annonces vendues vs l'ensemble), `sourceComparisonData` (Facebook vs Kijiji, source dérivée du préfixe `kijiji_` de l'ID — pas de `link` dans l'index léger), `geoOpportunityData` (volume/marge des verdicts `RADAR_GROUP` par ville).
