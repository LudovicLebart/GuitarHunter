...
- ✅ **Vitesse de vente réelle vs Liquidité prédite (implémenté 2026-08-06, refondu 2026-08-17)** : `StatsView.jsx::liquidityCorrelation` — nuage de points (un point = une vente, score `liquidity_score` en abscisse, délai réel `soldTimestamp - publishTimestamp` en jours en ordonnée) et coefficient de corrélation de Pearson sur les valeurs individuelles. Répond directement à la question "l'IA prédit-elle correctement quelles annonces se vendront vite ?". ⚠️ **La version initiale (par tranches Faible 0-4/Moyenne 5-7/Élevée 8-10, moyenne du score par tranche) produisait une droite quasi garantie par construction** — les tranches étant définies par ce même score, leur moyenne interne ne pouvait qu'augmenter avec l'ordre des tranches, peu importe la vraie relation avec la vitesse de vente (repéré par l'utilisateur : "les résultats prédits par l'IA forment une droite, c'est très suspect"). Le calcul sur valeurs brutes ne peut plus produire cet artefact.
