...
import { Target, ShoppingBag, Archive, Search } from 'lucide-react';
...
const FAKE_DEALS = [
    {
        ...
        location: "Mont-St-Hilaire, QC", taxonomy: "Acoustique_Vocal", type: 'acoustique',
    ...
    },
    {
        ...
        location: "Montréal, QC", taxonomy: "Électrique", type: 'electrique',
    ...
    },
    {
        ...
        location: "Laval, QC", taxonomy: "Électrique", type: 'electrique',
    ...
    },
    {
        ...
        location: "Brossard, QC", taxonomy: "Acoustique_Vocal", type: 'accessoire',
    ...
    },
    {
        ...
        location: "Longueuil, QC", taxonomy: "Acoustique_Vocal", type: 'acoustique',
    ...
    },
    {
        ...
        location: "Verdun, QC", taxonomy: "Basse", type: 'basse',
    ...
    },
    {
        ...
        location: "Repentigny, QC", taxonomy: "Électrique", type: 'electrique',
    ...
    },
    {
        ...
        location: "Saint-Hyacinthe, QC", taxonomy: "Électrique", type: 'electrique',
...
const MockupDashboard = ({ onClose }) => {
    ...
    // Quick verdict tabs (top of page)
    const VERDICT_TABS = [
        { id: 'ALL', label: 'Toutes', count: FAKE_DEALS.length },
        ...['PEPITE', 'FAST_FLIP', 'LUTHIER_PROJ', 'CASE_WIN', 'COLLECTION', 'BAD_DEAL']
            .map(id => ({
                id,
                label: ALL_FILTERS_CONFIG[id]?.pluralLabel || ALL_FILTERS_CONFIG[id]?.label || id,
                count: FAKE_DEALS.filter(d => d.verdict === id).length,
            }))
            .filter(t => t.count > 0),
    ];
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 mb-6">
                    <div className="relative mb-4">
                        ...
                        <input
                            ...
                            className="w-full bg-slate-800 border border-slate-700 rounded-xl pl-9 pr-4 py-2.5 text-sm text-slate-200 placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all"
                        />
                    </div>
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-1">
                        {VERDICT_TABS.map(tab => (
                            <button
                                key={tab.id}
                                onClick={() => handleFilterChange('verdict', tab.id)}
                                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-bold whitespace-nowrap transition-all shrink-0 ${filters.verdict === tab.id
                                    ? 'bg-blue-600 text-white shadow-md shadow-blue-500/20'
                                    : 'bg-slate-800 text-slate-400 hover:bg-slate-700 hover:text-slate-200 border border-slate-700'
                                    }`}
                            >
                                {tab.label}
                                <span className={`font-mono ${filters.verdict === tab.id ? 'text-blue-200' : 'text-slate-600'}`}>
                                    {tab.count}
                                </span>
                            </button>
                        ))}
